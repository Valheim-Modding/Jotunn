...
|$error_worldfileload|월드 파일을 로드하는 동안 오류가 발생했습니다. 백업을 확인하세요!|
...
|$menu_addserverfailed|서버를 추가하지 못했습니다!|
|$menu_addserverfailedincorrectformatting|입력값이 유효한 IP, URL 또는 초대 코드가 아닙니다!|
|$menu_addserverfailedresolvejoincode|가입 코드를 확인할 수 없습니다.|
|$menu_back|뒤로|
|$menu_backupcount|백업 $1|
|$menu_backuprestorefailedheader|백업을 복원하지 못했습니다!|
|$menu_backuprestorepromptheader|백업을 복원하시겠습니까?|
|$menu_backuprestorepromptrecover|삭제된 파일 $1은(는) $2에서 복원됩니다.|
|$menu_backuprestorepromptreplace|$1의 현재 버전이 백업되고 $2로 대체됩니다.|
|$menu_beard|수염|
|$menu_blondness|금발|
|$menu_cancel|취소|
|$menu_changelog|패치노트|
...
|$menu_cloudstoragefull|클라우드 저장용량이 가득 찼습니다!|
|$menu_cloudstoragefullcharacterprompt|이 캐릭터를 로컬 저장소에 저장하시겠습니까?|
|$menu_cloudstoragefullmovingsave|데이터 손실을 방지하기 위해 현재 세션의 저장 데이터를 로컬 저장소로 이동할 수 있습니다.|
|$menu_cloudstoragefulloperationfailed|클라우드 저장소가 가득 차서 작업을 실패했습니다. 일부 월드를 로컬 저장소로 이동하고 다시 시도하세요.|
|$menu_cloudstoragefullworldprompt|이 월드를 로컬 저장소에 저장하시겠습니까?|
|$menu_cloudstorageused|사용된 클라우드 저장소: $1/$2|
|$menu_collapse|접기|
|$menu_community|커뮤니티|
...
|$menu_done|완료|
|$menu_duplicatefilepromptheader|저장을 이동할 수 없습니다|
|$menu_duplicatefileprompttext|대상 위치에 이름이 $1인 저장이 이미 있습니다.|
|$menu_enteripinvite|IP 또는 초대 코드 입력|
...
|$menu_filter|필터|
|$menu_friends|친구|
|$menu_gamepreviewpopupheader|Valheim은 게임 미리보기에 있습니다.|
|$menu_gamepreviewpopuptext|일부 플랫폼 또는 게임 기능이 제대로 작동하지 않고 게임이 중단될 수 있습니다.|
|$menu_hair|머리카락|
|$menu_hairtone|머리색|
|$menu_ipportorinvitecode|[IP/URL]:[포트] 또는 [6자리 초대 코드]|
|$menu_joincode|참여 코드 $1|
|$menu_joindifferentserver|다른 서버에 참여합니다!|
|$menu_joingame|게임 참가|
|$menu_joinip|IP 참가|
...
|$menu_logoutprompt|현재 세션에서 로그아웃하시겠습니까?|
|$menu_male|남성|
|$menu_managesaves|저장 관리|
...
|$menu_movefailedheader|파일 이동을 실패했습니다!|
...
|$menu_newworldalreadyexistsmessage|$1(이)라는 월드가 이미 존재합니다!|
|$menu_no|아니요|
|$menu_ok|확인|
|$menu_password|비밀번호|
...
|$menu_tryagainorrestart|문제가 지속되면 다시 시도하거나 게임을 다시 시작하세요.|
...
|$msg_worldsavedcloudstoragefull|클라우드 저장소가 가득 찼습니다! 월드가 로컬 저장소로 이동되어 저장되었습니다.|
