...
|$menu_acceptedinvite|您已經接受邀請！|
|$menu_addserver|新增伺服器|
|$menu_addserverfailed|無法新增伺服器！|
|$menu_addserverfailedincorrectformatting|輸入的 IP、URL 或邀請代碼無效！|
|$menu_addserverfailedresolvejoincode|無法解析連結代碼|
|$menu_back|返回|
|$menu_backupcount|$1 個備份|
|$menu_backuprestorefailedheader|無法還原備份！|
|$menu_backuprestorepromptheader|是否還原備份？|
|$menu_backuprestorepromptrecover|已刪除的檔案 $1 將從 $2 還原。|
|$menu_backuprestorepromptreplace|目前版本的 $1 將會備份，並改由 $2 取代。|
|$menu_beard|鬍鬚|
|$menu_blondness|金髮度|
|$menu_cancel|取消|
|$menu_changelog|變更紀錄|
...
|$menu_cloudstoragefullcharacterprompt|您要改將這個角色儲存到本機存放區嗎？|
|$menu_cloudstoragefullmovingsave|為避免資料遺失，目前工作階段的儲存資料可能已移至本機存放區。|
|$menu_cloudstoragefulloperationfailed|由於您的雲端儲存空間已滿，此作業失敗。請將部分世界移至本機存放區，然後再試一次。|
|$menu_cloudstoragefullworldprompt|您要改將這個世界儲存到本機存放區嗎？|
...
|$menu_craft_tab|製作|
|$menu_credits|開發者名單|
|$menu_crossplayserver|跨平台遊戲|
|$menu_deleted|已刪除|
|$menu_disallowedcharactername|Can't create a character with this name|
|$menu_done|完成|
|$menu_duplicatefilepromptheader|無法移動儲存資料|
|$menu_duplicatefileprompttext|目的地位置已經有名稱為 $1 的儲存資料。|
...
|$menu_filter|篩選|
|$menu_friends|好友|
|$menu_gamepreviewpopupheader|《Valheim》已在「遊戲預覽」中推出|
|$menu_gamepreviewpopuptext|部分平台或遊戲功能可能無法正常運作，並因此導致遊戲當掉。|
|$menu_hair|頭髮|
|$menu_hairtone|髮色|
|$menu_ipportorinvitecode|[IP/URL]:[連接埠] 或 [6 位數邀請代碼]|
|$menu_joincode|連結代碼 $1|
|$menu_joindifferentserver|您將加入其他伺服器！|
|$menu_joingame|加入遊戲|
|$menu_joinip|加入 IP|
...
|$menu_newworld|新世界|
|$menu_newworldalreadyexists|世界已經存在！|
|$menu_newworldalreadyexistsmessage|名稱為 $1 的世界已經存在|
|$menu_no|否|
|$menu_ok|確定|
|$menu_password|密碼|
...
|$menu_tryagainorrestart|請再試一次，如果問題仍然存在，請重新啟動遊戲|
|$menu_upgrade_tab|升級|
...
|$msg_worldsavedcloudstoragefull|雲端儲存空間已滿！該世界已移至本機存放區並儲存|
