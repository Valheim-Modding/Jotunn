# Turkish translations list
All of the translation keys and Turkish text currently in the game.
This file is automatically generated from Valheim 0.221.12 using the JotunnDoc mod found on our GitHub.

|Key A-M |Turkish Text |
|---|---|
|$#piece_sapcollector_check|Çıkarıcıyı kontrol et|
|$alternate_key_label|Alternatif işlev: +|
|$ammo_arrows|ok|
|$ammo_bolts|Arbalet oku|
|$ammo_turretbolt|Karademir balista oku|
|$ammo_turretboltwood|Tahta balista oku|
|$ancient_chest|Kadim tabut|
|$animal_fish|Balık|
|$animal_fish1|Tatlı Su Levreği|
|$animal_fish10|Kuzey somonu|
|$animal_fish11|Magmabalığı|
|$animal_fish12|Balonbalığı|
|$animal_fish2|Turna|
|$animal_fish3|Ton balığı|
|$animal_fish4|Tetra|
|$animal_fish5|Trolbalığı|
|$animal_fish6|Dev ringa|
|$animal_fish7|Orfoz|
|$animal_fish8|Mercan morinası|
|$animal_fish9|Fenerbalığı|
|$base_item_modifier|Üs eşyası enerji kullanımı|
|$biome_ashlands|Kül Diyarı|
|$biome_blackforest|Kara Orman|
|$biome_darklands|Kara Topraklar|
|$biome_deepnorth|Kara Kuzey|
|$biome_edge|Dünyanın Kıyısı|
|$biome_forest|Orman|
|$biome_heath|Fundalık|
|$biome_meadows|Çayır|
|$biome_mistlands|Sisli Topraklar|
|$biome_mountain|Dağ|
|$biome_ocean|Okyanus|
|$biome_plains|Ova|
|$biome_swamp|Bataklık|
|$boss_hive_end|Dağıldık.|
|$boss_hive_start|Toplandık.|
|$button_a|A|
|$button_b|B|
|$button_back|Geri|
|$button_lalt|Sol alt|
|$button_lb|LB|
|$button_lcommand|L-Command|
|$button_lctrl|Sol ctrl|
|$button_ls|LS|
|$button_lshift|Sol Shift|
|$button_mouse0|Fare-1|
|$button_mouse1|Fare-2|
|$button_mouse2|Fare-3|
|$button_ps_a|✕|
|$button_ps_b|○|
|$button_ps_back|Share|
|$button_ps_lb|L1|
|$button_ps_ls|L3|
|$button_ps_rb|R1|
|$button_ps_rs|R3|
|$button_ps_start|Options|
|$button_ps_x|▫|
|$button_ps_y|△|
|$button_ralt|Sağ alt|
|$button_rb|RB|
|$button_rcommand|R-Command|
|$button_rctrl|Sağ ctrl|
|$button_return|Giriş|
|$button_rs|RS|
|$button_rshift|Sağ shift|
|$button_space|Boşluk|
|$button_start|Başlat|
|$button_x|X|
|$button_y|Y|
|$caption_absorbingproj|absorbing projectile|
|$caption_agreeing|agreeing|
|$caption_alerted|alerted|
|$caption_altar_empowered|Altar empowered|
|$caption_arriving|arriving|
|$caption_atgeir|Atgeir|
|$caption_attacking|attacking|
|$caption_awakening|awakening|
|$caption_axe|Axe|
|$caption_barking|barking|
|$caption_biting|biting|
|$caption_blockedattack|Attack blocked|
|$caption_blocking|blocking|
|$caption_born|being born|
|$caption_bow|Bow|
|$caption_breakingapart|breaking apart|
|$caption_breathingfire|breathing fire|
|$caption_bubbling|bubbling|
|$caption_building|building|
|$caption_burningfood|burning food|
|$caption_burp|Burping|
|$caption_burrowing|burrowing|
|$caption_bye|saying goodbye|
|$caption_calling|calling|
|$caption_callingforaid|calling for aid|
|$caption_casting|casting|
|$caption_castingmagic|casting|
|$caption_castingspell|casting a spell|
|$caption_catchfire|Something catching fire|
|$caption_cawing|cawing|
|$caption_charging|charging|
|$caption_chittering|chittering|
|$caption_closing|closing|
|$caption_creaking|creaking|
|$caption_destroyed|destroyed|
|$caption_dispersing|dispersing|
|$caption_dodging|dodging|
|$caption_donecooking|done cooking|
|$caption_doorclose|Door closing|
|$caption_dooropen|Door opening|
|$caption_drawn|being drawn|
|$caption_droppingitem|dropping an item|
|$caption_dying|dying|
|$caption_eating|eating|
|$caption_exploding|exploding|
|$caption_explosion|Explosion|
|$caption_firework_explode|Firework exploding|
|$caption_firing|firing|
|$caption_fizzling|fizzling|
|$caption_food_burnt|Food burnt|
|$caption_food_done|Food cooked|
|$caption_fueladded|Object being refueled|
|$caption_gateclose|Gate closing|
|$caption_gateopen|Gate opening|
|$caption_gettinghit|getting hit|
|$caption_ghostnoises|whooshing|
|$caption_groaning|groaning|
|$caption_growing|growing|
|$caption_growling|growling|
|$caption_grumbling|grumbling|
|$caption_grunting|grunting|
|$caption_healing|healing|
|$caption_hello|greeting|
|$caption_hit|striking|
|$caption_hitting|hitting|
|$caption_howling|howling|
|$caption_hurt|hurt|
|$caption_iceshatter|Ice shattering|
|$caption_inhaling|inhaling|
|$caption_itemcrafted|Item crafted|
|$caption_jumping|jumping|
|$caption_knife|Knife|
|$caption_laughing|laughing|
|$caption_levelup|Skill increased!|
|$caption_lightingfire|lighting fire|
|$caption_linebreak|line snapping|
|$caption_loaded|loaded|
|$caption_meteor_exploding|Meteor exploding|
|$caption_moving|moving|
|$caption_opening|opening|
|$caption_parried|Attack parried|
|$caption_platform_crumbling|Lava platform crumbling|
|$caption_platform_rising|Platform rising|
|$caption_player|Player|
|$caption_portalopen|Portal opening|
|$caption_potion|Potion consumed|
|$caption_projexploding|projectile exploding|
|$caption_puking|Puking|
|$caption_punching|Punching|
|$caption_rattling|rattling|
|$caption_ravencaw|Raven cawing|
|$caption_ravenflap|Raven flapping|
|$caption_recharging|recharging|
|$caption_refilled|refilled|
|$caption_refueled|refueled|
|$caption_reloading|reloading|
|$caption_repaired|Item repaired|
|$caption_rising|rising|
|$caption_rockdestroyed|Rock breaking|
|$caption_ruminating|ruminating|
|$caption_screeching|screeching|
|$caption_seagull|Seagull|
|$caption_shaking|shaking|
|$caption_shattering|shattering|
|$caption_shedding|shedding|
|$caption_shivering|shivering|
|$caption_shouting|shouting|
|$caption_shuttingdown|shutting down|
|$caption_sighing|sighing|
|$caption_snarling|snarling|
|$caption_spear|Spear|
|$caption_spinning|spinning|
|$caption_spitexploding|spit exploding|
|$caption_spitting|spitting|
|$caption_splashing|splashing|
|$caption_sputtering|sputtering|
|$caption_squawking|squawking|
|$caption_startup|starting up|
|$caption_stomping|stomping|
|$caption_summoning|summoning|
|$caption_sweeping|sweeping|
|$caption_swiping|swiping|
|$caption_swung|being swung|
|$caption_takingdamage|taking damage|
|$caption_takingoff|taking off|
|$caption_talking|talking|
|$caption_throwing|throwing|
|$caption_thunder|Distant thunder|
|$caption_tilling|tilling|
|$caption_trading|is pleased with a trade|
|$caption_treefall|Tree falling|
|$caption_used|used|
|$charredfortress|Kor Kale|
|$chat_entermessage|Sohbet mesajını gir|
|$chat_entertext|Metni girin...|
|$chat_kill|Kendini öldür|
|$chat_resetspawn|Canlanma noktasını sıfırla|
|$chat_shout|Bağır|
|$chat_whisper|Özel mesaj|
|$customization_beard01|Uzun 1|
|$customization_beard02|Uzun 2|
|$customization_beard03|Kısa 1|
|$customization_beard04|Kısa 2|
|$customization_beard05|Örgülü 1|
|$customization_beard06|Örgülü 2|
|$customization_beard07|Kısa 3|
|$customization_beard08|Yoğun 1|
|$customization_beard09|Örgülü 3|
|$customization_beard10|Örgülü 4|
|$customization_beard11|Kalın 2|
|$customization_beard12|Kraliyet 1|
|$customization_beard13|Kraliyet 2|
|$customization_beard14|Örgülü 5|
|$customization_beard15|Kısa 4|
|$customization_beard16|Taşsakini|
|$customization_beard17|Sade|
|$customization_beard18|Kraliyet 3|
|$customization_beard19|Gür|
|$customization_beard20|Dikenli|
|$customization_beard21|Kısa 5|
|$customization_beard22|Bıyık|
|$customization_beard23|Kırıntı Kapanı|
|$customization_beard24|Jöleli|
|$customization_beard25|Tıraşlı|
|$customization_beard26|Tutamaç|
|$customization_hair01|Atkuyruğu 1|
|$customization_hair02|Atkuyruğu 2|
|$customization_hair03|Örgülü 1|
|$customization_hair04|Atkuyruğu 3|
|$customization_hair05|Kısa 1|
|$customization_hair06|Uzun 1|
|$customization_hair07|Atkuyruğu 4|
|$customization_hair08|Kısa 2|
|$customization_hair09|Yana Yatık 1|
|$customization_hair10|Yana Yatık 2|
|$customization_hair11|Örgülü 2|
|$customization_hair12|Örgülü 3|
|$customization_hair13|Örgülü 4|
|$customization_hair14|Yana Yatık 3|
|$customization_hair15|Arkada toplanmış bukleler|
|$customization_hair16|Toplu örgüler|
|$customization_hair17|Düzenli örgüler|
|$customization_hair18|Kraliyet örgüleri|
|$customization_hair19|Bukleli 1|
|$customization_hair20|Bukleli 2|
|$customization_hair21|İkili topuz|
|$customization_hair22|Tek topuz|
|$customization_hair23|Kısa bukleli|
|$customization_hair24|Tıraşlı ve örgülü|
|$customization_hair25|Tek topuz 2|
|$customization_hair26|Kısa örgü|
|$customization_hair27|Kuvvet örgüsü|
|$customization_hair28|Tüccar örgüsü|
|$customization_hair29|Salık 2|
|$customization_hair30|Salık 3|
|$customization_hair31|Toplu örgü|
|$customization_hair32|Mullet|
|$customization_hair33|Vinland Tıraşı|
|$customization_hair34|Kale Muhafızı|
|$customization_hair35|Şampiyon|
|$customization_hair36|Vakanüvis|
|$customization_hair37|Güneşgetiren|
|$customization_nobeard|Sakal yok|
|$customization_nohair|Saç Yok|
|$deadspeak_bonemass|Minnettarız, savaşçı. Bu surette yaşayan bizler, ölümle gelen sükuneti haddinden fazla erteledik. Artık vakti geldi...|
|$deadspeak_eikthyr|Demek ölümüm senin ellerinden olacaktı? Oysa ne de küçük ve kırılgansın... Odin'e söyle, bilsin. Bu bedeni alt etmiş olabilir ama vahşi doğa onun kurallarına hiçbir zaman boyun eğmeyecek...|
|$deadspeak_elder|Kan ve kemikten olma zavallı yaratık, seni ince bir dal gibi kırmalıydım! Şimdiyse yitip gidiyor, ulu ağaca teslim oluyorum...|
|$deadspeak_fader|Yanlış bir kaderin peşinden gittim ve beni mahvetmekten başka bir şey yapmadı. Beni affet sevgilim...|
|$deadspeak_fish_shared1|Endişelenme. Mutlu ol.|
|$deadspeak_fish_shared2|Beni nehre götür... Sonra suya bırak...|
|$deadspeak_fish1|Ben tatlı su levreğiyim, sıradan levrek değil!|
|$deadspeak_fish10|Bir zamanlar akıntıya karşı zıplayabiliyordum. Artık yapamıyorum.|
|$deadspeak_fish11|Lütfen beni ateşin yanına göm...|
|$deadspeak_fish12|Arkadaşımı gördün mü? Adı Anika.|
|$deadspeak_fish2|Mızrakla balık avlayacağına düşmanlarını avla...|
|$deadspeak_fish3|Üstüme bir ton ağırlık çöktü birden sanki...|
|$deadspeak_fish4|Çok soğuk... ve karanlık... Ama ben böylesini seviyorum.|
|$deadspeak_fish5|Sen bizi sazan yerine mi koyuyorsun?!|
|$deadspeak_fish6|Bu iğrenç koku da ne? Benden gelmediği kesin ama!|
|$deadspeak_fish7|Öldüm ama bir yere gittiğim yok. Beni bu lanetli araftan kurtarmayacak mısın?|
|$deadspeak_fish8|Okyanusun en derin yerlerinde nelerin dolaştığını hiç merak ediyor musun? Etme. Bilmesen daha iyi.|
|$deadspeak_fish9|Işığı görüyor musun? Seni çağırıyor...|
|$deadspeak_hive|Bizi alt ettiğini mi sanıyorsun? Tekrar düşün... Biz hâlâ bir kovanız ve sonsuza dek öyle kalacağız. Kovan alt edilemez.|
|$deadspeak_moder|Odin'in yarattığı, kibirden sakın! Kadim saltanatım sonuna yaklaşıyor ama ardımdan gelenler daha da haşmetli olacak...|
|$deadspeak_queen|Odin'e soydaşlarımın yerime başkasını getireceğini söyle... Kraliyet asla sona ermez.|
|$deadspeak_yagluth|Böbürlenmeye mi geldin, küçük şey? Kaç sefer öldürüldüğümü, parçalandığımı, sürgün edildiğimi, yakıldığımı ve gömüldüğümü biliyor musun? İlk değilsin, son da olmayacaksın.|
|$dream_ashlands01|Karanlık bir gökyüzü ve kaynayan bulutlar olan bir rüyanın içinde uyanıyorsun. Dağın üstüne çöken gece, seni sıkıca saran bir pelerin gibi. Fırtına göğü yarıyor ve yarıktan sıcak korlar yağarak rüyadaki bedenini bir anda yok ediyor...|
|$dream_ashlands02|Uykunda devasa bir canavar peşinde karanlığı sürükleyip etrafında çemberler çizerek dolanırken geniş ve kasvetli bir mağaranın ortasında çömeliyorsun. Canavarın çevresinde zümrüt alevler titreşiyor ve boğazının hırıltısı ağaran gün boyunca sürüyor...|
|$dream_ashlands03|Rüyanda bir ziyafette kızarmış jambon yediğini görüyorsun. Basit ve lezzetli bir yemek. Keşke tüm rüyalar bunun gibi olabilseydi.|
|$dream_ashlands04|Beyaz cübbeli dokuz kadın gece yarısı yalın ayak dans ediyor. Dokuz kurt yeşil ormanda beyaz bir geyik avlıyor. Dağ yamacında dokuz çan çalıyor ve uyanıyorsun.|
|$dream_bonemass01|Rüyanda yüzlerce hayalet, yatağının yanı başında toplanıyor, yaşamı ve canlılığın sıcaklığını arıyor. Sonra birdenbire derinlerdeki bir şey toprağı sarsıyor ve hayaletler davul derisindeki ekmek kırıntıları misali sıçrayarak kayboluyorlar.|
|$dream_eikthyr01|Rüyanda, gökyüzü solgun bir ateşle yanarken, yemyeşil bir çayırda koşturuyorsun. Geriden dörtnala koşan toynak sesleri duysan da, arkana döndüğünde hiçbir şey görmüyorsun.<br/><br/>Uyandığında kalbin deli gibi çarpıyor.|
|$dream_elder01|Rüyanda orman önünde ayaklanıveriyor, ağaçları pelerin misali çekiyor ve karanlığı, yıldızlara perde çekiyor, dağın zirvesindeki dev boynuzlar, Ay'ı çevreliyor...|
|$dream_mistlands01|Rüyanda, gülümseyen savaşçılar ve zarif kızların bulunduğu bir salonda yürüyorsun. Neşelerine katılıp devam ederken çıplak olduğunu ve taşıdığın küçük tunç kalkanın utancını saklamaya yeterli olmadığını fark ediyorsun.<br/><br/>Sabah olduğunu görünce rahatlıyorsun.|
|$dream_mistlands02|Siyah gözlü Loki rüyanda sana yaklaşıyor ve iki elin için de sana birer yüzük veriyor. Yüzükler parmaklarında kaldığı sürece bir daha asla küçük düşürücü bir söz duymayacağını söylüyor.<br/><br/>Parmakların kulaklarındayken uyanıyorsun.|
|$dream_mistlands03|Uzun bir kuleye zar zor sığmış dönemeçli bir merdiven çıkıyorsun ve zirveye ulaşıp bitmek bilmeyen bir ormana doğru bakıyorsun.<br/><br/>Rüzgâr ağaç tepelerinin üstünden yeşil dalgalar estiriyor ve yüzeyin altında karanlık suretler hareket ediyor...|
|$dream_moder01|Rüyanda dağların üstünde uçtuğunu ve bütün Valheim'ın altında uzandığını görüyorsun.<br/><br/>Soğuk havada yön değiştirip dalışa geçerken, büyük bir suret hızlıca arkanda yükseliyor ve güneşi kapatıyor. Karanlıkta seninle konuşuyor. Beni bul.|
|$dream_random01|Rüyanda yukarı doğru akan bir nehir ve toprağın içine doğru büyüyen filizler görüyorsun…|
|$dream_random02|Bir kez daha, elinde babanın baltasının ağırlığıyla, savaşçılarına önderlik ediyorsun.<br/><br/>Ağzında savaş narasıyla uyanıyorsun…|
|$dream_random03|Rüyanda, büyük bir salondaki ateşin yanı başında oturuyor ve çevrendeki tanıdık sesleri işitiyorsun.<br/><br/>Simaları sis misali bulanıklaşıyor ve isimleri bir türlü aklına gelmiyor ama anılarının samimiyeti hâlâ orada…|
|$dream_random04|Suları yaran geminin burnunda dikiliyor ve tuzlu su önünde dalgalanırken, tepende martıların neşeyle öttüğün duyuyorsun.<br/><br/>Bu rüya sana, memleketindeyken hayatta olmanın nasıl bir his olduğunu hatırlatıyor.|
|$dream_random05|Rüyanda gece boyunca uzanan dev bir ağacı görüyorsun. Dallarının bir kısmı ateşler ile çatırdarken diğer yarısı da yemyeşil yapraklarla bürülü bir halde.|
|$dream_random06|Silahların çarpışması arasında, karanlık ve hafifçe parlayan uyku ovasında önünde hırlayan bir surat yükseliyor. Kalkan tutan kolun zayıf düşmüş, mızrağın kırılmış. Yaklaşan soğuk bıçağı memnuniyetle karşılıyorsun.<br/><br/>Bir ölüm rüyasından, ölümün bizzat kendisine uyanıyorsun.|
|$dream_random07|Rüyanda bir çayırda sırtüstü yattığını, bulutlara doğru baktığını görüyorsun. Adın yok, zihnin düşüncelerden arınmış. Ancak elinde sıcak bir el var.<br/><br/>Rüyanda gülüyorsun. Ama uyandığında yüzün gözyaşlarıyla ıslanmış.|
|$dream_random08|Kara ahşaptan oyulan bir teknede, yırtık pırtık yelkenlerin altında kollarını göğsünde birleştirmiş bir şekilde uzanıyorsun. Karanlıktaki parmak izleri gibi bulanık yüzler, cam gibi siyah ve düz bir denizde süzülmen için seni iterken tanıdık şarkılar mırıldanıyorlar.|
|$dream_random09|Savaş alanında yatıyorsun, rüya gören gözlerin dumanla örtülü bir gökyüzüne doğru yukarı bakıyor. Savaşçılarının seslenmeleri giderek zayıflıyor ve gözlerin ikinci kez kapanıyor. Büyük pençeler altından kayıp geçiyor ve yükseldiğini hissediyorsun, beşiğinden kaldırılan bir bebek gibi vücudundan kaldırıldığını...|
|$dream_random10|Derin uyku kuyusuna düşüyorsun ve rüyanda yalnızca karanlığı görüyorsun.|
|$dream_random11|Rüyanda merhametli savaşçılar ve zarif genç kızlarla dolu şaşaalı bir salon görüyorsun. Havada bir şarkı sesi dolanıyor, döşemeler buharlı yemeklerin ağırlığı altında inliyor, bal şarabı su gibi akıyor.<br/><br/>Kahkahalar kulaklarında hâlâ yankılanırken yavaşça uyanıyorsun...|
|$dream_random12|Aralıklarla ve huzursuz rüyalarla uyuyorsun, kâbusun ağırlığı göğsüne baskı yapıyor. Sabahın gelmesiyle huzur buluyorsun.|
|$dream_random13|Uyku bir nehir ve rüyalarsa canlı balıklar. Sabah ağın boş bir şekilde uyanıyorsun.|
|$dream_random14|Rüyanda yoldaşlarınla ava çıktığını, yeşil tepelerden ve sisli vadilerden geçtiğini görüyorsun. Karşında avın sendeliyor ve sen de öne atlayıp dişlerini sıcak ete geçiriyorsun.<br/><br/>Uyandığında ağzında hâlâ metal tadı var.|
|$dream_random15|Rüyanda karlı bir ormanda ilerlerken bir ağaca yaslanmış, gözleri kapalı olan ama nefes aldığı için göğsü hareket eden çıplak bir çocukla karşılaşıyorsun. Yanında diz çökerken burada seni bekleyerek asırlardır uyuduğunu biliyorsun. Omzuna dokunduğunda ikiniz de uyanıyorsunuz.|
|$dream_random16|Rüyanda dışarıda kış yaşanırken yerin derinliklerinde uyuyan kudretli bir ayı görüyorsun. Ayı uykusunda dönüyor, eti ve kürkü kat kat. Kafası ve uzuvları yok. Ayı etinden de bir yığın, neyse ki sessiz.|
|$dream_random17|Ertesi günün işlerini planlarken uyuyakalıyorsun ve rüyalarında odun keserek, yiyecek arayarak ve hızlı geyikleri avlayarak işlerini tamamlıyorsun. Eve yorgun ama mutlu bir şekilde dönüyorsun ama uyanıp aslında o güne başlamadığını fark ediyorsun...|
|$dream_yagluth01|Altın perdelerle dolu bir saray odasında, yüzü örtülü bir kralın önünde diz çöküyorsun. Örtüsünü yavaşça çıkartırken, ağzında tek bir cümle süzülüyor, Uyku, yalnızca bir maskedir.<br/><br/>Çığlık çığlığa uyanıyorsun.|
|$dvergr_buff|Cüce gücü|
|$dvergr_buff_description|İçin, kuvvetini arttıran tuhaf bir Cüce gücüyle dolar.|
|$enemy_abomination|Ucube|
|$enemy_asksvin|Kül Domuzu|
|$enemy_asksvin_hatchling|Kül Domuzu Yavrusu|
|$enemy_babyseeker|Arayıcı yavrusu|
|$enemy_bat|Yarasa|
|$enemy_bjorn|Ayı|
|$enemy_blob|Blob|
|$enemy_blobelite|Oozer|
|$enemy_blobfrost|Buz Damlası|
|$enemy_bloblava|Lav Damlası|
|$enemy_blobtar|Öbek|
|$enemy_boar|Yaban Domuzu|
|$enemy_boarpiggy|Domuzcuk|
|$enemy_bonemass|Kemik yığını|
|$enemy_bonemawserpent|Kemikçene|
|$enemy_boss_bonemass_deathmessage|Yığın artık hareket etmiyor|
|$enemy_boss_bonemass_spawnmessage|Yığın hareket ediyor|
|$enemy_boss_dragon_deathmessage|Moder ağlıyor|
|$enemy_boss_dragon_spawnmessage|Moder kızdı|
|$enemy_boss_fader_alertmessage|Zümrüt Alev yanmaya başlıyor|
|$enemy_boss_fader_deathmessage|Zümrüt Alev söndü|
|$enemy_boss_goblinking_deathmessage|Yagluth'un ruhu alt edildi|
|$enemy_boss_goblinking_spawnmessage|Yagluth'un sapkın ruhu çağrıldı|
|$enemy_boss_queen_alertmessage|Kraliçe hepsini istiyor|
|$enemy_boss_queen_deathmessage|Kraliçe çok yaşa|
|$enemy_charred|Kor|
|$enemy_charred_archer|Kor Nişancı|
|$enemy_charred_grunt|Kor Piyade|
|$enemy_charred_mage|Kor Lanet Ustası|
|$enemy_charred_melee|Kor Savaşçı|
|$enemy_charred_melee_Dyrnwyn|<color=orange>Lord Reto</color>|
|$enemy_charred_melee_Fader|Çağrılmış Kor Savaşçı|
|$enemy_charred_twitcher|Kor Seğiren|
|$enemy_charred_twitcher_summoned|Çağrılmış Seğiren|
|$enemy_charredspawnercross|Şer Timsali|
|$enemy_charredtwitcherspawner|Azap Eseri|
|$enemy_chicken|Tavuk|
|$enemy_deathsquito|Ölüm Sivrisi|
|$enemy_deer|Geyik|
|$enemy_dragon|Moder|
|$enemy_drake|Ejder|
|$enemy_draugr|Draugr|
|$enemy_draugrelite|Elite Draugr|
|$enemy_draugrspawner|Ceset yığını|
|$enemy_dvergr|Cüce düzenbaz|
|$enemy_dvergr_mage|Cüce büyücü|
|$enemy_dvergrs|Cüceler|
|$enemy_eikthyr|Eikthyr|
|$enemy_fader|Sönümleyici|
|$enemy_fader_codename|Zümrüt Alev|
|$enemy_fallenvalkyrie|Düşmüş Valkür|
|$enemy_fenring|Fenring|
|$enemy_fenringcultist|Tarikatçı|
|$enemy_fenringcultist_hildir|<color=orange>Geirrhafa</color>|
|$enemy_gdking|Kadim Olan|
|$enemy_gemgoblin|Riktig Fuling|
|$enemy_ghost|Hayalet|
|$enemy_gjall|Gjall|
|$enemy_goblin|Fuling|
|$enemy_goblin_hildir|<color=orange>Zil</color>|
|$enemy_goblinbrute|Fuling Azmanı|
|$enemy_goblinbrute_hildir|<color=orange>Thungr</color>|
|$enemy_goblinbrute_hildircombined|<color=orange>Zil ve Thungr</color>|
|$enemy_goblinking|Yagluth|
|$enemy_goblinshaman|Fuling şamanı|
|$enemy_greydwarf|Bozcüce|
|$enemy_greydwarfbrute|Bozcüce azmanı|
|$enemy_greydwarfshaman|Bozcüce şamanı|
|$enemy_greydwarfspawner|Bozcüce yuvası|
|$enemy_greyling|Yavru Bozcüce|
|$enemy_hare|Yabantavşanı|
|$enemy_hen|Tavuk|
|$enemy_kvastur|Kvastur|
|$enemy_kvastur_lore|Bir cadının yoldaşı güçlü malzemelerden yapılmalıdır.|
|$enemy_leech|Sülük|
|$enemy_lox|Lox|
|$enemy_loxcalf|Lox kalfı|
|$enemy_mistile|Sis atışı|
|$enemy_morgen|Morgen|
|$enemy_neck|Neck|
|$enemy_seeker|Arayıcı|
|$enemy_seekerbrute|Arayıcı asker|
|$enemy_seekerqueen|Kraliçe|
|$enemy_serpent|Yılan|
|$enemy_skeleton|İskelet|
|$enemy_skeleton_summoned|İskelett|
|$enemy_skeletonfire|<color=orange>Brenna</color>|
|$enemy_skeletonpoison|Zehirli İskelet|
|$enemy_skeletonspawner|Uğursuz kemik yığını|
|$enemy_stonegolem|Taş Golemi|
|$enemy_summonedroot|Çağrılmış Kök|
|$enemy_summonedtroll|Çağrılmış Trol|
|$enemy_surtling|Alaz|
|$enemy_tick|Kene|
|$enemy_troll|Trol|
|$enemy_ulv|Ulv|
|$enemy_unbjorn|Menfur|
|$enemy_volture|Külbaba|
|$enemy_wolf|Kurt|
|$enemy_wolfcub|Yavru kurt|
|$enemy_wraith|Heyula|
|$error_alreadyconnected|Zaten bağlısınız|
|$error_banned|Yasaklandınız|
|$error_cantrestorebackup|Yedek geri yüklenemedi!|
|$error_cantrestoremeta|Meta dosyası geri yüklenemedi!|
|$error_disconnected|Bağlantı sonlandı|
|$error_failedconnect|Bağlantı başarısız oldu|
|$error_incompatibleversion|Uyumsuz sürüm|
|$error_kicked|Sunucudan atıldın|
|$error_needslocalupdatetojoin|Valheim'ın bu sunucuyla uyumlu olmayan eski bir sürümünü kullanıyorsun. Bu sunucuya katılmak için lütfen oyununu güncelle.|
|$error_needsserverupdatetojoin|Bu sunucu senin kullandığın sürümle uyumlu olmayan eski bir Valheim sürümünü kullanıyor.|
|$error_nosuitablebackupfound|Uyumlu bir yedek bulunamadı!|
|$error_password|Yanlış şifre|
|$error_platformexcluded|Sunucu sahibi, kendi sunucusunda başka platformlardan gelen kullanıcıların oynamasına izin vermiyor.|
|$error_serverfull|Sunucu dolu|
|$error_worldfileload|Dünya dosyası yüklenirken hata oluştu. Yedekleri kontrol et!|
|$event_bats_end|Kazan sakinleşiyor|
|$event_bats_start|Kazanı karıştırdın|
|$event_bonemassarmy_end|Koku kayboldu|
|$event_bonemassarmy_start|Bataklıktan pis bir koku geliyor|
|$event_boss01_end|Eikthyr yenildi|
|$event_boss01_start|Eikthyr uyandı|
|$event_boss02_end|Kadim Olan yenildi|
|$event_boss02_start|Kadim Olan uyandı|
|$event_boss03_end|Kemik Kütlesi yenildi|
|$event_boss03_start|Kemik kütlesi uyandı|
|$event_boss04_end|Moder yenildi|
|$event_boss04_start|Moder uyandı|
|$event_boss05_end|Kral Yagluth yenildi|
|$event_boss05_start|Kral Yagluth uyandı|
|$event_caves_end|Ulumalar kayboluyor|
|$event_caves_start|Ulumalar derinlerden yankılanıyor|
|$event_charredarmy_end|Ordu geri çekiliyor|
|$event_charredarmy_start|Hortlak ordusu ilerliyor|
|$event_charredspawnerarmy_start|Ölüler çağrıldı|
|$event_charrespawnerarmy_end|Ölüler bir kez daha hareketsiz yatıyor|
|$event_eikthyrarmy_end|Yaratıklar sakinleşmeye başladı|
|$event_eikthyrarmy_start|Eikthyr ormandaki yaratıkları çağırıyor|
|$event_foresttrolls_end|Sarsıntı diniyor|
|$event_foresttrolls_start|Yer sallanıyor|
|$event_gdkingarmy_end|Orman tekrar uykuya daldı|
|$event_gdkingarmy_start|Orman hareket ediyor...|
|$event_gemgoblin_end|Onların icabına baktın|
|$event_gemgoblin_start|Onların icabına bak!|
|$event_ghosts_end|They have been banished, for now...|
|$event_ghosts_start|You feel a chill down your spine...|
|$event_gjallarmy_end|Görüşürüz Gjall.|
|$event_gjallarmy_start|N'aber Gjall?!|
|$event_goblinarmy_end|Sürü geri çekiliyor|
|$event_goblinarmy_start|Sürü saldırıyor|
|$event_hildirboss1_end|Yandı|
|$event_hildirboss1_start|Tam arkanda|
|$event_hildirboss2_end|Rahatlama vakti|
|$event_hildirboss2_start|Ürperdin|
|$event_hildirboss3_end|Prensipleri yine bozdun...|
|$event_hildirboss3_start|Onlar kardeşti be kanka|
|$event_moderarmy_end|Rüzgar dindi|
|$event_moderarmy_start|Dağlardan soğuk bir rüzgâr esiyor|
|$event_seekerarmy_end|Arayış sona erdi.|
|$event_seekerarmy_start|Seni arayıp buldular.|
|$event_skeletons_end|İskeletler savaşmaktan bitap düştü|
|$event_skeletons_start|İskelet sürprizi|
|$event_surtlings_end|Sülfür kokusu geçiyor|
|$event_surtlings_start|Havada sülfür kokusu var...|
|$event_wolves_end|Av bitti|
|$event_wolves_start|Peşindeler...|
|$faderlocation_bellholder|Çan Tutucu|
|$feedback_bug|Hata|
|$feedback_feedback|Geri Bildirim|
|$feedback_idea|Öneri|
|$feedback_send|Gönder|
|$feedback_subject|Konu|
|$feedback_text|Bir şey yazın|
|$feedback_topic|Geri Bildirim|
|$fish1_description|Leziz bir beyaz etli balık.|
|$fish10_description|Bu balık, suyun neredeyse buz tutacak kadar soğuk olmasını seviyor.|
|$fish11_description|Kimileri bu balığın yumurtalarını doğrudan sıcak lavlara bıraktığını söylüyor!|
|$fish12_description|Doğru pişirildiğinde lezzetli olur ama tadı biraz yakar.|
|$fish2_description|Çok fazla baharat gerektiren bir tatlı su balığı.|
|$fish3_description|Denizin tavuğu...|
|$fish4_description|Bütün hayatını karanlıkta geçirdiği için bir göze ihtiyacı yok.|
|$fish5_description|Bu balık, ufak akarsularda başa bela olur.|
|$fish6_description|Bu balık bozulduğunda geldiği bataklıktan daha beter kokar.|
|$fish7_description|Yanında karbonhidrat olursa tadından yenmez!|
|$fish8_description|Öyle şeyler görmüş ki... Çok rahatsız edici şeyler...|
|$fish9_description|Sarkan ışığı o tatlı küçük suratını görmeyi kolaylaştırıyor!|
|$guardianstone_bonemass_desc|Ey gezgin, bastığın toprağa bak<br/>Bak ki mezarımıza bastığını gör<br/>Burada etten sıyrılmış bin kemik yatar<br/>Sonun pek bir yakındır|
|$guardianstone_eikthyr_desc|Demirden boynuzları<br/>Yerle bir eder dağları taşları<br/>Toynakları andırır yıldırımları<br/>Gürdür sesi, andırır fırtınaları|
|$guardianstone_fader_desc|Asil ve gururlu bir baba<br/>Alevli göklere yükseldi<br/>Sonra delilik maskesini indirip<br/>Gönlünden geçeni değiştirdi|
|$guardianstone_hook_activate|Gücü etkinleştir|
|$guardianstone_hook_alreadyactive|Mevcut güç|
|$guardianstone_hook_deactivate|Gücü kapat|
|$guardianstone_hook_name|Av ganimeti kancası|
|$guardianstone_hook_power_activate|Etkinleştirildi|
|$guardianstone_hook_power_deactivate|Kapatıldı|
|$guardianstone_moder_desc|Ay ve güneşe uzanan kara kanatları<br/>Gelir annemiz dağdan aşağı<br/>Yaşlar gözünden yağmur misali yağar<br/>Sesi bizi tekrar eve çağırır|
|$guardianstone_name|Kurban taşı|
|$guardianstone_theelder_desc|Ormanın İlki, Korunun-Kralı<br/>Dibine sinenleri koruyup kollardı<br/>Evvel şehirlerin dikildiği yerde büyür kökleri<br/>Kanları onun şarabı, etleri onun ziyafeti|
|$guardianstone_thequeen_desc|Zırhla doğdu,<br/>Çoğunun annesi oldu.<br/>Taçsız kraliçe,<br/>Dipteki hükümdar.|
|$guardianstone_trophyplaced|Yitik kurban edildi|
|$guardianstone_yagluth_desc|Çağlar evvel tacı vardı<br/>Kan kırmızı semada takardı<br/>Artık yok eski halinden eser<br/>Tadamaz artık ölümü, ruhu ser|
|$heightmap_issue_message|Dünya haritasının senkronize olmamasıyla ilgili kritik bir sorunu düzeltebilmek için, bazı oyuncuların dünyalarının bazı küçük bölümlerinde değişiklikler gerçekleştirilecektir. Bu alanlara yakın inşa edilmiş yapılarda hatalar oluşabilir. Bu sorunların yaşanmamasını sağlamak için elimizden gelenin en iyisini yapmaya çalışsak da, bazen bu tarz hatalar kaçınılmaz olabilmekte. Yaşanan rahatsızlıktan dolayı gerçekten çok üzgünüz.|
|$hud_addpin|Yer işareti ekle|
|$hud_altplace|Birleştirme / Diğer seçenekler|
|$hud_altplacement|Alternatif yerleştirme modu:|
|$hud_autopickup|Oto-Topla|
|$hud_build|İnşa Et|
|$hud_building|Yapı|
|$hud_building2|Ağır Yapılar|
|$hud_buildmenu|İnşaat menüsü|
|$hud_changeorientation|Yön değiştir|
|$hud_copypiece|Kopyala|
|$hud_crafting|Atölye|
|$hud_crossoffpin|İşaretin üzerini çarpıla|
|$hud_cyclesnap|Birleştirmeyi değiştir|
|$hud_eitrrequired|Eitr gerekiyor|
|$hud_emotes_blowkiss|Öpücük gönder|
|$hud_emotes_bow|Eğil|
|$hud_emotes_challenge|Meydan oku|
|$hud_emotes_cheer|Tezahürat yap|
|$hud_emotes_comehere|Buraya gel|
|$hud_emotes_cower|Çök|
|$hud_emotes_cry|Ağla|
|$hud_emotes_dance|Dans et|
|$hud_emotes_despair|Ümitsizliğe kapıl|
|$hud_emotes_flex|Hava at|
|$hud_emotes_headbang|Kafanı salla|
|$hud_emotes_kneel|Diz çök|
|$hud_emotes_laugh|Gül|
|$hud_emotes_loveyou|Seviyorum seni|
|$hud_emotes_nonono|Hayır hayır hayır|
|$hud_emotes_point|İşaret et|
|$hud_emotes_relax|Rahatına bak|
|$hud_emotes_rest|Dinlen|
|$hud_emotes_roar|Kükre|
|$hud_emotes_shrug|Omuz silk|
|$hud_emotes_sit|Oturdum|
|$hud_emotes_thumbsup|Başparmak yukarıda|
|$hud_emotes_toast|Kadeh kaldır|
|$hud_emotes_vibe|Keyfine bak|
|$hud_emotes_wave|El salla|
|$hud_equipping|Kuşanıyorsun:|
|$hud_feasts|Ziyafetler|
|$hud_food|Yiyecek|
|$hud_furniture|Mobilya|
|$hud_hidden_messagehud_notification|HUD disabled. To enable, press Ctrl + F3|
|$hud_hidden_messagehud_notification_gamepad|HUD disabled. To enable, press |
|$hud_hidemap|Haritayı gizle|
|$hud_hidepin|Yer işaretlerini aç/kapa|
|$hud_itemstucktar|Katranda kalmış|
|$hud_loadingrequired|Silah yüklenmedi|
|$hud_mapday|Gün|
|$hud_maxsummonsreached|Ölü Kaldıran'ın daha fazla birimi kontrol edecek kadar güçlü değil.|
|$hud_maxsummonsreached_greenroots|Your Staff of the Wild is not strong enough to control more summons.|
|$hud_maxsummonsreached_trollstav|Your Trollstav is not strong enough to control more summons.|
|$hud_meads|Şarap|
|$hud_misc|Diğer|
|$hud_namepin|Yer işaretini isimlendir|
|$hud_nothingtobuild|İnşa edilecek bir şey yok|
|$hud_off|Kapalı|
|$hud_on|Açık|
|$hud_pet|Sev|
|$hud_pin_hildir1|Yanan mezar|
|$hud_pin_hildir2|Uluyan mağara|
|$hud_pin_hildir3|Mühürlü kule|
|$hud_ping|İşaret|
|$hud_place|Yerleştir|
|$hud_powernotready|Güç hazır değil|
|$hud_publicpos|Diğer oyunculara tarafından görülebilir|
|$hud_ready|Hazır|
|$hud_reloading|Yükleniyor|
|$hud_remove|Kaldır|
|$hud_removepin|Yer işaretini kaldır|
|$hud_rename|Yeniden isimlendir|
|$hud_repair|Onar|
|$hud_require|Gereken|
|$hud_rotate|Çevir|
|$hud_saddle_already|üstünde zaten eyer var|
|$hud_saddle_ready|yolculuğa hazır|
|$hud_saddle_remove|Eyeri kaldır|
|$hud_sharedmap|Haritacılık Masası|
|$hud_showmap|Haritayı göster|
|$hud_snappoint_attach|Bağlantı|
|$hud_snappoint_bottom|Alt|
|$hud_snappoint_center|Merkez|
|$hud_snappoint_corner|Köşe|
|$hud_snappoint_edge|Kenar|
|$hud_snappoint_end|Son|
|$hud_snappoint_inner|İç|
|$hud_snappoint_mid|Orta|
|$hud_snappoint_top|Üst|
|$hud_switchcategory|Kategori değiştir|
|$hud_switchitem|Eşya değiştir|
|$hud_tame|Evcilleştir|
|$hud_tamecheck|Kontrol Et|
|$hud_tamedone|evcilleşti|
|$hud_tamefollow|seni takip ediyor|
|$hud_tamefrightened|Korkmuş|
|$hud_tamehappy|Mutlu|
|$hud_tamehungry|Karnı Aç|
|$hud_tameinprogress|Alışıyor|
|$hud_tamelove|seni seviyor|
|$hud_tamelove2|seni çok seviyor|
|$hud_tamelove3|sana sokuluyor|
|$hud_tameness|Evcillik|
|$hud_tamestay|duruyor|
|$hud_unequipping|Çıkartıyorsun:|
|$hud_wild|Vahşi|
|$hud_zoom|Yakınlaştır|
|$hud_zoom_in_out|Uzaklaştır/yakınlaştır|
|$info_label|Info|
|$interface_leftarrow|<|
|$interface_rightarrow|>|
|$invalid_keybind_header|Invalid Input|
|$invalid_keybind_text|This action can't be bound to selected key|
|$inventory_activeeffects|Aktif etkiler|
|$inventory_blunt|Künt|
|$inventory_chop|Doğrama|
|$inventory_craftbutton|Üret|
|$inventory_crafting|Atölye|
|$inventory_craftingprog|Üretiliyor|
|$inventory_damage|Hasar|
|$inventory_dmgmod|Hasar çarpanı|
|$inventory_drop|Bırak|
|$inventory_fire|Ateş|
|$inventory_frost|Buz|
|$inventory_full|Envanter dolu|
|$inventory_immune|Bağışık|
|$inventory_lightning|Yıldırım|
|$inventory_logs|Mesaj kayıtları|
|$inventory_logs_new|Mesaj kayıtlarında kilidi açılan yeni eşyalar var.|
|$inventory_maxquality|En yüksek kalite|
|$inventory_move|Taşı|
|$inventory_new|Yeni eşya|
|$inventory_onlyoneingredient|Bu tarif gösterilen malzemelerden yalnızca bir tanesini gerektiriyor.|
|$inventory_pickaxe|Kazma|
|$inventory_pickup|Al|
|$inventory_pierce|Delme|
|$inventory_poison|Zehir|
|$inventory_pvp|Diğer oyunculardan hasar al (PVP)|
|$inventory_quality|Kalite|
|$inventory_recipes|Tarifler|
|$inventory_repairbutton|Bir eşyayı onar|
|$inventory_resistant|Dirençli|
|$inventory_selectedgp|Seçili Yitik güç|
|$inventory_siege|Kuşatma|
|$inventory_skills|Yetenekler|
|$inventory_slash|Kesme|
|$inventory_slightlyresistant|Biraz dayanıklı|
|$inventory_slightlyweak|Biraz zayıf|
|$inventory_spirit|Ruh|
|$inventory_split|Böl|
|$inventory_splitstack|Yığını böl|
|$inventory_stackall|Yığınları yerleştir|
|$inventory_stationlevel|İstasyon seviyesi (Yapı geliştirmeleriyle artar)|
|$inventory_stationlevelreq|Gereken istasyon seviyesi|
|$inventory_style|Tarz|
|$inventory_tabs|Pencere değiştir|
|$inventory_takeall|Hepsini al|
|$inventory_texts|Valheim Külliyatı|
|$inventory_totalskill|Toplam yetenek|
|$inventory_transfer|Aktar|
|$inventory_trophies|Av Ganimetleri|
|$inventory_upgrade|$1 kalitesini $2 kalitesine geliştir|
|$inventory_upgradebutton|Geliştir|
|$inventory_use|Kullan/Kuşan|
|$inventory_veryresistant|Çok dirençli|
|$inventory_veryweak|Çok zayıf|
|$inventory_weak|Zayıf|
|$item_amber|Kehribar|
|$item_amber_description|<color=yellow>Değerli Eşya</color>|
|$item_amberpearl|Kehribar inci|
|$item_amberpearl_description|<color=yellow>Değerli Eşya</color>|
|$item_ancientroot|Kadim kök|
|$item_ancientroot_empty|Kadim kök tamamen kurumuşa benziyor|
|$item_ancientroot_full|Kadim kök enerjiyle titriyor|
|$item_ancientroot_half|Kadim kökün parıltısı sönüyor|
|$item_ancientseed|Kadim tohum|
|$item_ancientseed_description|Kulağına tuttuğunda içinden gelen minik fısıltılar duyabilirsin...|
|$item_armor|Zırh|
|$item_arrow_bronze|Tunç temrenli ok|
|$item_arrow_bronze_description|Çakmak taşından daha keskin olan tunç, ölümün göz alıcı bir ulağıdır.|
|$item_arrow_carapace|Kabuk ok|
|$item_arrow_carapace_description|Ağır ve sivri olan bu ok can yakar.|
|$item_arrow_charred|Kor Ok|
|$item_arrow_charred_description|Bu oka kor bir uyluk kemiği oyularak şekil verilmiş ve herhangi bir metal kadar sert.|
|$item_arrow_fire|Ateşli ok|
|$item_arrow_fire_description|Bu ok isabet ettiği her şeyi yakar.|
|$item_arrow_flint|Çakmak taşı temrenli ok|
|$item_arrow_flint_description|Temreni çakmak taşından, postları delen bir ok.|
|$item_arrow_frost|Buzlu ok|
|$item_arrow_frost_description|Delici bir buz parçası.|
|$item_arrow_iron|Demir temrenli ok|
|$item_arrow_iron_description|Temreni demirden olan bu okun yelekleri kara tüylerden yapılmış.|
|$item_arrow_needle|İğne ok|
|$item_arrow_needle_description|Son dikiş.|
|$item_arrow_obsidian|Obsidiyen ok|
|$item_arrow_obsidian_description|Karanlıktan yontulmuş bir temren.|
|$item_arrow_poison|Zehirli ok|
|$item_arrow_poison_description|Uzaktan gelen acı bir ısırık.|
|$item_arrow_silver|Gümüş ok|
|$item_arrow_silver_description|Huzursuz ruhları sakinleştirecek bir iğne.|
|$item_arrow_wood|Ahşap ok|
|$item_arrow_wood_description|Sivriltilmiş daldan yapılmış bir ok.|
|$item_askbladder|Kül Domuzu Mesanesi|
|$item_askbladder_description|Hâlâ asidik bir kokusu var. Belirgin bir şekilde.|
|$item_askhide|Kül Domuzu Derisi|
|$item_askhide_description|Bu dayanıklı deri kalın ve dokununca hâlâ sıcak.|
|$item_asksvin_egg|Kül Domuzu Yumurtası|
|$item_asksvin_egg_description|Kaya kadar sert ancak içinde bir şeyin varlığını hissedebiliyorsun. Bu şey sıcak tutulmalı.|
|$item_asksvin_meat|Kül Domuzu Kuyruğu|
|$item_asksvin_meat_cooked|Pişmiş Kül Domuzu Kuyruğu|
|$item_asksvin_meat_cooked_description|Bu etin yoğun ve olgun bir aroması var ancak doğru bir şekilde kızartıldığında oldukça lezzetli.|
|$item_asksvin_meat_description|Çiğken bile hâlâ biraz tütsülü kokuyor.|
|$item_asksvincarrionneck|Kül Domuzu Boynu|
|$item_asksvincarrionneck_description|Yaşamının son evresinde olan bir boyun.|
|$item_asksvincarrionpelvic|Kül Domuzu Leğen Kemiği|
|$item_asksvincarrionpelvic_description|Dört ayaklı bir canlının leğen kemiği.|
|$item_asksvincarrionribcage|Kül Domuzu Göğüs Kafesi|
|$item_asksvincarrionribcage_description|Bu kaburgaların üzerindeki et çoktan temizlenmiş.|
|$item_asksvincarrionskull|Kül Domuzu Kafatası|
|$item_asksvincarrionskull_description|Şaşırtıcı derecede büyük bir beyin için yeri olan kalın bir kafatası.|
|$item_atgeir_blackmetal|Karademir atgeir|
|$item_atgeir_blackmetal_description|Kırılması neredeyse imkânsız olan karademirden yapılmış, şeytani görünümlü bir yontma baltası.|
|$item_atgeir_bronze|Tunç atgeir|
|$item_atgeir_bronze_description|Gerçek savaşçılara yaraşır bir alet.|
|$item_atgeir_himminafl|Himmin afl|
|$item_atgeir_himminafl_description|Bir çekiç olmayabilir ama bizzat Thor da bu silahı beğenirdi.|
|$item_atgeir_iron|Demir atgeir|
|$item_atgeir_iron_description|Kan içici, kafatası kırıcı, ölümün getiricisi.|
|$item_atgeir_wood|Tahta Atgeir|
|$item_atgeir_wood_description|Çok acıtmayabilir ama menzili geniş!|
|$item_axe_berzerkr|Vahşi Savaşçı Baltaları|
|$item_axe_berzerkr_blood|Kanatan Vahşi Savaşçı Baltaları|
|$item_axe_berzerkr_blood_description|Ölüme ne kadar yakınsan o kadar ağır darbeler indireceğin kesindir.|
|$item_axe_berzerkr_description|Öfkene teslim ol ve katliamla yüzleş.|
|$item_axe_berzerkr_lightning|Fırtınalı Vahşi Savaşçı Baltaları|
|$item_axe_berzerkr_lightning_description|Bu baltaları kullandığında etrafını öyle bir vahşet sarar ki bizzat Thor seninle gurur duyar.|
|$item_axe_berzerkr_nature|Kadim Vahşi Savaşçı Baltaları|
|$item_axe_berzerkr_nature_description|En ilkel içgüdülerine teslim oluyorsun ve çevrendeki doğa sana yardımcı olmak için elini uzatıyor.|
|$item_axe_blackmetal|Karademir balta|
|$item_axe_blackmetal_description|Zümrüt parıltısına sahip karademirden dövülmüş, kusursuz dengeye sahip bir balta.|
|$item_axe_bronze|Tunç balta|
|$item_axe_bronze_description|Ağzı bir gülümsemeyi andın biçimde kıvrımlı, cilalı tunç bir balta.|
|$item_axe_early|Antik Baltalar|
|$item_axe_early_description|Çok öncelere ait, dünyanın hâlâ toy ve eksik olduğu zamanlardan kalma kudretli silahlar...|
|$item_axe_flint|Çakmak taşı balta|
|$item_axe_flint_description|Taştan daha keskindir.|
|$item_axe_iron|Demir balta|
|$item_axe_iron_description|Keskin ve sağlamdır, oduncunun dostudur.|
|$item_axe_jotunbane|Jötunn kesen|
|$item_axe_jotunbane_description|Kadim devler bile bu silahın zehirli ısırığına karşı koyamazdı.|
|$item_axe_rusty|Paslı demir balta KULLANILMAMIŞ|
|$item_axe_rusty_description|Kullanılmamış|
|$item_axe_stone|Taş balta|
|$item_axe_stone_description|Ağaç kesmek için ilkel bir balta.|
|$item_axe_wood|Tahta Balta|
|$item_axe_wood_description|Bu balta odun kesebilse çok garip olmaz mıydı?|
|$item_axehead1|Tuhaf Balta Başı|
|$item_axehead1_description|Metal kısmı, güneş ışığının altında garip garip parıldıyor. Nedense içten içe bu baltanın tek başına bir bütün hâline gelemeyeceğini biliyorsun...|
|$item_axehead2|Gizemli Balta Başı|
|$item_axehead2_description|Bu silah ne tür savaşlara tanıklık etmiştir acaba? Geçmişini göremiyorsun ama geleceğini şekillendirmek senin elinde...|
|$item_backstab|Arkadan bıçaklama|
|$item_barberkit|Berber seti|
|$item_barberkit_description|En iyi berberlere uygun bir set.|
|$item_barley|Arpa|
|$item_barley_description|Arpa demeti.|
|$item_barleyflour|Arpa unu|
|$item_barleyflour_description|Ekmek pişirmek için mükemmel.|
|$item_barleywine|Ateş dirençli arpa şarabı|
|$item_barleywine_description|Ateşe karşı güçlendirir.|
|$item_barleywinebase|Arpa şarabı özü: Ateş direnci|
|$item_barrelrings|Fıçı Çemberleri|
|$item_barrelrings_description|Mükemmel yuvarlaklığa sahip bu metal çemberler fıçıyı bir arada tutmaya yarar. Odun eklemen yeter!|
|$item_battleaxe|Savaş baltası|
|$item_battleaxe_blackmetal|Karademir Savaş Baltası|
|$item_battleaxe_blackmetal_description|Yeşil, sen onları ikiye bölmeden önce düşmanlarının gördüğü son şey olacak.|
|$item_battleaxe_crystal|Kristal savaş baltası|
|$item_battleaxe_crystal_description|Silahın da düşmanın da içini gösterir.|
|$item_battleaxe_description|Kafa yaran bu balta, her savaşçının neşesi.|
|$item_battleaxe_skullsplittur|Kafakırdıran|
|$item_battleaxe_skullsplittur_description|En yoğun pusun içinde bile kırılacak kafataslarını bulur.|
|$item_battleaxe_wood|Tahta Savaş Baltası|
|$item_battleaxe_wood_description|Gerçeği kadar olmasa da göz korkutucu.|
|$item_beechseeds|Kayın tohumları|
|$item_beechseeds_description|Kayın ağacı istiyorsan toprağa ek.|
|$item_bell|Çan|
|$item_bell_description|Çan kimin için çalıyor?|
|$item_bellfragment|Çan Parçası|
|$item_bellfragment_description|Bu kadim parça, kırık bir çanın bir parçası gibi görünüyor...|
|$item_beltstrength|Megingjord|
|$item_beltstrength_description|Giyen kişiye insanüstü kuvvet bahşeder.|
|$item_bilebag|Safra torbası|
|$item_bilebag_description|Bir gjall'ın cesedinden alınan yakıcı safra.|
|$item_bilebomb|Safra bombası|
|$item_bilebomb_description|Dikkatli tut.|
|$item_birchseeds|Huş tohumları|
|$item_birchseeds_description|Huş ağacı istiyorsan toprağa ek.|
|$item_bjorn_meat|Ayı Eti|
|$item_bjorn_meat_cooked|Pişmiş Ayı Eti|
|$item_bjorn_meat_cooked_description|Tadı zafer gibi.|
|$item_bjorn_meat_description|Bu yağlı et, vücudu kış uykusu boyu idare etmek içindi.|
|$item_bjornhide|Ayı Derisi|
|$item_bjornhide_description|Kalın ve kürklü bir deri, kaba fakat inanılmaz derecede yumuşak.|
|$item_bjornpaw|Ayı Pençesi|
|$item_bjornpaw_description|Yemiş toplamaya ya da uzuv parçalamaya, uygun pençeler.|
|$item_blackcore|Kara çekirdek|
|$item_blackcore_description|Tamamı kadim güçle dolu.|
|$item_blackmarble|Siyah mermer|
|$item_blackmarble_description|Değişken renklerle birleşen masif taştan bir blok.|
|$item_blackmetal|Karademir|
|$item_blackmetal_description|Ağır bir kara metal külçesi.|
|$item_blackmetalscrap|Hurda karademir|
|$item_blackmetalscrap_description|Bükülmüş bir karademir öbeği.|
|$item_blacksoup|Kara çorba|
|$item_blacksoup_description|Tatlı ve ekşinin mükemmel dengesi.|
|$item_blackwood|Kül Odunu|
|$item_blackwood_description|Ateş ve külle sağlamlaştırılmış odun.|
|$item_blobvial|Tıpalı Küçük Şişe|
|$item_blobvial_description|Uçucu bir şeyler tutacak kadar kalın, fakat parçalanabilecek kadar kırılgan.|
|$item_blockarmor|Zırh bloklama|
|$item_blockforce|Darbe bloklama|
|$item_blockpower|Bloklama gücü|
|$item_bloodbag|Kan torbası|
|$item_bloodbag_description|Sülükten çıkanlar. Öğ!|
|$item_bloodclot|Kan pıhtısı|
|$item_bloodclot_description|Taşırken delmemeye dikkat et...|
|$item_bloodpudding|Domuz sosisi|
|$item_bloodpudding_description|Lezzet fışkırıyor.|
|$item_blueberries|Yaban mersini|
|$item_blueberries_description|Küçük ama lezzetli.|
|$item_boar_meat|Yaban domuzu eti|
|$item_boar_meat_cooked|Pişmiş yaban domuzu eti|
|$item_boar_meat_cooked_description|Dünyevi bir lezzet.|
|$item_boar_meat_description||
|$item_boarjerky|Kurutulmuş yaban domuzu eti|
|$item_boarjerky_description|Tuzlu ve yağsız.|
|$item_bolt_blackmetal|Karademir arbalet oku|
|$item_bolt_blackmetal_description|Göz alıcı, karademirden bir arbalet oku.|
|$item_bolt_bone|Kemik arbalet oku|
|$item_bolt_bone_description|Sararmış kemikten ilkel bir arbalet oku.|
|$item_bolt_carapace|Kabuk arbalet oku|
|$item_bolt_carapace_description|Ağır ve sağlam bir arbalet oku.|
|$item_bolt_charred|Kor Arbalet Oku|
|$item_bolt_charred_description|Ön koldan alınan sağlam bir kemiğe şekil verilerek oluşturulmuş ölümcül arbalet oku.|
|$item_bolt_iron|Demir arbalet oku|
|$item_bolt_iron_description|Sağlam, demirden bir arbalet oku.|
|$item_bombblob_frost|Damla Bombası: Buz|
|$item_bombblob_frost_description|Uzun süre tutarsan elin orada donup kalabilir.|
|$item_bombblob_lava|Damla Bombası: Lav|
|$item_bombblob_lava_description|Küçük şişe alevli bir yıkım vadedercesine elini ısıtıyor.|
|$item_bombblob_poison|Damla Bombası: Zehir|
|$item_bombblob_poison_description|Bu küçük şişe kırılırsa dikkatli olmakta fayda var.|
|$item_bombblob_poisonelite|Damla Bombası: Seçkin Zehir|
|$item_bombblob_poisonelite_description|Buharını solumanız tavsiye edilmez.|
|$item_bombblob_tar|Damla Bombası: Katran|
|$item_bombblob_tar_description|İçindeki yapışkan kımıldanıyor, âdeta seni tehdit ediyor gibi.|
|$item_bonefragments|Kemik parçacıkları|
|$item_bonefragments_description|Parçalanmış kemiklerden oluşan bir öbek.|
|$item_bonemawmeat|Kemikçene Eti|
|$item_bonemawmeat_cooked|Pişmiş Kemikçene Eti|
|$item_bonemawmeat_cooked_description|Kaynayan deniz bu ete hiçbir şey yapmadı ancak onu ateş üstünde kızartmak ona nefis bir gevreklik kattı.|
|$item_bonemawmeat_description|Lezzetli bir beyaz balık eti. Kemiklerin için çok faydalıdır!|
|$item_bonemawscale|Kemikçene Pulu|
|$item_bonemawscale_description|Tıpkı kemik gibi görünüyor. En iyisi bu yaratığın anatomisini pek sorgulamayalım...|
|$item_bonemawtooth|Kemikçene Dişi|
|$item_bonemawtooth_description|Bu birçok cesur denizcinin ölümüne sebep oldu.|
|$item_bow|Kaba yay|
|$item_bow_ashlands|Kül Dişi|
|$item_bow_ashlands_description|Küllerinden yeniden doğan bu yay sarsılmaz bir kuvvet barındırıyor.|
|$item_bow_ashlandsblood|Kan Dişi|
|$item_bow_ashlandsblood_description|Bu yaydan salınan oklar emsalsiz bir hiddetle deriyi deler.|
|$item_bow_ashlandsroot|Kök Dişi|
|$item_bow_ashlandsroot_description|Bükülen dalın güneşe yöneldiği gibi bu yay da katliama yönelmesi için yapılmış.|
|$item_bow_ashlandsstorm|Fırtına Dişi|
|$item_bow_ashlandsstorm_description|Okların bir yıldırım kadar hızlı uçsun.|
|$item_bow_description|Kaba ama kullanışlı bir yay.|
|$item_bow_draugrfang|Draugr dişi|
|$item_bow_draugrfang_description|Parlak kirişle bağlanmış kara ahşaptan korkutucu bir yay.|
|$item_bow_finewood|Kaliteli ahşap yay|
|$item_bow_finewood_description|Sağlam ve esnek ahşaptan yapılmış basit bir yay.|
|$item_bow_huntsman|Avcı yayı|
|$item_bow_huntsman_description|Titizlikle yontulup kurulmuş bu yay, her avcıyı sevindirecektir.|
|$item_bow_snipesnap|Omurgaçatlatan|
|$item_bow_snipesnap_description|Bu yayı kullanmak yorucu iştir ama buna değer.|
|$item_bread|Ekmek|
|$item_bread_description|Leziz bir somun ekmek.|
|$item_breaddough|Ekmek hamuru|
|$item_breaddough_description|Fırına girmeye hazır.|
|$item_bronze|Tunç|
|$item_bronze_description|Güçlü bir bakır ve kalay alaşımı.|
|$item_bronzenails|Tunç çiviler|
|$item_bronzenails_description|Gemi ve mobilya inşaatında kullanılır.|
|$item_bronzescrap|Hurda tunç|
|$item_bronzescrap_description|Eskiyip oksitlenmiş olsa da eritilerek tekrar kullanılabilir.|
|$item_bug_meat|Arayıcı eti|
|$item_bug_meat_cooked|Pişmiş arayıcı eti|
|$item_bug_meat_cooked_description|Dolgun beyaz et, gerçek ziyafet masalarına layık.|
|$item_bug_meat_description|Kabuklarını kırıp açtığında ortaya çıkan etleri yumuşak ve dolgundur.|
|$item_candlewick|Fitil|
|$item_candlewick_description|Uzun süreli hoş bir ışık kaynağı elde etmek için bu fitilleri yanıcı bir şeyin içine koy.|
|$item_cape_ash|Kül Pelerin|
|$item_cape_ash_description|Bizzat Nornlar tarafından örülen bir kader gibi bu pelerine girişik bir desen oluşturmak üzere ince metal iplikler nakşedildi.|
|$item_cape_asksvin|Kül Domuzu Pelerini|
|$item_cape_asksvin_description|Bu kalın pelerin, bir geminin yelkenine benzer şekilde rüzgârı yakalar.|
|$item_cape_deerhide|Geyik derisi pelerin|
|$item_cape_deerhide_description|Kırsal moda.|
|$item_cape_feather|Tüy pelerin|
|$item_cape_feather_description|Bu pelerini takmak seni neredeyse uçabilecekmişsin gibi hafif hissettiriyor!|
|$item_cape_linen|Keten pelerin|
|$item_cape_linen_description|Sıradan bir gezgin pelerini.|
|$item_cape_lox|Lox pelerini|
|$item_cape_lox_description|En büyük hayvanlardan birine ait kalın ve sıcak bir post.|
|$item_cape_odin|Odin'in pelerini|
|$item_cape_odin_description|Odin'in gözde savaşçıları en kaliteli kumaşları hak eder.|
|$item_cape_trollhide|Trol derisi pelerin|
|$item_cape_trollhide_description|Trol derisi sert ve aynı zamanda esnektir.|
|$item_cape_wolf|Kurt kürkü pelerin|
|$item_cape_wolf_description|Kurtlar hayatta kalma konusunda doğuştan uzmandırlar ama bu arkadaş biraz talihsizmiş. Postu artık kar soğuğunda seni ısıtacak.|
|$item_carapace|Kabuk|
|$item_carapace_description|Kitin zırh plakası.|
|$item_carrot|Havuç|
|$item_carrot_description|Turuncu lezzet.|
|$item_carrotseeds|Havuç tohumları|
|$item_carrotseeds_description|Havuç seviyorsan ekebilirsin...|
|$item_carrotsoup|Havuç çorbası|
|$item_carrotsoup_description|Çoğunlukla havuçtan yapılan sıcak ve leziz bir çorba.|
|$item_catapult_ammo|Patlayıcı Yük|
|$item_catapult_ammo_description|En iyi sonuç için mancınıkla kullan. Dikkatli taşı.|
|$item_catapult_training_ammo|Gritaş Yükü|
|$item_catapult_training_ammo_description|En iyi sonucu elde etmek için mancınıkla kullan. Yolunda kırılgan şeylerin olmadığından emin ol.|
|$item_celestialfeather|Göksel Tüy|
|$item_celestialfeather_description|Düşmüş valkürün eski benliğinden kalan tek şey.|
|$item_ceramicplate|Porselen Plaka|
|$item_ceramicplate_description|Bu ne kadar ısınırsa ısınsın diğer tarafı tuhaf bir şekilde serin kalıyor.|
|$item_chain|Zincir|
|$item_chain_description|Demirden bir zincir halkası.|
|$item_chancetoapplyse|Şu etkiyi uygulama ihtimali:|
|$item_charcoalresin|Kömür Reçinesi|
|$item_charcoalresin_description|Uzun zaman önce ateşe verilen bir ağacın reçinesi. Hâlâ daha fazla yanmaya hazır.|
|$item_charredbone|Kor Kemik|
|$item_charredbone_description|Ardından yanmış etin ayırt edici kokusu takip ediyor.|
|$item_charredcogwheel|Kor Çark|
|$item_charredcogwheel_description|Bu, birkaç zekice makine için kullanılabilir...|
|$item_charredskull|Kor Kafatası|
|$item_charredskull_description|Uzun zaman önce ölmüş bir savaşçının kararmış kafatası. Düzgünce gömülmek bile artık ona huzur sağlamayacaktır.|
|$item_chest_berserker|Ayı Desenleri|
|$item_chest_berserker_description|Bu karmaşık desenler uzak diyarlardaki savaşçıları koruyabiliyorsa seni de elbette koruyabilirler.|
|$item_chest_berserker_undead|Menfurkemiği Kafes|
|$item_chest_berserker_undead_description|Azap çeken bir ayının parçalanmış kemiklerinden üretildi.|
|$item_chest_bronze|Tunç göğüs zırhı|
|$item_chest_bronze_description|Tunçtan dövülmüş göğüs zırhı.|
|$item_chest_carapace|Kabuk göğüs zırhı|
|$item_chest_carapace_description|Dev bir böceğin parıldayan kabuğundan üretilen bir göğüs zırhı.|
|$item_chest_dress1|Basit kahverengi elbise|
|$item_chest_dress1_description|Basit bir kahverengi elbise.|
|$item_chest_dress10|Basit normal elbise|
|$item_chest_dress10_description|Basit bir elbise.|
|$item_chest_dress2|Şallı kahverengi elbise|
|$item_chest_dress2_description|İnsanı sıcak tutan bir şalın eşliğinde kahverengi bir elbise.|
|$item_chest_dress3|Boncuklu kahverengi elbise|
|$item_chest_dress3_description|Boncuklu ve gümüş süslü kahverengi bir elbise.|
|$item_chest_dress4|Basit mavi elbise|
|$item_chest_dress4_description|Basit bir mavi elbise.|
|$item_chest_dress5|Şallı mavi elbise|
|$item_chest_dress5_description|İnsanı sıcak tutan bir şalın eşliğinde mavi bir elbise.|
|$item_chest_dress6|Boncuklu mavi elbise|
|$item_chest_dress6_description|Boncuklu ve bronz süslü mavi bir elbise.|
|$item_chest_dress7|Basit sarı elbise|
|$item_chest_dress7_description|Basit bir sarı elbise.|
|$item_chest_dress8|Şallı sarı elbise|
|$item_chest_dress8_description|İnsanı sıcak tutan bir şalın eşliğinde sarı bir elbise.|
|$item_chest_dress9|Boncuklu sarı elbise|
|$item_chest_dress9_description|Boncuklu ve gümüş süslü sarı bir elbise.|
|$item_chest_fenris|Fenris postu|
|$item_chest_fenris_description|Yaratık, uluması diyarın uzak noktalarında duyulsun diye derin nefes alabiliyordu.|
|$item_chest_flametal|Oddemir Göğüs Zırhı|
|$item_chest_flametal_description|Bu gizemli oddemir ve kor kemik birleşimi, Midgard'da bulabileceğin her şeyden daha dayanıklı bir koruyucu katman olarak işlev görüyor.|
|$item_chest_harvester1|Hasat Tuniği|
|$item_chest_harvester1_description|Tarlada çalışırken buna uygun giyinmek önemlidir. Kısa bir tunik giymek serinlik sağlar.|
|$item_chest_harvester2|Hasat Elbisesi|
|$item_chest_harvester2_description|Tarlada çalışırken buna uygun giyinmek önemlidir. Bu uzun elbise, çalışırken dizlerini örter.|
|$item_chest_hildir_description|Hildir'e aittir, lütfen bulursanız getirin.|
|$item_chest_hildir1|Hildir'in pirinç sandığı|
|$item_chest_hildir2|Hildir'in gümüş sandığı|
|$item_chest_hildir3|Hildir'in bronz sandığı|
|$item_chest_iron|Demir pul zırh|
|$item_chest_iron_description|En güçlü darbeler dışındaki tüm darbeleri karşılayan demir pul zırh.|
|$item_chest_leather|Deri tunik|
|$item_chest_leather_description|Hayvan derisinden yapılmış bir tunik.|
|$item_chest_mage|Eitr dokumalı cüppe|
|$item_chest_mage_ashlands|Embla'nın Cübbesi|
|$item_chest_mage_ashlands_description|İlk sahirenin gücüyle dolu olan bu cübbe, kadim büyü sanatını kullanmak isteyenlere bir lütuf sunacak.|
|$item_chest_mage_description|Bu ustaca katman katman tasarlanmış cüppelerin her dikiş ve kıvrımında büyü ve efsunlar bulunur.|
|$item_chest_medium_ashlands|Ask'ın Göğüs Zırhı|
|$item_chest_medium_ashlands_description|Midgard'daki ilk insan en hayati organlarını, belki de tıpkı bunun gibi bir göğüs zırhıyla nasıl koruyacağını biliyordu.|
|$item_chest_pcuirass|Takviyeli kuraza|
|$item_chest_pcuirass_description|Güzelce işlenmiş ve en güçlü bıçak darbelerine bile dayanıklı.|
|$item_chest_rags|Paçavra tunik|
|$item_chest_rags_description|Hiç yoktan iyidir.|
|$item_chest_root|Kök gövde zırhı|
|$item_chest_root_description|Kadim kökler ve kabuklarla garip bir şekilde birleştirilip dokunmuş hafif bir zırh.|
|$item_chest_trollleather|Trol derisi tunik|
|$item_chest_trollleather_description|Trollerin derisini yüzmek zordur ama derileri serttir ve sıcak tutar.|
|$item_chest_tunic1|Basit mavi tunik|
|$item_chest_tunic1_description|Basit bir mavi tunik.|
|$item_chest_tunic10|Basit normal tunik|
|$item_chest_tunic10_description|Basit bir tunik.|
|$item_chest_tunic2|Pelerinli mavi tunik|
|$item_chest_tunic2_description|Bir pelerin eşliğinde mavi bir tunik.|
|$item_chest_tunic3|Boncuklu mavi tunik|
|$item_chest_tunic3_description|Boncuklu ve gümüş süslü mavi bir tunik.|
|$item_chest_tunic4|Basit kırmızı tunik|
|$item_chest_tunic4_description|Basit bir kırmızı tunik.|
|$item_chest_tunic5|Pelerinli kırmızı tunik|
|$item_chest_tunic5_description|Bir pelerin eşliğinde kırmızı bir tunik.|
|$item_chest_tunic6|Boncuklu kırmızı tunik|
|$item_chest_tunic6_description|Boncuklu ve bronz süslü kırmızı bir tunik.|
|$item_chest_tunic7|Basit sarı tunik|
|$item_chest_tunic7_description|Basit bir sarı tunik.|
|$item_chest_tunic8|Pelerinli sarı tunik|
|$item_chest_tunic8_description|Bir pelerin eşliğinde sarı bir tunik.|
|$item_chest_tunic9|Boncuklu sarı tunik|
|$item_chest_tunic9_description|Boncuklu ve gümüş süslü sarı bir tunik.|
|$item_chest_wolf|Kurt zırhı|
|$item_chest_wolf_description|Vahşi görünümlü ve sıcak tutan bir kurt kürkü yelek. Soğuğa karşı korur.|
|$item_chicken_egg|Yumurta|
|$item_chicken_egg_cold|(Çok soğuk)|
|$item_chicken_egg_description|Sıcak tutarak içinden ne çıkacağını gör... Gerçi önce yumurta mı ondan çıktı acaba?|
|$item_chicken_egg_stacked|(Yığınlı halde yetişemez)|
|$item_chicken_egg_warm|(Sıcak)|
|$item_chicken_meat|Tavuk eti|
|$item_chicken_meat_cooked|Pişmiş tavuk eti|
|$item_chicken_meat_cooked_description|Tadı tavuk gibi.|
|$item_chicken_meat_description|Tüm tavuklar atalarından kalma leziz olma lanetini taşır.|
|$item_chitin|Çitin|
|$item_chitin_description|Deniz kabuklusunun bir parçası.|
|$item_cloudberries|Norveç böğürtleni|
|$item_cloudberries_description|Ormanın cevheri.|
|$item_club|Sopa|
|$item_club_description|Kaba fakat işe yarar bir silah.|
|$item_coal|Kömür|
|$item_coal_description|Bir kömür öbeği.|
|$item_coin|Sikke|
|$item_coin_description|Altın sikke.|
|$item_coins|Sikkeler|
|$item_coins_description|<color=yellow>Değerli Eşya</color>|
|$item_copper|Bakır|
|$item_copper_description|İşlenmeye hazır bir saf bakır külçesi.|
|$item_copperore|Bakır madeni|
|$item_copperore_description|İşlenmemiş bakır. Eritme fırınında işlenmesi gerekiyor.|
|$item_copperscrap|Bakır hurda|
|$item_copperscrap_description|Birinin hurdası başkasının hazinesidir.|
|$item_crafter|Üreten:|
|$item_crossbow_arbalest|Arbalet|
|$item_crossbow_arbalest_description|Yavaş ama güçlü bir silah.|
|$item_crossbow_ripper|Yırtıcı|
|$item_crossbow_ripper_blood|Yara Yırtıcı|
|$item_crossbow_ripper_blood_description|Düşmanlarını parçalara ayırmaya hazır.|
|$item_crossbow_ripper_description|Düşmanlarını yırtar. Bu kadar basit.|
|$item_crossbow_ripper_lightning|Fırtına Yırtıcı|
|$item_crossbow_ripper_lightning_description|Yıldırımlar fazlasıyla berbat bir rüzgâr gibi düşmanlarını yarıp geçecek.|
|$item_crossbow_ripper_nature|Kök Yırtıcı|
|$item_crossbow_ripper_nature_description|Ok, düşmanını yerine mıhlamazsa topraktan biten kökler kesinlikle mıhlayacaktır.|
|$item_cryptkey|Bataklık anahtarı|
|$item_cryptkey_description|Kısmen kabuklaşmış çamurla kaplı ve leş gibi kokuyor.|
|$item_crystal|Kristal|
|$item_crystal_description|Toprağın derinlerinden gelen bir kristal parçası.|
|$item_cultivator|Saban|
|$item_cultivator_description|Toprağı sürmeye yarayan tarım aleti.|
|$item_curedsquirrelhamstring|Kurutulmuş Sincap Tendonu|
|$item_curedsquirrelhamstring_description|Esnek ve güçlü. Bu tendon oldukça hızlı ve çevik bir hayvandan geliyor olmalı.|
|$item_current|Mevcut|
|$item_damagemultiplierhp|Eksik can başına hasar artar|
|$item_damagemultipliertotal|Eksik toplam can miktarına göre hasar artar|
|$item_dandelion|Karahindiba|
|$item_dandelion_description|Bir karahindiba.|
|$item_deer_meat|Geyik eti|
|$item_deer_meat_cooked|Pişmiş geyik eti|
|$item_deer_meat_cooked_description|Onca koşuşturmaya değdi.|
|$item_deer_meat_description||
|$item_deerhide|Geyik derisi|
|$item_deerhide_description|Geyikten yüzülmüş temiz deri.|
|$item_deerstew|Geyik güveci|
|$item_deerstew_description|Ağızda dağılır kıvamda.|
|$item_deflection|Savuşturma gücü|
|$item_deflection_OLD|Saptırma|
|$item_demister|Peri Işığı|
|$item_demister_description|En yoğun sislerde sana rehberlik edip bağlı kalacak bir peri.|
|$item_destructible_gucksack|Jölemsi öbeği|
|$item_destructible_unstablelavarock|Dengesiz Lav Kayası|
|$item_dlc|DLC eşyası|
|$item_dragonegg|Ejderha yumurtası|
|$item_dragonegg_description|Göründüğünden çok daha ağır ve içinden ince bir uğultu yükseliyor.|
|$item_dragontear|Ejderha gözyaşı|
|$item_dragontear_description|Bir ejderhanın gizemli enerjiyle atan son göz yaşları.|
|$item_drink|Drink|
|$item_durability|Sağlamlık|
|$item_dvergrkey|Mühürkıran|
|$item_dvergrkey_description|Bir Cüce mührünü kırmak için kullanılan bir nesne.|
|$item_dvergrkeyfragment|Mühürkıran parçası|
|$item_dvergrkeyfragment_description|Cüce mühürkıranının bir parçası.|
|$item_dvergrneedle|Cüce çıkarıcısı|
|$item_dvergrneedle_description|Bir şeyi delmek için harika bir eşyaya benziyor...|
|$item_dvergrtankard|Cüce kupası|
|$item_dvergrtankard_description|Çok fazla şarap alabilir!|
|$item_Dyrnwyn_blade|Dyrnwyn Kılıç Parçası|
|$item_Dyrnwyn_blade_description|Eski bir kılıcın parçaları. Tüm parçalar tekrar birleştirilirse muhtemelen bir bütün hâline getirilebilir.|
|$item_Dyrnwyn_hilt|Dyrnwyn Kabza Parçası|
|$item_Dyrnwyn_hilt_description|Çoktan unutulmuş bir kılıcın kabzası. Biri tüm parçalara sahip olsa belki yeniden dövülebilirdi...|
|$item_Dyrnwyn_tip|Dyrnwyn Uç Parçası|
|$item_Dyrnwyn_tip_description|Bu metal parçası bir kılıcın ucuna benziyor. Daha fazla parça bulunursa biri belki kılıcı yeniden dövebilir...|
|$item_eat|Ye|
|$item_ectoplasm|Ektoplazma|
|$item_ectoplasm_description|Eskiden yaşayan bir şeyin huzursuz özü...|
|$item_egg_cooked|Pişmiş yumurta|
|$item_egg_cooked_description|Sahanda!|
|$item_eitr|Arıtılmış eitr|
|$item_eitr_description|Bu, hayatın ürünü, kendini tüketen zehir. Cüceler kendilerine has tasarımlarda kullanmak üzere bunu arıtırlar.|
|$item_eitrregen_modifier|Eitr yenilenmesi|
|$item_eitruse|Eitr kullan|
|$item_elderbark|Kadim kabuk|
|$item_elderbark_description|Kadim ve sağlam bir malzeme.|
|$item_entrails|İç organlar|
|$item_entrails_description|Bir şeyin içinden çıkan uzun ve yapışkan organlar.|
|$item_eyescream|Göz Glase|
|$item_eyescream_description|Buz gibi ve kaymaklı.|
|$item_fader_drop|Sönümleyici Yadigârı|
|$item_fader_drop_description|Mistik güç başka bir gün açığa çıkacak.|
|$item_falldamage|Düşme hasarı|
|$item_feastashlands|Kül Diyarı Gurme Kâsesi|
|$item_feastashlands_description|Yeni piştiği için mi yoksa içindeki kül domuzu eti yüzünden mi bu yemekten buhar çıktığını kestirmek güç. Her hâlükârda asma yemişleri ve eğrelti otu filizleriyle beraber sunulan bu baharatlı et o kadar lezzetli oluyor ki hiç fark etmez!|
|$item_feastblackforest|Kara Orman Yemek Tepsisi|
|$item_feastblackforest_description|Kara Orman'dan gelen bu lezzet şölenine karşı koyamayacaksın! Baharatlı deve dikeni ve havuçla servis edilen geyik eti, leziz reçellerle tamamlanır. Hadi bir ısırık daha al!|
|$item_feaster|Servis Tepsisi|
|$item_feaster_description|Masayı dilediğin yiyecek ve içecekle donatarak misafirlerini leziz bir ziyafetle etkile.|
|$item_feaster_remove_description|<color=yellow>Once a feast has been eaten of, it won't return any resources if dismantled.</color>|
|$item_feastmeadows|Kızarmış Bütün Çayır Yaban Domuzu|
|$item_feastmeadows_description|Üzerine bal sürülerek mükemmel bir şekilde kızartılmış, mantar ve ahududu yatağında sunulan bir yaban domuzu. Yanında da geyik kaburgaları ve Neck kuyrukları ile servis edilir. Böylesine bir ziyafetin karnını doyurup gününü güzelleştirmeme ihtimali yoktur!|
|$item_feastmistlands|Sisli Topraklar Mantar Ziyafeti|
|$item_feastmistlands_description|Mantarseverlere müjde! Farklı türde mantarların, mantarda marine edilmiş arayıcı etinin ve içi mantarla doldurulmuş sisli yaban tavşanının tadına bakın. Lezzetli öz su sosunu da denemeyi unutmayın.|
|$item_feastmountains|Doyurucu Dağ Oduncusu Yahnisi|
|$item_feastmountains_description|Soğukta bir gün geçirdikten sonra bu buhar çıkaran lezzet dolu çömleğin etrafında toplanıp ısınmak gibisi yoktur. Genelde sert ve çiğnemesi zor olmasına rağmen bu kurt eti ağızda resmen eriyecek kadar yumuşak. Yanında da baharatlarla tatlandırılmış soğanlar ve havuçlar var.|
|$item_feastoceans|Denizcinin Ganimeti|
|$item_feastoceans_description|Balık, balık ve daha fazla balık! Bir de balık şeklinde kesilmiş yılan eti! Okyanusun lezzetlerini keşfet! Hâlâ gemi yolculuklarına alışmaya çalışanlar için közlenmiş sebzeler de var.|
|$item_feastplains|Turta Tepsisi|
|$item_feastplains_description|Lezzeti tüm ovalarda konuşulur! Fırından yeni çıkmış çeşit çeşit turtalar ve ekmeklerle içini ısıtacak bu ziyafeti deneyimle.|
|$item_feastswamps|Bataklık Sakininin Aşı|
|$item_feastswamps_description|Sülüklerin yenebildiğini kim bilirdi ki? Doğru bir şekilde hazırlayarak (uzun süre pişirilip bolca baharat eklendiğinde) onları bu ziyafetteki cesur gurmelere sunabilirsin. Tabii midesi hassas olanlar için yanında leziz şiş sosisler ve şalgamlar da var.|
|$item_feathers|Tüyler|
|$item_feathers_description|Küçük bir tüy öbeği.|
|$item_fiddleheadfern|Eğrelti Otu Filizi|
|$item_fiddleheadfern_description|Kıvrımlı sebzeler!|
|$item_fierysvinstew|Alevli Domuz Yahnisi|
|$item_fierysvinstew_description|Bu ağır yahni her maceracının menüsünde olmalı.|
|$item_finewood|Kaliteli Odun|
|$item_finewood_description|Güzel şekilde yontulabilecek yüksek kaliteli ahşap.|
|$item_fircone|Köknar kozalağı|
|$item_fircone_description|Köknar ağacı istiyorsan toprağa ek.|
|$item_fireworkrocket_blue|Mavi havai fişek|
|$item_fireworkrocket_cyan|Camgöbeği havai fişek|
|$item_fireworkrocket_description|Bu roket tekrardan uçuyor!|
|$item_fireworkrocket_green|Yeşil havai fişek|
|$item_fireworkrocket_purple|Mor havai fişek|
|$item_fireworkrocket_red|Kırmızı havai fişek|
|$item_fireworkrocket_white|Basit havai fişek|
|$item_fireworkrocket_yellow|Sarı havai fişek|
|$item_fish_cooked|Pişmiş balık|
|$item_fish_cooked_description|Tütsülenmiş balığın lezzetli yanı.|
|$item_fish_raw|Çiğ balık|
|$item_fish_raw_description|İyi bir av.|
|$item_fishandbread|Balık ekmek|
|$item_fishandbread_description|Karanın ve denizin armağanları.|
|$item_fishandbreaduncooked|Pişmemiş balık ekmek|
|$item_fishingbait|Balık yemi|
|$item_fishingbait_ashlands|Sıcak balık yemi|
|$item_fishingbait_ashlands_description|Bazı balıklar suların zaten ılık olduğu yerleri sever ve bu yem de sıcaklığı kaynama noktasına yaklaştırır.|
|$item_fishingbait_cave|Soğuk balık yemi|
|$item_fishingbait_cave_description|Bu yem pek bir şeye benzemese de soğuk ve karanlık yerlerde yaşayan balıklar için ziyafet gibidir.|
|$item_fishingbait_deepnorth|Buzlu balık yemi|
|$item_fishingbait_deepnorth_description|Pek besleyici olmadığı için bu yeme gelecek tek balıklar sadece buz yemeye alışık olanlardır.|
|$item_fishingbait_description|Cücelerin kullandığı sıradan bir balık yemi. Olta ayrı satılır.|
|$item_fishingbait_forest|Yosunlu balık yemi|
|$item_fishingbait_forest_description|Ormandaki ölü troller sık sık birkaç balık çeker, bu yemle süreci hızlandırabilirsin!|
|$item_fishingbait_mistlands|Sisli balık yemi|
|$item_fishingbait_mistlands_description|Balıkları saklı sulardan sana doğru getirecek bir yem.|
|$item_fishingbait_ocean|Ağır balık yemi|
|$item_fishingbait_ocean_description|Bu yem denizin diplerinde yüzen balıkları çekecek kadar derinlere batar.|
|$item_fishingbait_plains|Sivri balık yemi|
|$item_fishingbait_plains_description|Senin ölüm sivrilerine bağışıklığın olmadığı gibi, buradaki balıkların da bu atıştırmalığa bağışıklığı yok!|
|$item_fishingbait_swamp|Yapışkan balık yemi|
|$item_fishingbait_swamp_description|İğrenç kokabilir ama bu yem tarafından cezbedilen balıklar çok daha beterdir.|
|$item_fishingrod|Olta|
|$item_fishingrod_description|Cücelerin genel olarak kullandığı bir olta.|
|$item_fishwraps|Balık dürüm|
|$item_fishwraps_description|Ekmek ve balık, daha ne olsun?|
|$item_fistweapon_bjorn|Ayının Pençeleri|
|$item_fistweapon_bjorn_description|Yırtmak ve parçalamak için yapıldı.|
|$item_fistweapon_bjorn_undead|Menfurkemiği Ezicipençeler|
|$item_fistweapon_bjorn_undead_description|Bu pençeler eti kemik demeden parçalar.|
|$item_fistweapon_fenris|Et kesiciler|
|$item_fistweapon_fenris_description|Pençeler kurtların işine yarıyorsa neden vikinglerin işine yaramasın?|
|$item_flametal|Oddemir|
|$item_flametal_description|Bir meteorun saf ve titrekçe ışıldayan çekirdeği.|
|$item_flametal_old|Kadim Metal|
|$item_flametal_old_description|Kadim zamanlardan solmuş bir metal. Bir zamanlar ne için kullanıldığını kimse bilmiyor.|
|$item_flametalore|Flametal Ore|
|$item_flametalore_description|Işıldayan damarlarından ısı yayan garip bir metal. Eritme fırınında işlenmesi gerekiyor.|
|$item_flametalore_old|Parlayan Metal Cevheri|
|$item_flax|Keten|
|$item_flax_description|Keten bitkisine ait işlenmemiş lifler.|
|$item_flint|Çakmak taşı|
|$item_flint_description|Keskin bıçağa dönüştürülebilir.|
|$item_food|Yiyecek|
|$item_food_duration|Süre|
|$item_food_eitr|Eitr|
|$item_food_health|Sağlık|
|$item_food_regen|İyileştirme|
|$item_food_stamina|Enerji|
|$item_fragrantbundle|Rayihalı Karışım|
|$item_fragrantbundle_description|Bu bitkilerin keskin ama hoş bir kokusu vardır. Tabii her yaratık aynı fikirde olmayabilir...|
|$item_freezegland|Donmuş beze|
|$item_freezegland_description|Bu gizemli organ, sıcaklığı mükemmel seviyede tutuyor.|
|$item_freshseaweed|Taze Su Yosunu|
|$item_freshseaweed_description|Bu bitkinin tuzlu su kokusu akıllara engin suları getirir.|
|$item_fulladrenaline|Adrenalin etkisi|
|$item_gemstone_blue|İyolit|
|$item_gemstone_blue_description|Işık bu mücevherden keskin bir şekilde yansıyor mu, yoksa taşın kendisinin içinden mi geliyor?|
|$item_gemstone_green|Yeşim|
|$item_gemstone_green_description|Bu mücevher sanki canlı bir şeymiş gibi enerjiyle titreşiyor.|
|$item_gemstone_red|Kan Taşı|
|$item_gemstone_red_description|Bu mücevheri tatmin etmek için kaç anlaşmanın yapıldığını merak ediyorsun. Kaç avuç güç karşılığında üstüne kanını akıttı?|
|$item_goblintotem|Fuling totemi|
|$item_goblintotem_description|Yagluth'un kadim güçlerini yayar.|
|$item_grausten|Gritaş|
|$item_grausten_description|Gözenekli ancak sağlam.|
|$item_greydwarfeye|Bozcüce gözü|
|$item_greydwarfeye_description|Bozcüce'nin süt akı göz küresi.|
|$item_guck|Jölemsi|
|$item_guck_description|Fermante edilmiş balık gibi kokuyor.|
|$item_hammer|Çekiç|
|$item_hammer_description|Elinde bu varken devasa salonlar ve kudretli surlar inşa edebilirsin.|
|$item_hardantler|Sert çatal boynuz|
|$item_hardantler_description|Çok sert bir çatal boynuz parçası.|
|$item_hardrock|Mysterious Rock|
|$item_hardrock_description|It's just a rock...|
|$item_hardrock_knowledge1|Fear not death, for the hour of your doom is set, and no man can escape it.|
|$item_hardrock_knowledge10|You can keep as quiet as you like, but one day, someone will find you.|
|$item_hardrock_knowledge11|No matter how much suffering you went through, you never want to let go of those memories.|
|$item_hardrock_knowledge12|When you come out of the storm, you won’t be the same person who walked in. That’s what the storm is all about.|
|$item_hardrock_knowledge13|A rock feels no pain, and an island never cries.|
|$item_hardrock_knowledge2|Better to fight and fall than to live without hope.|
|$item_hardrock_knowledge3|Runes shall you find, and rightly read, of wondrous weight and mighty magic.|
|$item_hardrock_knowledge4|Man cannot discover new oceans unless he has the courage to lose sight of the shore.|
|$item_hardrock_knowledge5|He who has a why to live can bear almost any how.|
|$item_hardrock_knowledge6|The wound is the place where the light enters you.|
|$item_hardrock_knowledge7|We suffer more in imagination than in reality.|
|$item_hardrock_knowledge8|Pain is inevitable. Suffering is optional.|
|$item_hardrock_knowledge9|Whatever it is you’re seeking won’t come in the form you’re expecting.|
|$item_hare_meat|Yabantavşanı eti|
|$item_hare_meat_cooked|Pişmiş yabantavşanı eti|
|$item_hare_meat_cooked_description|Lifli ama lezizdir.|
|$item_hare_meat_description|Yabantavşanının eti az ama lezzetlidir.|
|$item_healthhitreturn|Darbelerde can kazandırır|
|$item_healthuse|Sağlık kullan|
|$item_heat_modifier|Isı direnci|
|$item_helmet_berserker|Ayının Başlığı|
|$item_helmet_berserker_description|Kudretli bir ayının ilkel hiddetinin seni yutmasına izin ver.|
|$item_helmet_berserker_undead|Menfurkemiği Maske|
|$item_helmet_berserker_undead_description|Takıldığında düşmana korku, dosta güven salar.|
|$item_helmet_bronze|Tunç miğfer|
|$item_helmet_bronze_description|Beynini kafatasında tutmana yardımcı olur.|
|$item_helmet_bronzehorned|Boynuzlu tunç miğfer|
|$item_helmet_bronzehorned_description|Fazladan karizma için boynuz takılmış bir tunç miğfer.|
|$item_helmet_carapace|Kabuk miğfer|
|$item_helmet_carapace_description|İnsanlar dev bir karıncaya benzediğini söyleyebilirler. Ama bunu yalnızca bir kez diyecekler.|
|$item_helmet_celebration|Kutlama Başlığı|
|$item_helmet_celebration_description|Bu başlığı taktığında bir mutluluk bastırıyor üstüne!|
|$item_helmet_drake|Ejder miğferi|
|$item_helmet_drake_description|İncelikle ve ustaca işlenmiş bir miğfer.|
|$item_helmet_dverger|Dverger tacı|
|$item_helmet_dverger_description|Zindan kâşifleri için taşınabilir ve kesintisiz bir ışık kaynağı.|
|$item_helmet_fenris|Fenris başlığı|
|$item_helmet_fenris_description|Yaratığın gözleri, bir savaşçının gücünü tek bakışta ölçebilmesi için bilge ve uyanıktı.|
|$item_helmet_fishinghat|Balıkçı şapkası|
|$item_helmet_fishinghat_description|Bu ilgi çekici şapka yalnızca en tecrübeli maceraların oltalarına takılabilir.|
|$item_helmet_flametal|Oddemir Miğferi|
|$item_helmet_flametal_description|Bu miğferi takarken düşmanların kafanı ısırıp koparmaya çalışmadan önce iki kez düşünür.|
|$item_helmet_hat1|Bağlı mavi başörtü|
|$item_helmet_hat1_description|Kullanışlı bir mavi başörtü.|
|$item_helmet_hat10|Basit mor başlık|
|$item_helmet_hat10_description|Basit ama şık bir mor başlık.|
|$item_helmet_hat2|Bükülmüş yeşil başörtü|
|$item_helmet_hat2_description|Süslü bir yeşil başörtü.|
|$item_helmet_hat3|Kürk kahverengi başlık|
|$item_helmet_hat3_description|En güzelinden bir deriden yapılmış sıcak tutan bir kürk başlık.|
|$item_helmet_hat4|Gösterişli yeşil başlık|
|$item_helmet_hat4_description|Yeşil ve gösterişli bir başlık.|
|$item_helmet_hat5|Basit kırmızı başlık|
|$item_helmet_hat5_description|Basit ama şık bir kırmızı başlık.|
|$item_helmet_hat6|Bağlı sarı başörtü|
|$item_helmet_hat6_description|Kullanışlı bir sarı başörtü.|
|$item_helmet_hat7|Bükülmüş kırmızı başörtü|
|$item_helmet_hat7_description|Süslü bir kırmızı başörtü.|
|$item_helmet_hat8|Kürk gri başlık|
|$item_helmet_hat8_description|En güzelinden bir yünden yapılmış sıcak tutan bir kürk başlık.|
|$item_helmet_hat9|Gösterişli turuncu başlık|
|$item_helmet_hat9_description|Turuncu ve gösterişli bir başlık.|
|$item_helmet_iron|Demir miğfer|
|$item_helmet_iron_description|Parlak demirden yapılan, kahramana layık bir başlık.|
|$item_helmet_leather|Deri miğfer|
|$item_helmet_leather_description|Sertleştirilmiş deriden başlık.|
|$item_helmet_mage|Eitr dokumalı başlık|
|$item_helmet_mage_ashlands|Embla'nın Başlığı|
|$item_helmet_mage_ashlands_description|İlk sahire bile suratını kapatmanın gizemine değer vermişti. Söylentilere göre onu böyle onurlandırmak büyülerini daha da güçlü hâle getirirmiş.|
|$item_helmet_mage_description|Büyücülük gözlerden belli olur, bu yüzden çoğu büyücü esrarengiz uğraşlarını gizlemek için başlık giyer.|
|$item_helmet_medium_ashlands|Ask'ın Başlığı|
|$item_helmet_medium_ashlands_description|Midgard'ın ilk insanı olarak Ask, üstüne çok fazla dikkat çekmemenin önemli olduğunu biliyordu.|
|$item_helmet_midsummercrown|Yaz Tacı|
|$item_helmet_midsummercrown_description|Çiçeklerden oluşan bir taç ile yazı kutla.|
|$item_helmet_odin|Odin'in başlığı|
|$item_helmet_odin_description|Odin'in gözde savaşçıları en kaliteli kumaşları hak eder.|
|$item_helmet_padded|Takviyeli miğfer|
|$item_helmet_padded_description|Tam oturan ve ince işçilikli başlık.|
|$item_helmet_root|Kök maskesi|
|$item_helmet_root_description|Bu kök ve kabuktan yapılma budak kafana mükemmel bir şekilde oturuyor.|
|$item_helmet_strawhat|Hasır Şapka|
|$item_helmet_strawhat_description|Güneş çarpmasından kaçınmanın en iyi yolu.|
|$item_helmet_sweatband|Kafa Bandı|
|$item_helmet_sweatband_description|Biraz... nemli hissettiriyor.|
|$item_helmet_trollleather|Trol derisi miğfer|
|$item_helmet_trollleather_description|Trol derisini işlemek güçtür ama zırhı sıra dışı kılar.|
|$item_helmet_witchhat|Sivri şapka|
|$item_helmet_witchhat_description|Bu şapkanın her kıyafete büyülü bir hava katacağı kesin.|
|$item_helmet_yule|Yule şapkası|
|$item_helmet_yule_description|Ev cinlerini anımsatan bir kırmızı başlık.|
|$item_hide|Deri|
|$item_hide_description|Bir hayvanın derisi.|
|$item_hildir_divan|Divan|
|$item_hildirkey_description|Sahibini özlemişe benziyor...|
|$item_hildirkey1|Hildir'in pirinç anahtarı|
|$item_hildirkey2|Hildir'in gümüş anahtarı|
|$item_hildirkey3|Hildir'in bronz anahtarı|
|$item_hoe|Çapa|
|$item_hoe_description|Çiftçilerin toprağı işlemek için kullandığı alet.|
|$item_honey|Bal|
|$item_honey_description|Tatlı ve leziz.|
|$item_honeyglazedchicken|Ballı tavuk|
|$item_honeyglazedchicken_description|Kusursuz bir şekilde ızgaralandı. Hem göze hem de ağza hitap ediyor.|
|$item_honeyglazedchickenuncooked|Pişmemiş ballı tavuk|
|$item_iron|Demir|
|$item_iron_description|İşlenmeye hazır bir saf demir külçesi.|
|$item_ironnails|Demir çiviler|
|$item_ironnails_description|Gelişmiş inşaat projeleri için gereklidir.|
|$item_ironore|Demir madeni|
|$item_ironore_description|İşlenmemiş demir. Eritme fırınında işlenmesi gerekiyor.|
|$item_ironpit|Demir çukur|
|$item_ironpit_description|Odun ve çıra ile doldurulmayı bekleyen boş bir kazan|
|$item_ironscrap|Hurda demir|
|$item_ironscrap_description|Eskiyip paslanmış olsa da eritilerek tekrar kullanılabilir.|
|$item_jotunpuffs|Jötunn pufları|
|$item_jotunpuffs_description|Büyüyle şişirilmiş bu altın küreler, Jötunn'ların düştüğü yerlerde beliriyor.|
|$item_juteblue|Mavi jüt|
|$item_juteblue_description|Doğal liflerden ve cüce saçından yapılmıştır.|
|$item_jutered|Kırmızı jüt|
|$item_jutered_description|Sağlam, sert bir kumaş.|
|$item_knife_blackmetal|Karademir bıçak|
|$item_knife_blackmetal_description|Güçlü, keskin ve kapkara bir bıçak.|
|$item_knife_butcher|Kasap bıçağı|
|$item_knife_butcher_description|Ehlîleştirilmiş hayvanları kesmek için özel olarak tasarlanmış bir kasap bıçağı.|
|$item_knife_chitin|Derinlikler usturası|
|$item_knife_chitin_description|Derinlerden gelen bir bıçak.|
|$item_knife_copper|Bakır bıçak|
|$item_knife_copper_description|Parıldayan bakır bir bıçak.|
|$item_knife_flint|Çakmak taşı bıçak|
|$item_knife_flint_description|Keskinleştirilmiş çakmak taşından güvenilir bir alet.|
|$item_knife_silver|Gümüş bıçak|
|$item_knife_silver_description|Acıya sebep olan tehlikeli bir parça.|
|$item_knife_skollandhati|Sköll ve Hati|
|$item_knife_skollandhati_description|Zamanında sana ihanet edenleri bir kez, nefret ettiklerini iki kez bıçakla.|
|$item_knife_wood|Tahta Bıçak|
|$item_knife_wood_description|Kimisi bu bıçağın tereyağı için daha uygun olduğunu söyler.|
|$item_knockback|Yere serme|
|$item_kvastur_description|Bir cadının en iyi arkadaşı.|
|$item_lantern|Cüce feneri|
|$item_lantern_description|Sıradan bir meşale çok eski moda kalırdı.|
|$item_lavabomb|Bazalt Bombası|
|$item_lavabomb_description|Yeterli ısıyla patlama anında katılaşır.|
|$item_leatherscraps|Deri hurdaları|
|$item_leatherscraps_description|Küçük bir deri hurdası öbeği.|
|$item_legs_berserker|Ayının Peştamalı|
|$item_legs_berserker_description|Önemli yerlerin hepsini kapatıyor... sadece önemli yerleri.|
|$item_legs_berserker_undead|Menfurkemiği Etek|
|$item_legs_berserker_undead_description|Sert bir eteğe işlenmiş kalın deri ve kemik parçaları.|
|$item_legs_bronze|Bronz plaka bacaklıklar|
|$item_legs_bronze_description|Bacaklarını koruman için tunç dizçekler.|
|$item_legs_carapace|Kabuk dizçekler|
|$item_legs_carapace_description|Sert kabuktan bacak zırhı.|
|$item_legs_fenris|Fenris bacaklıkları|
|$item_legs_fenris_description|Yaratığın bacakları, uzun sıçramalar yapabilmesi için ince ve güçlüydü.|
|$item_legs_flametal|Oddemir Çizmeleri|
|$item_legs_flametal_description|Ağır bir pantolon ısıya karşı yalıtım sağlarken sağlam çizmeler dizlerini alttan gelen darbelerden korur.|
|$item_legs_iron|Demir dizçekler|
|$item_legs_iron_description|Bacaklarını koruman için demir dizçekler.|
|$item_legs_leather|Deri pantolon|
|$item_legs_leather_description|Yürürken biraz gıcırdıyor.|
|$item_legs_mage|Eitr dokumalı pantolon|
|$item_legs_mage_ashlands|Embla'nın Pantolonu|
|$item_legs_mage_ashlands_description|İlk sahirenin gerçekten tıpkı bunun gibi bir pantolon giyip giymediğini asla bilemeyeceğiz.|
|$item_legs_mage_description|Büyücülerin giydiği pantolonlar hep dar olur. Büyü için gereken odaklanmayı bu rahatsızlık teşvik eder...|
|$item_legs_medium_ashlands|Ask'ın Pantolonu|
|$item_legs_medium_ashlands_description|Midgard'a ayak basan ilk insan tedbirli olmayı tercih ediyordu. Bu pantolonu giyenler ondan bir iki şey öğrenebilir.|
|$item_legs_pgreaves|Takviyeli dizçekler|
|$item_legs_pgreaves_description|Ustaca üretilmiş bacak koruması.|
|$item_legs_rags|Paçavra pantolon|
|$item_legs_rags_description|Çıplaklığa karşı birebir.|
|$item_legs_root|Kök bacaklıkları|
|$item_legs_root_description|Kadim kökler ve kabuklarla garip bir şekilde birleştirilip dokunmuş hafif bir zırh.|
|$item_legs_trollleather|Trol derisi pantolon|
|$item_legs_trollleather_description|Sert trol derisinden pantolon.|
|$item_legs_wolf|Kurt bacaklıklar|
|$item_legs_wolf_description|Kabarık kurt kürkü bacaklıklar.|
|$item_limitfallspeed|Düşme hızını sınırlar|
|$item_linenthread|Keten iplik|
|$item_linenthread_description|Güçlü keten bitkisi liflerinden dokunmuş kaliteli bir keten.|
|$item_loxmeat|Lox eti|
|$item_loxmeat_cooked|Pişmiş Lox eti|
|$item_loxmeat_cooked_description|Oldukça iri ve güzel pişmiş, Valhalla'ya yakışır bir et!|
|$item_loxmeat_description|Yağlı bir çiğ et dilimi.|
|$item_loxpelt|Lox postu|
|$item_loxpelt_description|Ağır bir yoğun ve küflü post.|
|$item_loxpie|Lox etli turta|
|$item_loxpie_description|O mis kokuyu burnuna çekmek için hamurunu ayır. Enfes!|
|$item_loxpie_uncooked|Pişmemiş lox turtası|
|$item_loxpie_uncooked_description|Fırına girmeye hazır.|
|$item_mace_bronze|Tunç topuz|
|$item_mace_bronze_description|Çubuğun ucuna takılmış bir baş ağrısı.|
|$item_mace_eldner|Oddemir Gürz|
|$item_mace_eldner_blood|Kan Tokmağı|
|$item_mace_eldner_blood_description|Kan bu gürzün neredeyse içine işlemiş ve hep daha fazlasını arzuluyor.|
|$item_mace_eldner_description|Ağır ve dikenli oddemir. Düşmanlarının suratlarını göçertmek için mükemmel.|
|$item_mace_eldner_lightning|Fırtına Yıldızı|
|$item_mace_eldner_lightning_description|Bulutlu sabahlarda özellikle etkili.|
|$item_mace_eldner_nature|Klossen|
|$item_mace_eldner_nature_description|Darbenin kuvveti düşmanlarını yere serecek kadar güçlü değilse belki kadim kökler onları senin için yerlerinde tutabilir.|
|$item_mace_iron|Demir topuz|
|$item_mace_iron_description|Ahşap bir sapa takılı, yumruk büyüklüğünde bir demir topağı.|
|$item_mace_needle|Kirpi|
|$item_mace_needle_description|Tehditkâr çivilerle dolu olan ölümcül bir silah.|
|$item_mace_silver|Buzdöver|
|$item_mace_silver_description|Bu gümüşün neden yüreklere korku saldığını kurbanlarına hatırlat.|
|$item_mace_wood|Tahta Gürz|
|$item_mace_wood_description|İnsan birinin sıradan bir sopa yerine neden bunu yaptığını merak ediyor.|
|$item_magecap|Büyücü mantarı|
|$item_magecap_description|Zarif mavi büyücü mantarları, büyü güçlü sinir uçları gibi çatırdıyor.|
|$item_magicallystuffedmushroom|Dolu mantar|
|$item_magicallystuffedmushroom_description|Büyü aromasıyla taşıyor.|
|$item_magicallystuffedmushroomuncooked|Doldurulmuş çiğ mantar|
|$item_mandible|Altçene|
|$item_mandible_description|Daha mükemmel bir silah, insanın elinden bile zor çıkar.|
|$item_marinatedgreens|Terbiye Edilmiş Yeşillikler|
|$item_marinatedgreens_description|Acılı, sakızımsı, tatlı... Bu çılgın yemek hem dilini hem de zihnini gıdıklıyor.|
|$item_mashedmeat|Et Püresi|
|$item_mashedmeat_description|Artakalan et onu doğru bir şekilde püre hâline getirirsen gerçekten oldukça lezzetli olabilir!|
|$item_maxadrenaline|Adrenalin|
|$item_mead_bugrepellent|Sokma Karşıtı Karışım|
|$item_mead_bugrepellent_description|Ovalarda geçirilecek günler için harika bir içecek.|
|$item_mead_bzerker|Vahşi Savaşçı Şarabı|
|$item_mead_bzerker_description|İçinde zehirli bir şey karışıyor, o kadar mayalandı ki sonunda potansiyeli kullanılabilecek. Yine de canavarı serbest bırakırken dikkatli ol.|
|$item_mead_eitr_lingering|Devamlı Eitr Şarabı|
|$item_mead_eitr_lingering_description|Eitr yenilenmesini arttırır.|
|$item_mead_eitr_minor|Zayıf eitr şarabı|
|$item_mead_eitr_minor_description|Eitr yeniler.|
|$item_mead_frostres|Buz dirençli şarap|
|$item_mead_frostres_description|Soğuğa karşı korur.|
|$item_mead_hasty|Ratatosk Toniği|
|$item_mead_hasty_description|Sincap, dünya ağacının gövdesinde aşağı yukarı koşarken seri olmalı. Artık Valheim'ı ziyaret ediyor olmasa da hâlâ onu hatırlayanlar var...|
|$item_mead_hp_lingering|Devamlı İyileştirme Şarabı|
|$item_mead_hp_lingering_description|Can yenilenmesini arttırır.|
|$item_mead_hp_major|Güçlü şifa şarabı|
|$item_mead_hp_major_description|Sağlık yeniler.|
|$item_mead_hp_medium|Orta seviye şifa şarabı|
|$item_mead_hp_medium_description|Sağlık yeniler.|
|$item_mead_hp_minor|Zayıf şifa şarabı|
|$item_mead_hp_minor_description|Sağlık yeniler.|
|$item_mead_lightfoot|Çeviklik Şarabı|
|$item_mead_lightfoot_description|Şişe ağzına kadar dolu ama o kadar hafif ki sanki içinde hiçbir şey yok.|
|$item_mead_poisonres|Zehir dirençli şarap|
|$item_mead_poisonres_description|Zehre karşı güçlendirir.|
|$item_mead_stamina_lingering|Devamlı enerji şarabı|
|$item_mead_stamina_lingering_description|Enerji yenilenmesini arttırır.|
|$item_mead_stamina_medium|Orta seviye enerji şarabı|
|$item_mead_stamina_medium_description|Enerjiyi yeniler.|
|$item_mead_stamina_minor|Zayıf enerji şarabı|
|$item_mead_stamina_minor_description|Enerji yeniler.|
|$item_mead_strength|Trol Dayanıklılığı Şarabı|
|$item_mead_strength_description|Hangi yaratık bir trolden daha güçlüdür? Elinde bu içecek olan bir viking tabii ki!|
|$item_mead_swimmer|Vananidir İçkisi|
|$item_mead_swimmer_description|Bunu sadece elinde tutmak bile sana capcanlı hissettiriyor. Okyanus seni çağırıyor.|
|$item_mead_tamer|Hayvan Fısıltısı Karışımı|
|$item_mead_tamer_description|Domuz ahırı gibi kokuyor. Muhtemelen tadı da pek farklı değildir.|
|$item_mead_tasty|Leziz şarap|
|$item_mead_tasty_description|Krallar sofrasına layık olan kutsal şarap.|
|$item_mead_trollpheromones|Aşk İksiri|
|$item_mead_trollpheromones_description|Trol mağarasının yoğun kokusu, şişenin etrafındaki havaya nüfuz ediyor.|
|$item_meadbase_description|Mayalanması gerekiyor.|
|$item_meadbasebugrepellent|Şarap Özü: Sokma Karşıtı|
|$item_meadbasebzerker|Şarap Özü: Vahşi Savaşçı|
|$item_meadbaseeitr|Şarap özü: Zayıf eitr|
|$item_meadbaseeitr_lingering|Şarap Özü: Devamlı Eitr|
|$item_meadbasefrostresist|Şarap özü: Buz direnci|
|$item_meadbasehasty|Şarap Özü: Ratatosk|
|$item_meadbasehealth|Şarap özü: Zayıf iyileştirme|
|$item_meadbasehealth_lingering|Şarap Özü: Devamlı Can|
|$item_meadbasehealth_major|Şarap özü: Güçlü iyileştirme|
|$item_meadbasehealth_medium|Şarap özü: Orta seviye iyileştirme|
|$item_meadbaselightfoot|Şarap Özü: Çeviklik|
|$item_meadbasepoisonresist|Şarap özü: Zehir direnci|
|$item_meadbasestamina|Şarap özü: Zayıf enerji|
|$item_meadbasestamina_lingering|Şarap özü: Devamlı enerji|
|$item_meadbasestamina_medium|Şarap özü: Orta seviye enerji|
|$item_meadbasestrength|Şarap Özü: Trol Dayanıklılığı|
|$item_meadbaseswimmer|Şarap Özü: Vananidir|
|$item_meadbasetamer|Şarap Özü: Hayvan Fısıltısı|
|$item_meadbasetasty|Şarap özü: Lezzetli|
|$item_meat_cooked|Pişmiş et|
|$item_meat_cooked_description|Kemikte pişirilmiş, şölen sofrasına layık bir tat.|
|$item_meat_raw|Çiğ et|
|$item_meat_raw_description|Pişirerek leziz bir lokmaya dönüştürebileceğin bir çiğ et parçası.|
|$item_meat_rotten|Çürümüş et|
|$item_meat_rotten_description|Et kurtçuk kaynıyor. İğrenç kokuyor.|
|$item_meatplatter|Et tabağı|
|$item_meatplatter_description|Savaş yakıtı.|
|$item_meatplatteruncooked|Pişmemiş et tabağı|
|$item_mechanicalspring|Mekanik yay|
|$item_mechanicalspring_description|Tuzak inşa etmek üzere Cüceler tarafından inşa edilen gizemli bir mekanizma.|
|$item_mincemeatsauce|Kıymalı Sos|
|$item_mincemeatsauce_description|Yoğun et suyundaki lezzet parçaları.|
|$item_mistharesupreme|Üstün sisli yabantavşanı|
|$item_mistharesupreme_description|Hayatın büyük hazlarından birisi.|
|$item_mistharesupremeuncooked|Pişmemiş üstün sisli yabantavşanı|
|$item_moltencore|Eriyik Çekirdek|
|$item_moltencore_description|İçinde serbest bırakılmaya hazır kuvvetli bir enerji barındırıyor.|
|$item_morgenheart|Morgen Kalbi|
|$item_morgenheart_description|Önceden attığına inanmak güç.|
|$item_morgensinew|Morgen Kirişi|
|$item_morgensinew_description|Sakız gibi.|
|$item_movement_modifier|Hareket hızı|
|$item_mushroom_bzerker|Zehirli Mantar|
|$item_mushroom_bzerker_description|Kimisi ormanda bulduğun her şeyi yiyebileceğini söyler. Ama bu durum, bu mantar için geçerli değil.|
|$item_mushroomblue|Mavi mantar|
|$item_mushroomblue_description|Hafif bir mavi tonuyla parlar.|
|$item_mushroomcommon|Mantar|
|$item_mushroomcommon_description|Ormanın ganimeti.|
|$item_mushroomomelette|Mantarlı omlet|
|$item_mushroomomelette_description|Ağızda kalan topraksı tadıyla leziz bir omlet.|
|$item_mushroomyellow|Sarı mantar|
|$item_mushroomyellow_description|Enerji dolu parlayan mantar.|
|$item_necktail|Neck kuyruğu|
|$item_necktail_description|Çiğken yenmez ama pişirildiğinde oldukça lezzetli bir atıştırmalık olur.|
|$item_necktailgrilled|Kızarmış Neck kuyruğu|
|$item_necktailgrilled_description|Kömürde kızarmış bu enfes et, hafiften yosun ve ot tadı veriyor.|
|$item_needle|İğne|
|$item_needle_description|Bir Ölüm Sivrisi'nin sivri ucu|
|$item_noteleport|Geçitten geçmez|
|$item_oakseeds|Palamut|
|$item_oakseeds_description|Meşe ağacı istiyorsan toprağa ek.|
|$item_obsidian|Obsidiyen|
|$item_obsidian_description|Siyah volkanik cam.|
|$item_onehanded|Tek Elli|
|$item_onion|Soğan|
|$item_onion_description|Kıtır kıtır ve ağız yakan tat.|
|$item_onionseeds|Soğan tohumları|
|$item_onionseeds_description|Sağlıklı soğan yetiştirmek için ek.|
|$item_onionsoup|Soğan çorbası|
|$item_onionsoup_description|Zengin lezzet.|
|$item_ooze|Balçık|
|$item_ooze_description|Çürümüş ve leş gibi kokuyor. Bunu ne yapacaksın ki?|
|$item_oozebomb|Balçık bombası|
|$item_oozebomb_description|Kokusu dayanılmaz...|
|$item_parryadrenaline|Savuşturma adrenalini|
|$item_parrybonus|Savuşturma bonusu|
|$item_parts|parçalar|
|$item_pickable_barley|Arpa|
|$item_pickable_bogiron|Bataklık demiri|
|$item_pickable_branch|Dal|
|$item_pickable_hairstrands|Fenris tüyü|
|$item_pickable_meatpile|Et yığını|
|$item_pickable_meteorite|Meteor|
|$item_pickable_obsidian|Obsidiyen|
|$item_pickable_skeletalremains|İskelet kalıntıları|
|$item_pickable_sulfurrock|Kükürt Kayası|
|$item_pickaxe_antler|Boynuz kazma|
|$item_pickaxe_antler_description|Bu alet en inatçı kayaları bile parçalayabilecek kadar sert.|
|$item_pickaxe_blackmetal|Karademir kazma|
|$item_pickaxe_blackmetal_description|Parıldayan kara demirden güçlü ve iyi bir kazma.|
|$item_pickaxe_bronze|Tunç kazma|
|$item_pickaxe_bronze_description|Kaliteli tunç kazma. En sert kayaları bile parçalayabilir.|
|$item_pickaxe_iron|Demir kazma|
|$item_pickaxe_iron_description|Sertleştirilmiş demirden, sağlam bir alet.|
|$item_pickaxe_stone|Taş kazma|
|$item_pickaxe_stone_description|Taştan yapılan bir kaya parçalayıcısı.|
|$item_pinecone|Çam kozalağı|
|$item_pinecone_description|Çam ağacı istiyorsan toprağa ek.|
|$item_piquantpie|Mayhoş Turta|
|$item_piquantpie_description|Bu turtayı yapmak için biraz zaman ve çaba gerekir ama tadı buna fazlasıyla değer.|
|$item_piquantpie_uncooked|Pişmemiş Mayhoş Turta|
|$item_pot_shard_description|Kırılgan bir şeyin bir parçası.|
|$item_pot_shard_green|Çömlek Parçası|
|$item_powdereddragonegg|Toz Ejderha Yumurtası Kabuğu|
|$item_powdereddragonegg_description|Ejderha yumurtası sert ve üzerinde çalışması zor bir malzemedir. Gerçi nasıl olduysa bu ufalanıp parıltılı toz hâline getirilmiş...|
|$item_proustitepowder|Prustit Tozu|
|$item_proustitepowder_description|Bu dengesiz toz, büyük bir potansiyel barındırıyor.|
|$item_pukeberries|Öğürtlen|
|$item_pukeberries_description|Yanlış yerleştirilmiş yemekleri hızlıca kurtarmanı ve yeniden yapmanı sağlar.|
|$item_pungentpebble_description|Bu kurumuş topaklarda yoğun, doğal bir koku var. Ne olduğunu sorgulamamak en iyisi.|
|$item_pungentpebbles|Kokulu Atık|
|$item_quality|Kalite|
|$item_queen_drop|Kraliçe eşyası|
|$item_queen_drop_description|Arayıcı Kraliçe ganimet eşyası için yer tutucu.|
|$item_queenbee|Kraliçe arı|
|$item_queenbee_description|Arıların kraliçesi!|
|$item_queensjam|Ahu reçeli|
|$item_queensjam_description|Ahududu ve böğürtlenin o bildiğimiz leziz karışımı.|
|$item_raspberries|Ahududu|
|$item_raspberries_description|Leziz mi leziz.|
|$item_repairlevel|Onarım istasyonu seviyesi|
|$item_resin|Reçine|
|$item_resin_description|Güçlü yalıtıma sahip yapışkan ağaç reçinesi. Ateşle temas ettiğinde yavaş ve sürekli olarak yanar.|
|$item_roastedcrustpie|Kavrulmuş Kabuklu Turta|
|$item_roastedcrustpie_description|Bu tatlı seni gün boyu enerjik tutar.|
|$item_roastedcrustpie_uncooked|Pişmemiş Kavrulmuş Kabuklu Turta|
|$item_root|Kök|
|$item_root_description|Kadim bir ağaç çotuğunun eski bir kökü. Hem esnek hem de dayanıklı görünüyor.|
|$item_roundlog|Ağaç kütüğü|
|$item_roundlog_description|Kütükten kulübe yapmak için mükemmel.|
|$item_royaljelly|Asil jöle|
|$item_royaljelly_description|Krallara ve kraliçelere layık jöle.|
|$item_ruby|Yakut|
|$item_ruby_description|<color=yellow>Değerli Eşya</color>|
|$item_saddleasksvin|Kül Domuzu Eyeri|
|$item_saddleasksvin_description|Bir kül domuzunun sırtı biraz yumruludur. Bu yüzden birini sürmek istiyorsan bir eyere ihtiyacın olacak.|
|$item_saddlelox|Lox eyeri|
|$item_saddlelox_description|Sürmek için bir Lox'un üzerinde kullan.|
|$item_salad|Salata|
|$item_salad_description|Taze, çıtır yapraklar.|
|$item_sap|Özsu|
|$item_sap_description|Ulu Ağaç'tan gelen kutsal kan.|
|$item_sausages|Sosisler|
|$item_sausages_description|Sırayla dizilmiş iştah açan ve tütsülenmiş et.|
|$item_scalehide|Pullu deri|
|$item_scalehide_description|Parıldayan pullardan oluşan bir post.|
|$item_scorchingmedley|Yakıcı Türlü|
|$item_scorchingmedley_description|Çeşitli bir beslenme düzeni önemlidir. Öyleyse neden bu vejetaryen seçeneği denemiyorsun?|
|$item_scythe|Tırpan|
|$item_scythe_description|Kullanılan alet doğru olunca yapılacak iş de kolay olur.|
|$item_scythehandle|Tırpan Sapı|
|$item_scythehandle_description|İyi aletlerin sağlam bir temele ihtiyacı vardır.|
|$item_seekeraspic|Arayıcı etli jölesi|
|$item_seekeraspic_description|Hafif elektrik tadıyla titreyen bir jöle.|
|$item_seekerqueen_drop|Görkemli Kabuk|
|$item_seekerqueen_drop_description|Majestelerinin iradesi sağlam ve sarsılmazdır ancak bu kabuk parçası belki daha da öyledir.|
|$item_serpentmeat|Yılan eti|
|$item_serpentmeat_description|Deniz yılanından bir parça et, balık gibi kokuyor.|
|$item_serpentmeatcooked|Pişmiş yılan eti|
|$item_serpentmeatcooked_description|Deniz yılanından pişmiş bir parça et, güzel kokuyor.|
|$item_serpentscale|Yılan pulu|
|$item_serpentscale_description|Bir deniz yılanının parlak metali andıran pulu.|
|$item_serpentstew|Yılan güveci|
|$item_serpentstew_description|Bal ve yılan kokuyor...|
|$item_seteffect|Set etkisi|
|$item_sharpeningstone|Biley taşı|
|$item_sharpeningstone_description|Dönmeye hazır bir bileme taşı.|
|$item_shield_banded|Şeritli kalkan|
|$item_shield_banded_description|Demirle çevrili bu kalkan, gerçek bir savaşçının yoldaşıdır.|
|$item_shield_blackmetal|Karademir kalkan|
|$item_shield_blackmetal_description|En ölümcül kılıç darbelerini bile savuşturan, en güçlü metalden dövülmüş kalkan.|
|$item_shield_blackmetal_tower|Karademir kule kalkan|
|$item_shield_blackmetal_tower_description|Parlak karademirden yapılmış bir kule kalkan.|
|$item_shield_bonetower|Kemik kule kalkan|
|$item_shield_bonetower_description|Ölen savaşçıların kemikleri iyi koruma sağlar.|
|$item_shield_bronzebuckler|Tunç küçük kalkan|
|$item_shield_bronzebuckler_description|Hafif darbelere dayanıklı, parlak tunçtan kalkan.|
|$item_shield_carapace|Kabuk kalkan|
|$item_shield_carapace_description|Düşmanlarının neredeyse kırılamayan kabuğu müthiş bir kalkandır.|
|$item_shield_carapacebuckler|Kabuk küçük kalkan|
|$item_shield_carapacebuckler_description|Arayıcıların kafatası sağlam olup ağır olmadığı için küçük ve çevik bir kalkan için kusursuzdur.|
|$item_shield_flametal|Oddemir Kalkan|
|$item_shield_flametal_description|Kalkan, bir vikingin en önemli silahıdır ve hep de öyle olacaktır.|
|$item_shield_flametal_tower|Oddemir Kule Kalkanı|
|$item_shield_flametal_tower_description|En iyi savunma harika bir savunmadır.|
|$item_shield_iron_square|Demir kalkan|
|$item_shield_iron_square_description|Demir kılıçları kıran, savaş hattının olmazsa olmazı.|
|$item_shield_iron_tower|Demir kule kalkan|
|$item_shield_iron_tower_description|Güçlü demirden yapılan uzun bir kalkan.|
|$item_shield_ironbuckler|Demir küçük kalkan|
|$item_shield_ironbuckler_description|Hafifliği ve eğimli merkezi, saldırı savuşturmayı mükemmel kılar.|
|$item_shield_knight|Şövalye kalkanı KULLANILMAMIŞ|
|$item_shield_knight_description|Demir ile güçlendirilmiş tahta kalkan. KULLANILMAMIŞ|
|$item_shield_serpentscale|Yılan pullu kalkan|
|$item_shield_serpentscale_description|Üst üste geçen pullardan oluşan sağlam bir kalkan.|
|$item_shield_silver|Silver Shield|
|$item_shield_silver_description|Parlak gümüşten kalkan.|
|$item_shield_wood|Tahta kalkan|
|$item_shield_wood_description|Basit bir tahta kalkan.|
|$item_shield_woodtower|Tahta kule kalkan|
|$item_shield_woodtower_description|Ağır ama sağlam bir tahta kalkan.|
|$item_shieldcore|Kalkan Çekirdeği|
|$item_shieldcore_description|İçindeki koruma kuvveti serbest bırakılmaya hazır.|
|$item_shocklatesmoothie|Kirshake|
|$item_shocklatesmoothie_description|Uykuyu açar!|
|$item_silver|Gümüş|
|$item_silver_description|İşlenmeye hazır bir saf gümüş külçesi.|
|$item_silvernecklace|Gümüş Kolye|
|$item_silvernecklace_description|<color=yellow>Değerli Eşya</color>|
|$item_silverore|Gümüş madeni|
|$item_silverore_description|İşlenmemiş gümüş. Eritme fırınında işlenmesi gerekiyor.|
|$item_sizzlingberrybroth|Fokurdayan Yemiş Çorbası|
|$item_sizzlingberrybroth_description|Bu çorba neredeyse karıncalandıran bir hisle midene iniyor.|
|$item_sledge_demolisher|Yıkıcı|
|$item_sledge_demolisher_description|Bu kudretli balyoz ortalığı dağıtmak için can atıyor.|
|$item_sledge_iron|Demir balyoz|
|$item_sledge_iron_description|Kahramanlara layık olan kudretli bir çekiç.|
|$item_sledge_wood|Tahta Balyoz|
|$item_sledge_wood_description|Sanki bundan daha çok hasar vermeli.|
|$item_smokebomb|Duman Bombası|
|$item_smokebomb_description|Herkes dumanda nefes alamayacağını bilir.|
|$item_smokepuff|Duman Mantarı|
|$item_smokepuff_description|Umarım pişirdikten sonra tadı daha güzel olur.|
|$item_softtissue|Yumuşak doku|
|$item_softtissue_description|Hâlâ kadim anılarla canlılığını az da olsa koruyor.|
|$item_sparkler|Maytap|
|$item_sparkler_description|Renkli ve parlak ışıklar saçan bir çubuk. Çok güzel!|
|$item_sparklingshroomshake|Köpüklü Mantar İçeceği|
|$item_sparklingshroomshake_description|Belki güne başlamak için en iyi tat olmayabilir ancak sana ihtiyacın olan desteği sağlar.|
|$item_spear_ancientbark|Kadim kabuk mızrak|
|$item_spear_ancientbark_description|Bu mızrak, eğri büğrü görünmesine rağmen kusursuz bir dengeye sahip ve oldukça sağlamdır.|
|$item_spear_bronze|Tunç mızrak|
|$item_spear_bronze_description|Cilalanmış tunçtan bir uca sahip sağlam bir mızrak.|
|$item_spear_carapace|Kabuk mızrak|
|$item_spear_carapace_description|Kusursuz sivriliğe ulaşana kadar bilenmiş bu mızrak son derece ölümcüldür.|
|$item_spear_chitin|Derinlikler zıpkını|
|$item_spear_chitin_description|Okyanusun gazabı.|
|$item_spear_flint|Çakmak taşı mızrak|
|$item_spear_flint_description|Gözünün kestirdiği hedefe ölümü, bırak kolun göndersin.|
|$item_spear_splitner|Splitnir|
|$item_spear_splitner_blood|Kanatan Splitnir|
|$item_spear_splitner_blood_description|Her savaş için küçük bir fedakârlık yapılmalıdır...|
|$item_spear_splitner_description|Düşmanlarının kalplerini ortadan ikiye böl.|
|$item_spear_splitner_lightning|Fırtınalı Splitnir|
|$item_spear_splitner_lightning_description|Fırtına gürlemesi göğü yarsın.|
|$item_spear_splitner_nature|Kadim Splitnir|
|$item_spear_splitner_nature_description|Bu mızrağın vurduğu yerde topraktan doğanın kuvvetleri fışkırır.|
|$item_spear_wolffang|Diş mızrak|
|$item_spear_wolffang_description|Kurdun dişi ölümden sonra bile kana susuyor.|
|$item_spear_wood|Tahta Mızrak|
|$item_spear_wood_description|En az gerçek bir mızrak kadar keskin, gerçek mızrak uçları tahtadan yapılsaydı yani.|
|$item_spiceashlands|Alevli Baharat Tozu|
|$item_spiceashlands_description|Bu karışımda artık hangi baharatlar kullanıldıysa çok acı oldukları aşikâr. Baharatlar zaten yeterince acı değilmiş gibi bir de gibi toz hâline getirilmeden önce kurutulup tütsüleniyor.|
|$item_spiceforests|Ormanlık Bitki Karışımı|
|$item_spiceforests_description|Kekik ve mercanköşk özenle toplanıp kurutulduktan sonra karıştırılır. Bu karışım, en basit malzemelerden hazırlanan yemeği bile bir lezzet şölenine dönüştürür.|
|$item_spicemistlands|Saklı Tepelerin Bitkileri|
|$item_spicemistlands_description|İyi bir otacı, tam olarak nerede yabani sarımsak bulabileceğini bilir ama işinin sırrını kimseyle paylaşmaz! Yine de cebinde biraz sikke varsa özenle toplanmış bu sarımsak tomurcuğu ve yaprak karışımıyla kendini şımartabilirsin.|
|$item_spicemountains|Dağ Tepesi Toz Biberi|
|$item_spicemountains_description|Sadece en deneyimli otacılar defne ağacını bulup yapraklarını ve kabuğunu toplayabilir. Dikkatli bir şekilde hazırlandıktan sonra yemeklere eklenen bu baharat, yemeğin acısını da artırır.|
|$item_spiceoceans|Denizcinin Bitki Karışımı|
|$item_spiceoceans_description|Ekşi kuzukulağı, otacının açıklamamayı tercih ettiği bir şeyle karıştırıldığında balıklar için enfes bir yem olur. Her uzman balıkçı, balık tutma şansını artırmak için bu karışımı yanında bulundurursa iyi eder!|
|$item_spiceplains|Otacının Çayır Karışımı|
|$item_spiceplains_description|Bu karışımda onuncu dünyada nadir bulunan selam otu hakkıyla kullanılır. Kökleri, yaprakları ve tohumları toplanır, ardından olması gereken miktarlarda karıştırılır.|
|$item_spicymarmalade|Acı Marmelat|
|$item_spicymarmalade_description|Ekşi yapraklar ve turta yemişleriyle dengelenen şekerli bal.|
|$item_staff_lightning|Gümbür|
|$item_staff_lightning_description|Birazdan yaşanacaklar seni şoka uğratabilir.|
|$item_staffclusterbomb|Parçalama Asası|
|$item_staffclusterbomb_description|Bu asanın gerçek gücünü yalnızca sabrı ve odağı olanlar ortaya çıkarabilir.|
|$item_stafffireball|Köz Asası|
|$item_stafffireball_description|Muspelheim'ın cehennem sıcakları bu asanın yapabilecekleriyle karşılaştırıldığında neredeyse acınası bir hal alıyor...|
|$item_staffgreenroots|Yaban Asası|
|$item_staffgreenroots_description|Kadim doğal kuvvetler bu asanın içinde kıvrılmış ve uyku hâlinde, serbest bırakılmaya hazır bir şekilde bekliyor.|
|$item_stafficeshards|Buz Asası|
|$item_stafficeshards_description|Zamanın sonunun habercisi olacak üç yıllık kış kadar soğuk bir asa.|
|$item_stafflaser|Ayaz Asası|
|$item_stafflaser_description|Epey soğuk...|
|$item_staffredlightning|Gümbür|
|$item_staffredlightning_description|Birazdan yaşanacaklar seni şoka uğratabilir.|
|$item_staffredtroll|Trol Asası|
|$item_staffredtroll_description|Ölüm ve yıkıma sebep olması için öfkeli bir canavar çağırır.|
|$item_staffshield|Koruma Asası|
|$item_staffshield_description|Az miktarda kan kurban etme karşılığında büyücüyü, büyülü bir kabuk altında korur.|
|$item_staffskeleton|Ölü Kaldıran|
|$item_staffskeleton_description|Ölüleri kaldırmak için biraz kan kurban et. Birden fazla iskelet canlandırmak için kafatasını geliştir ve onları daha güçlü yapmak için kan büyünü yükselt.|
|$item_stagbreaker|Geyik Ezen|
|$item_stagbreaker_description|Tanrılara layık bir silah! Bununla aldığın bir darbeyi hissetmemek gibi bir şansın yok...|
|$item_staminahold|Enerji çek|
|$item_staminause|Enerji kullan|
|$item_stone|Taş|
|$item_stone_description|Bildiğimiz kaya.|
|$item_stonerock|Rock|
|$item_stonerock_description|It's a stone.|
|$item_sulfurstone|Kükürt|
|$item_sulfurstone_description|Çürük yumurta gibi kokuyor.|
|$item_surtlingcore|Alaz çekirdeği|
|$item_surtlingcore_description|İçindeki ateş inceden dışına nüksediyor.|
|$item_swimspeed_modifier|Yüzme hızı|
|$item_sword_blackmetal|Karademir kılıç|
|$item_sword_blackmetal_description|Ölüm saçan bu güzelim kılıç, yeşilimsi bir parıltıyla ışığı yakalıyor.|
|$item_sword_bronze|Tunç kılıç|
|$item_sword_bronze_description|Kana susamış bir dost.|
|$item_sword_dyrnwyn|Dyrnwyn|
|$item_sword_dyrnwyn_description|Kırık kılıç bir kez daha birleştirildi. Kadim savaşçıların anılarıyla harlanan alevleri cayır cayır yanıyor.|
|$item_sword_fire|Dyrnwyn|
|$item_sword_fire_description|Kınından çıkarıldığında tıslayarak etrafına sönmez bir ateş saçan, akıldışı bir kılıç.|
|$item_sword_iron|Demir kılıç|
|$item_sword_iron_description|Yaşam ve ölüm arasındaki ince çizgi, bu kılıcın ağzı boyunca uzanıyor.|
|$item_sword_krom|Krom|
|$item_sword_krom_description|Parlak olduğu kadar ölümcüldür de... ve çok parlaktır.|
|$item_sword_mistwalker|Sisgezgini|
|$item_sword_mistwalker_description|Sönük parıltısı sisi yarıp geçiyor.|
|$item_sword_niedhogg|Nidhögg|
|$item_sword_niedhogg_blood|Kanatan Nidhögg|
|$item_sword_niedhogg_blood_description|Kanın akıyorsa düşmanlarınınki de kesinlikle akacaktır.|
|$item_sword_niedhogg_description|Adını dünya ağacının köklerinde yaşayan kötü ejderhadan alan bu kılıç, yoluna çıkanlar için kıyameti müjdeliyor.|
|$item_sword_niedhogg_lightning|Fırtınalı Nidhögg|
|$item_sword_niedhogg_lightning_description|Gelecek acının bir belirtisi olan yıldırımın gücü bu kılıcın kenarlarında dans ediyor.|
|$item_sword_niedhogg_nature|Kadim Nidhögg|
|$item_sword_niedhogg_nature_description|Bu kılıcın isminde olduğu gibi düşmanlarını köklerle sarmala.|
|$item_sword_silver|Gümüş kılıç|
|$item_sword_silver_description|En saf metalden yapılan bu kılıcın dokunuşu karşısında kirli hiçbir şey dayanamaz.|
|$item_sword_slayer|Biçici|
|$item_sword_slayer_blood|Gaddar Biçici|
|$item_sword_slayer_blood_description|Kanıyorsa onu öldürebilirsin.|
|$item_sword_slayer_description|Bu kudretli kılıç düşmanların kanına susuyor.|
|$item_sword_slayer_lightning|Cezalandıran Biçici|
|$item_sword_slayer_lightning_description|Bu kılıca bağlanan yıldırım oldukça kararsız. Sürekli çarpacak bir şey arıyor.|
|$item_sword_slayer_nature|Kadim Biçici|
|$item_sword_slayer_nature_description|Kılıç, dünyanın kadim kuvvetleriyle uyum içinde çalışarak ölüm ve katliamı amaçlıyor.|
|$item_sword_sword_2h_description|Lütfen kullanırken dikkatli ol.|
|$item_sword_wood|Tahta Kılıç|
|$item_sword_wood_2h|Tahta Ulu Kılıç|
|$item_sword_wood_description|Genelde zararsızdır, tabii çok ısrarcı değilsen.|
|$item_tamedonly|Yalnızca ehlîleştirilmiş hayvanlara zarar verir.|
|$item_tankard|Kupa bardak|
|$item_tankard_anniversary|Kutlama Boynuzu|
|$item_tankard_anniversary_description|Geleli bir sene oluyor... Şerefe!|
|$item_tankard_description|Şerefe!|
|$item_tankard_odin|Odin'in şarap boynuzu|
|$item_tankard_odin_description|Odin'in gözde savaşçıları en kaliteli içkileri hak eder.|
|$item_tar|Katran|
|$item_tar_description|Yapışkan bir katran öbeği|
|$item_thistle|Deve dikeni|
|$item_thistle_description|Güzel ama iğnesi batabilir.|
|$item_thunderstone|Şimşek taşı|
|$item_thunderstone_description|Enerji dolu...|
|$item_tin|Kalay|
|$item_tin_description|İşlenmeye hazır bir saf kalay külçesi.|
|$item_tinore|Kalay madeni|
|$item_tinore_description|İşlenmemiş kalay. Eritme fırınında işlenmesi gerekiyor.|
|$item_torch|Meşale|
|$item_torch_description|Çevresine ısı ve ışık yayarak karanlığı kovar.|
|$item_total|Toplam|
|$item_trinket|Süs|
|$item_trinketblackdamagedealth|Demir Deri|
|$item_trinketblackdamagedealth_description|İçi şiddetle dolmuş sert bir deri.|
|$item_trinketblackdtamina|Gölge Örtüsü|
|$item_trinketblackdtamina_description|Ölümle dans eden ve düşmanın saldırılarından kaçınanlar için mükemmel.|
|$item_trinketbronzehealth|Ormanın Kalbi|
|$item_trinketbronzehealth_description|Kadim hayatın parçalarıyla çarpıyor.|
|$item_trinketbronzestamina|Bronz Tılsım|
|$item_trinketbronzestamina_description|Bir ayının dayanıklılığını temsil eden güzel bir tılsım.|
|$item_trinketcarapaceeitr|Titreşen Küpeler|
|$item_trinketcarapaceeitr_description|Eğer dikkatli dinlersen kayıp sihirli ruhların zayıf yankılarını duyabilirsin...|
|$item_trinketchitinswim|Kader Yüzgeci|
|$item_trinketchitinswim_description|Zihnini ve ruhunu boşalt, şekilsiz ol, suyla bütünleş.|
|$item_trinketflametaleitr|Jörmundcuk|
|$item_trinketflametaleitr_description|Parlayan taştan azapla kıvrananların çığlıkları duyuluyor.|
|$item_trinketflametalstaminahealth|Ateş Kalp|
|$item_trinketflametalstaminahealth_description|Kalbin çarpışında yanışını duyabiliyorsun, arzuyla yanıyor.|
|$item_trinketironhealth|Demir Broş|
|$item_trinketironhealth_description|Narin fakat savunmaya yarayan bir ekipman.|
|$item_trinketironstamina|Halhal|
|$item_trinketironstamina_description|Hafif ve hız ile doldurulmuş.|
|$item_trinketscalestaminadamage|Çınlayan Pranga|
|$item_trinketscalestaminadamage_description|Jilet gibi keskin bir ayak bileği zinciri.|
|$item_trinketsilverdamage|Kurt Bakışı|
|$item_trinketsilverdamage_description|Bir kurdun keskin ve öfkeli zihninden ilham alan bir kafa bandı.|
|$item_trinketsilverresist|Kristal Kalp|
|$item_trinketsilverresist_description|Donmuş bir keder ve çaresizlik parçası.|
|$item_trollhide|Trol derisi|
|$item_trollhide_description|Kalın ve sağlam bir deri. Trolleri öldürmek bu yüzden zordur.|
|$item_trophy_abomination|Ucube ganimeti|
|$item_trophy_abomination_description|O kadar uzun bir süredir ölüymüş ki tekrar yaşamaya başlamış.|
|$item_trophy_abomination_lore|O kadar uzun bir süredir ölüymüş ki tekrar yaşamaya başlamış.|
|$item_trophy_asksvin|Kül Domuzu Ganimeti|
|$item_trophy_asksvin_description|Dostane gülümsemesine kanma çünkü kolunu kolayca ısırıp koparabilir.|
|$item_trophy_asksvin_lore|Kudretli bir yaratık ancak artık etrafta daha fazla gezemeyecek.|
|$item_trophy_bjorn|Ayı Başı|
|$item_trophy_bjorn_description|Bakışı hâlâ korkutucu...|
|$item_trophy_bjorn_lore|Ormanın efendisinin kendisi olduğu yanılgısına sahipti.|
|$item_trophy_bjorn_undead|Menfur Ganimeti|
|$item_trophy_bjorn_undead_description|O bakış artık yok...|
|$item_trophy_bjorn_undead_lore|Gecenin en korkulu gölgesi.|
|$item_trophy_blob|Vıcık avı ganimeti|
|$item_trophy_blob_description|Vıcık vıcık bir maddeden ibaret pis kokulu bir yumru.|
|$item_trophy_blob_lore|Vıcık vıcık bir maddeden ibaret pis kokulu bir yumru.|
|$item_trophy_boar|Yaban domuzu başı|
|$item_trophy_boar_description|Bu domuz başı her türlü ev için harika bir dekorasyon malzemesi olur.|
|$item_trophy_boar_lore|Bu domuz başı her türlü ev için harika bir dekorasyon malzemesi olur.|
|$item_trophy_bonemass|Kemik Kütlesi avı ganimeti|
|$item_trophy_bonemass_description|Görünmeyen bir güç tarafından bir arada tutulan kemik ve yapışkan madde yığını.<br/><br/>Onu Kurban Taşları'na adak olarak sun.|
|$item_trophy_bonemass_lore|Kara topraktan ve eski kemiklerden ibaret bir yığın. Neyse ki artık sessiz.|
|$item_trophy_bonemaw|Kemikçene Ganimeti|
|$item_trophy_bonemaw_description|Yoğun kemikten oluşan, koruyucu olduğu kadar tehlikeli de olan bir kafatası.|
|$item_trophy_bonemaw_lore|Bu sivri dişler, canavar kendi sonuna ulaşana kadar birçok denizcinin ölümüne sebep olmuştu.|
|$item_trophy_brutebro|Thungr başı|
|$item_trophy_brutebro_description|Artık o kadar da çetin değil.|
|$item_trophy_brutebro_lore|İki kardeşten küçük olanı. Kardeşinin büyülerini kıskanıyor.|
|$item_trophy_charredarcher|Nişancı Ganimeti|
|$item_trophy_charredarcher_description|Bu bacaklar sonsuz güç barındırabilir.|
|$item_trophy_charredarcher_lore|Bu asker artık ilerlemiyor.|
|$item_trophy_charredmage|Lanet Ustası Ganimeti|
|$item_trophy_charredmage_description|İçindeki sıcak parıltı neredeyse dışarı yayılacakmış gibi görünüyor. Dikkatli tut.|
|$item_trophy_charredmage_lore|Dikkatli dinlersen sönük büyü fısıltısını hâlâ duyabilirsin.|
|$item_trophy_charredmelee|Savaşçı Ganimeti|
|$item_trophy_charredmelee_description|Bu kafatası sanki seneler boyunca birçok darbe almış gibi kırıklarla kaplı.|
|$item_trophy_charredmelee_lore|Ölüyken bile bakışları amansız.|
|$item_trophy_cultist|Tarikatçı ganimeti|
|$item_trophy_cultist_description|Gözündeki ateş hâlâ sönmemiş.|
|$item_trophy_cultist_hildir|Geirrhafa başı|
|$item_trophy_cultist_hildir_description|Sana buz gibi bir bakış atıyor.|
|$item_trophy_cultist_hildir_lore|Beyaz ayaza karşı direnmek yerine bütünleşmeyi seçtiği için soydaşlarından ayrı kaldı.|
|$item_trophy_cultist_lore|Gözündeki ateş hâlâ sönmemiş.|
|$item_trophy_deathsquito|Ölüm Sivrisi başı|
|$item_trophy_deathsquito_description|Bu şeye ölü haldeyken bile dokunmak hoşuna gitmiyor.|
|$item_trophy_deathsquito_lore|Bu şeye ölü haldeyken bile dokunmak hoşuna gitmiyor.|
|$item_trophy_deer|Geyik başı|
|$item_trophy_deer_description|Avdaki hünerlerine iyi bir örnek olsa da Valhalla'ya girmek için geyikten fazlasını öldürmen gerekecek.|
|$item_trophy_deer_lore|Avdaki hünerlerine iyi bir örnek olsa da Valhalla'ya girmek için geyikten fazlasını öldürmen gerekecek.|
|$item_trophy_dragonqueen|Moder avı ganimeti|
|$item_trophy_dragonqueen_description|Ölümden sonra bile haşmetini koruyan bir ejderha başı.<br/><br/>Onu Kurban Taşları'na adak olarak sun.|
|$item_trophy_dragonqueen_lore|Kadim yara izleriyle kaplı bu baş, sayısız savaşçının gördüğü son şey oldu.|
|$item_trophy_draugr|Draugr başı|
|$item_trophy_draugr_description|Geceleyin fısıldamaya başlarsa ağzını bağla...|
|$item_trophy_draugr_lore|Geceleyin fısıldamaya başlarsa ağzını bağla...|
|$item_trophy_draugrelite|Seçkin Draugr başı|
|$item_trophy_draugrelite_description|Parlayan kızıl gözlerdeki o ölü bakış, tüylerinin ürpermesine sebep oluyor.|
|$item_trophy_draugrelite_lore|Parlayan kızıl gözlerdeki o ölü bakış, tüylerinin ürpermesine sebep oluyor.|
|$item_trophy_dvergr|Cüce başı|
|$item_trophy_dvergr_description|Açıkçası bunları duvara asmayı düşünmek bile biraz tedirgin edici...|
|$item_trophy_dvergr_lore|Ne kadar güzel örgüler!|
|$item_trophy_eikthyr|Eikthyr başı|
|$item_trophy_eikthyr_description|Bu kesik baştan güç sızıyor.<br/><br/>Onu Kurban Taşları'na adak olarak sun.|
|$item_trophy_eikthyr_lore|Bir zamanlar ormanın kralı olan bu yaratığın boynuzlarının boyutu gerçekten muazzam.|
|$item_trophy_elder|Kadim Olan'ın başı|
|$item_trophy_elder_description|Bu kesik baştan güç sızıyor.<br/><br/>Onu Kurban Taşları'na adak olarak sun.|
|$item_trophy_elder_lore|Ölüyken bile taşıdığı gizli irfanla sırıtıyor gibi görünüyor.|
|$item_trophy_fader|Sönümleyici Ganimeti|
|$item_trophy_fader_description|Kurtarılamayacak kadar yozlaşmış yeşil ejderha.<br/><br/>Onu adak taşlarına sun.|
|$item_trophy_fader_lore|Sağlıklı zihninden geriye yalnızca parçalar kalmış. Belki de onu öldürmek bir merhametti.|
|$item_trophy_fallenvalkyrie|Düşmüş Valkür Ganimeti|
|$item_trophy_fallenvalkyrie_description|Ölü olmasına rağmen kanın akmasını arzuluyor.|
|$item_trophy_fallenvalkyrie_lore|Bir dehşet alametine dönüştürülmeden önce bir zamanlar hiddetli ve asil bir varlıktı.|
|$item_trophy_fenring|Fenring başı|
|$item_trophy_fenring_description|Yarı kurt yarı insan olan bu yaratığın suratında acı dolu bir hırlayış donup kalmış.|
|$item_trophy_fenring_lore|Yarı kurt yarı insan olan bu yaratığın suratında acı dolu bir hırlayış donup kalmış.|
|$item_trophy_foresttroll|Orman Trolü başı|
|$item_trophy_foresttroll_description|Trolün kösele gibi derisi, kadim sembollerin solmuş izlerini taşıyor.|
|$item_trophy_foresttroll_lore|Trolün kösele gibi derisi, kadim sembollerin solmuş izlerini taşıyor.|
|$item_trophy_frosttroll|Buz Trolü başı|
|$item_trophy_frosttroll_description|Gözlerinin içindeki gümüş renkli benekler hâlâ parıldayıp fırıl fırıl dönüyor. Uzun süre bakma...|
|$item_trophy_frosttroll_lore|Gözlerinin içindeki gümüş renkli benekler hâlâ parıldayıp fırıl fırıl dönüyor. Uzun süre bakma...|
|$item_trophy_ghost|Hayalet Ganimeti|
|$item_trophy_ghost_description|Hâlâ yarım kalmış işler hakkında fısıldıyor mu?|
|$item_trophy_ghost_lore|Hâlâ rahatsız edici bir his uyandırıyor...|
|$item_trophy_gjall|Gjall başı|
|$item_trophy_gjall_description|Uçup gitmez herhalde.|
|$item_trophy_gjall_lore|Uçup gitmez herhalde.|
|$item_trophy_goblin|Fuling başı|
|$item_trophy_goblin_description|Gevşek ve yeşil deri katmanları bir çift kara ve nefret dolu gözün çevresinde toplanmış.|
|$item_trophy_goblin_lore|Gevşek ve yeşil deri katmanları bir çift kara ve nefret dolu gözün çevresinde toplanmış.|
|$item_trophy_goblinbrute|Azman Fuling başı|
|$item_trophy_goblinbrute_description|Bu koca boz kafa, bir kaya kadar ağır.|
|$item_trophy_goblinbrute_lore|Bu koca boz kafa, bir kaya kadar ağır.|
|$item_trophy_goblinking|Yagluth'un başı|
|$item_trophy_goblinking_description|Ölü bir kralın taçsız başı.|
|$item_trophy_goblinking_lore|Kimi zaman gözleri hâlâ kıpırdayıp dudakları sessizce bir şeyler fısıldıyor...|
|$item_trophy_goblinshaman|Şaman Fuling başı|
|$item_trophy_goblinshaman_description|Bu yaratığı öldürmek bile çarpık dudaklarındaki bilgiç sırıtışı silmeye yetmedi...|
|$item_trophy_goblinshaman_lore|Bu yaratığı öldürmek bile çarpık dudaklarındaki bilgiç sırıtışı silmeye yetmedi...|
|$item_trophy_greydwarf|Bozcüce başı|
|$item_trophy_greydwarf_description|Bir Bozcüce'nin yosunlu kesik başı.|
|$item_trophy_greydwarf_lore|Bir Bozcüce'nin yosunlu kesik başı.|
|$item_trophy_greydwarfbrute|Azman Bozcüce başı|
|$item_trophy_greydwarfbrute_description|Bu boğumlu başı gövdesinden ayırmak için yedi darbe indirmen gerekti.|
|$item_trophy_greydwarfbrute_lore|Bu boğumlu başı gövdesinden ayırmak için yedi darbe indirmen gerekti.|
|$item_trophy_greydwarfshaman|Şaman Bozcüce başı|
|$item_trophy_greydwarfshaman_description|Geri dönmeye kalkabilir, bu yüzden yeni filizleri budamayı ihmal etme...|
|$item_trophy_greydwarfshaman_lore|Geri dönmeye kalkabilir, bu yüzden yeni filizleri budamayı ihmal etme...|
|$item_trophy_growth|Öbek ganimeti|
|$item_trophy_growth_description|Kara ve yapışkan bir kütle.|
|$item_trophy_growth_lore|Kara ve yapışkan bir kütle.|
|$item_trophy_hare|Yabantavşanı başı|
|$item_trophy_hare_description|Bunlar şans getirirmiş. Ama asıl sahibine değil tabii.|
|$item_trophy_hare_lore|Bunlar şans getirirmiş. Ama asıl sahibine değil tabii.|
|$item_trophy_hatchling|Ejder başı|
|$item_trophy_hatchling_description|Derisi hâla soğuk.|
|$item_trophy_hatchling_lore|Derisi hâla soğuk.|
|$item_trophy_leech|Sülük avı ganimeti|
|$item_trophy_leech_description|Deri yapış yapış olsa da göz alıcı kırmızı siyah desenlere sahip.|
|$item_trophy_leech_lore|Deri yapış yapış olsa da göz alıcı kırmızı siyah desenlere sahip.|
|$item_trophy_lox|Lox başı|
|$item_trophy_lox_description|Dev bir yaratığın kalın kürkle kaplı başı.|
|$item_trophy_lox_lore|Dev bir yaratığın kalın kürkle kaplı başı.|
|$item_trophy_morgen|Morgen Ganimeti|
|$item_trophy_morgen_description|Yaşayan kâbus sonuna ulaştı.|
|$item_trophy_morgen_lore|Böyle bir yaratığı yalnızca en çarpık zihinler çağırabilirdi.|
|$item_trophy_neck|Neck başı|
|$item_trophy_neck_description|Parlak gözler ve ustura keskinliğindeki dişler, bu küçük kertenkelenin sakin görünen tabiatıyla zıt düşüyor.|
|$item_trophy_neck_lore|Parlak gözler ve ustura keskinliğindeki dişler, bu küçük kertenkelenin sakin görünen tabiatıyla zıt düşüyor.|
|$item_trophy_seeker|Arayıcı başı|
|$item_trophy_seeker_brute|Arayıcı asker başı|
|$item_trophy_seeker_brute_description|Düşmüş bir şampiyonun kafası.|
|$item_trophy_seeker_brute_lore|Düşmüş bir şampiyonun kafası.|
|$item_trophy_seeker_description|Göründüğü kadar narin değil. Kanatlardaki deri uçmayı hatırlarcasına alev ışığını yakalıyor.|
|$item_trophy_seeker_lore|Göründüğü kadar narin değil. Kanatlardaki deri uçmayı hatırlarcasına alev ışığını yakalıyor.|
|$item_trophy_seekerqueen|Kraliçe başı|
|$item_trophy_seekerqueen_description|Yeterince arayış yaptı.|
|$item_trophy_seekerqueen_lore|Yeterince arayış yaptı.|
|$item_trophy_serpent|Deniz yılanı başı|
|$item_trophy_serpent_description|Pullar parıltısını yitirse de gözler hâlâ ışıl ışıl.|
|$item_trophy_serpent_lore|Pullar parıltısını yitirse de gözler hâlâ ışıl ışıl.|
|$item_trophy_sgolem|Taş Dev başı|
|$item_trophy_sgolem_description|Bu kristalize kaya oluşumundan harika bir zemin dekorasyonu olur.|
|$item_trophy_sgolem_lore|Bu kristalize kaya oluşumundan harika bir zemin dekorasyonu olur.|
|$item_trophy_shamanbro|Zil başı|
|$item_trophy_shamanbro_description|Ölümüne sadık ol lafını çok ciddiye aldı.|
|$item_trophy_shamanbro_lore|Büyük olan kardeş. Kardeşinin kuvvetine sahip olabilmeyi dilerdi.|
|$item_trophy_skeleton|İskelet avı ganimeti|
|$item_trophy_skeleton_description|Kurukafanın cansız sırıtışı sana ölümün kaçınılmazlığını hatırlatıyor.|
|$item_trophy_skeleton_hildir|Brenna başı|
|$item_trophy_skeleton_hildir_description|Bir şekilde hâlâ yanıyor.|
|$item_trophy_skeleton_hildir_lore|Öfkesi o kadar alevliydi ki ölümünden sonra bile sönmedi.|
|$item_trophy_skeleton_lore|Kurukafanın cansız sırıtışı sana ölümün kaçınılmazlığını hatırlatıyor.|
|$item_trophy_skeletonpoison|Zehirli İskelet avı ganimeti|
|$item_trophy_skeletonpoison_description|Kokuşmuş ve çürümüş bir kurukafa. Ne diye yanına aldığını bilmiyorsun...|
|$item_trophy_skeletonpoison_lore|Kokuşmuş ve çürümüş bir kurukafa. Ne diye yanına aldığını bilmiyorsun...|
|$item_trophy_surtling|Alaz avı ganimeti|
|$item_trophy_surtling_description|Solgun alevlerin çelenk gibi sardığı bu av ganimeti, bir köz gibi hâlâ için için yanıyor.|
|$item_trophy_surtling_lore|Solgun alevlerin çelenk gibi sardığı bu av ganimeti, bir köz gibi hâlâ için için yanıyor.|
|$item_trophy_tick|Kene başı|
|$item_trophy_tick_description|Bundan sohbet malzemesi olur...|
|$item_trophy_tick_lore|Bundan sohbet malzemesi olur...|
|$item_trophy_troll|Trol başı|
|$item_trophy_troll_description|Trolün kösele gibi derisi, kadim sembollerin solmuş izlerini taşıyor.|
|$item_trophy_troll_lore|Trolün kösele gibi derisi, kadim sembollerin solmuş izlerini taşıyor.|
|$item_trophy_ulv|Ulv ganimeti|
|$item_trophy_ulv_description|Pek de uslu olmayan bir oğlanın sert kuyruğu.|
|$item_trophy_ulv_lore|Pek de uslu olmayan bir oğlanın sert kuyruğu.|
|$item_trophy_volture|Külbaba Ganimeti|
|$item_trophy_volture_description|Bir akbaba gibi ancak volkanik iklimlerde yaşıyor.|
|$item_trophy_volture_lore|Dikkatli gözleri hâlâ ziyafeti çekilecek cesetler arıyor gibi görünüyor.|
|$item_trophy_wolf|Kurt başı|
|$item_trophy_wolf_description|Ölümün katılığı üzerine çökmüş olsa da postu kanla matlaşmış ve boğazına sessiz bir uluma takılıp kalmış.|
|$item_trophy_wolf_lore|Ölümün katılığı üzerine çökmüş olsa da postu kanla matlaşmış ve boğazına sessiz bir uluma takılıp kalmış.|
|$item_trophy_wraith|Heyula avı ganimeti|
|$item_trophy_wraith_description|Bir heyulanın dökülmüş derisi, yalnızca ay ışığında görülebilen ve uçuşan bir cüppe.|
|$item_trophy_wraith_lore|Bir heyulanın dökülmüş derisi, yalnızca ay ışığında görülebilen ve uçuşan bir cüppe.|
|$item_turnip|Şalgam|
|$item_turnip_description|Bir şalgam.|
|$item_turnipseeds|Şalgam tohumları|
|$item_turnipseeds_description|Sağlıklı şalgam yetiştirmek için ek.|
|$item_turnipstew|Şalgam güveci|
|$item_turnipstew_description|Besleyici ve şifa dolu.|
|$item_turretbolt|Karademir balista oku|
|$item_turretbolt_description|Bu kalın balista okları en güçlü düşmanların postunu bile delip geçer.|
|$item_turretbolt_flametal|Oddemir Balista Oku|
|$item_turretbolt_flametal_description|Valheim'ın tamamındaki en sert metallerden birinden dövülen bu balista oku düşmanlarını kesinlikle uzak tutacaktır.|
|$item_turretboltwood|Tahta balista oku|
|$item_turretboltwood_description|Düşmanlara karşı sıkı bir savunma sağlayan sağlam, tahta balista okları.|
|$item_twohanded|Çift Elli|
|$item_UndeadBjornRibcage|Menfur Kaburgası|
|$item_UndeadBjornRibcage_description|Vahşi gücün bir yadigârı, yabanın öfkesini taşıyan zırhlar dövmek için mükemmel.|
|$item_utility|Yardımcı eşya|
|$item_value|Değer|
|$item_vikingcupcake|Dondurulmuş Tatlı Ekmek|
|$item_vikingcupcake_description|Başarıları kutlamak için yenilen tatlı ve leziz bir ikram!|
|$item_vikingcupcake_uncooked|Pişmemiş Tatlı Ekmek|
|$item_vikingcupcake_uncooked_description|Fırınlanmaya hazır.|
|$item_vineberry|Asma Yemişi Demeti|
|$item_vineberry_description|Bu sulu yemişler hem ekşi hem de tatlı.|
|$item_vineberryseeds|Kül Asması Tohumları|
|$item_vineberryseeds_description|Her an ufalanabileceklermiş gibi görünüyorlar.|
|$item_vinegreenseeds|Sarmaşık Tohumları|
|$item_vinegreenseeds_description|Bu küçük tohumlar, büyüyüp koca binaları saran sarmaşıklara dönüşebilir.|
|$item_volture_meat|Külbaba Eti|
|$item_volture_meat_cooked|Pişmiş Külbaba Eti|
|$item_volture_meat_cooked_description|Sakızımsı ve biraz kuru bir et. Biraz baharat muhtemelen tat katar ancak her şekilde mideni tutacaktır.|
|$item_volture_meat_description|Bu kuşun her türlü şeyle beslendiği düşünülünce yemeyi aklına bile getirmeden önce etini pişirsen iyi edersin.|
|$item_voltureegg|Külbaba Yumurtası|
|$item_voltureegg_description|Dokununca sıcak ve protein dolu.|
|$item_weight|Ağırlık|
|$item_wishbone|Lades kemiği|
|$item_wishbone_description|Bu antik kemik, unutulan pek çok şeyin yerini hatırlıyor.|
|$item_wisp|Peri|
|$item_wisp_description|Saçma şeyler fısıldayıp duruyor...|
|$item_witheredbone|Solgun kemik|
|$item_witheredbone_description|Yıllanmış odun misali iç içe geçen dev bir kemik.|
|$item_wolf_meat|Kurt eti|
|$item_wolf_meat_cooked|Pişmiş kurt eti|
|$item_wolf_meat_cooked_description|Yırtıcı bir lezzet.|
|$item_wolf_meat_description||
|$item_wolf_skewer|Kurt şiş|
|$item_wolf_skewer_description|Lezzet akıyor.|
|$item_wolfclaw|Fenris pençesi|
|$item_wolfclaw_description|Demir gibi sert ve keskin.|
|$item_wolffang|Kurt dişi|
|$item_wolffang_description|Hâlâ keskin.|
|$item_wolfhairbundle|Fenris tüyü|
|$item_wolfhairbundle_description|Kalın, sert bir tüy yumağı. Güçlü bir kokusu var.|
|$item_wolfjerky|Kurutulmuş kurt eti|
|$item_wolfjerky_description|Sert ve lezzet dolu.|
|$item_wolfpelt|Kurt postu|
|$item_wolfpelt_description|Kürkten tüylü bir post.|
|$item_wood|Odun|
|$item_wood_description|İnşaatta kullanılabilecek iyi ve güçlü bir ahşap.|
|$item_yagluththing|Parçalanmış ruh|
|$item_yagluththing_description|Bu ve sonraki dünyanın arasında bölünmüş çarpık bir ruh olan Yagluth'un kalıntıları.|
|$item_yggdrasilporridge|Yggdrasil lapası|
|$item_yggdrasilporridge_description|Ulu Ağaç'ın özsuyuyla yapılır ve bir lokması bile bütün vücuduna sıcak bir parıltı verir.|
|$item_yggdrasilwood|Yggdrasil odunu|
|$item_yggdrasilwood_description|Tanrıeti, Ulu Ağaç'ın odunu.|
|$item_ymirremains|Ymir Eti|
|$item_ymirremains_description|Dev Ymir'in dünyevi kalıntıları.|
|$keyboard_ChatInput|Mesaj gir|
|$keyboard_FilterField|Filtre gir|
|$language_abenaki|Alnôbaôdwawôgan*|
|$language_bulgarian|български*|
|$language_chinese|简体中文|
|$language_chinese_trad|繁體中文|
|$language_croatian|Croatian*|
|$language_czech|Čeština*|
|$language_danish|Dansk*|
|$language_dutch|Nederlands|
|$language_english|English|
|$language_estonian|Estonian*|
|$language_finnish|Suomi*|
|$language_french|Français|
|$language_georgian|ქართული*|
|$language_german|Deutsch|
|$language_greek|Ελληνικά*|
|$language_hindi|हिन्दी*|
|$language_hungarian|Magyar*|
|$language_icelandic|íslenska*|
|$language_italian|Italiano*|
|$language_japanese|日本語|
|$language_korean|한국어|
|$language_latvian|Latviski*|
|$language_lithuanian|Lietuvių kalba*|
|$language_macedonian|македонски*|
|$language_norwegian|Norsk*|
|$language_polish|Polski|
|$language_portuguese_brazilian|Português (Brasil)|
|$language_portuguese_european|Português europeu*|
|$language_romanian|Romanian*|
|$language_russian|Русский|
|$language_serbian|Serbian*|
|$language_slovak|Slovenčina*|
|$language_spanish|Español|
|$language_swedish|Svenska*|
|$language_thai|ไทย*|
|$language_turkish|Türkçe|
|$language_ukrainian|українська|
|$loading_healthwarning|Çok az sayıda insan, ışık örüntüleri veya yanıp sönen ışıklar gibi video oyunlarında veya diğer video içeriklerinde bulunan belirli görüntülere maruz kaldığında epilepsi nöbeti yaşayabilir. Bilgisayar ekranında veya video oyunu oynarken belirli örüntülere veya arka planlara maruz kalmak bu kişilerde epilepsi nöbetine yol açabilir. Belirli koşullar, daha önce nöbet veya epilepsi geçmişi olmayan kişilerde bile daha önce tespit edilmemiş epilepsi semptomlarına sebep olabilir.|
|$loading_healthwarning2|Sende veya ailendeki birinde epilepsi semptomları bulunuyorsa Valheim oynamadan önce doktoruna danış. Oynarken baş dönmesi, görme bozukluğu, göz veya kas seğirmeleri, bilinç kaybı, oryantasyon bozukluğu, istemsiz herhangi bir hareket veya kasılma semptomlarından herhangi birini yaşarsan DERHAL oynamayı bırakıp tekrar Valheim oynamaya devam etmeden önce doktoruna danış.|
|$loading_healthwarningtitle|IŞIĞADUYARLILIK NÖBETİ UYARISI: OYNAMADAN ÖNCE OKU|
|$loading_savenotification|Bu oyunda otomatik kayıt özelliği vardır. Lütfen otomatik kayıt simgesini gördüğünde oyunu veya konsolu kapatma.|
|$loadscreen_tip01|Atölyelerini, silahlarını ve zırhını geliştirmeyi unutmayın.|
|$loadscreen_tip02|Beslenmek sağlığını ve enerjini arttırır. Üçlü yiyecek bölmeni bunu göz önünde bulundurarak doldur.|
|$loadscreen_tip03|Geliştirilmiş atölyeler, yeni üretim imkanlarını açıp fazladan ekipman yükseltmelerine olanak tanırlar.|
|$loadscreen_tip04|Dört adet ana üretim türü vardır: Ağaç işçiliği, taş işçiliği, demir işçiliği ve aşçılık.|
|$loadscreen_tip05|Düşmanlar, oyuncu üretimi yapıların yakınında canlanmazlar (atölyeler, kamp ateşleri vb.)|
|$loadscreen_tip06|Teknede seyahat etmenin en güvenli yöntemi oturmaktır.|
|$loadscreen_tip07|Valheim'e gönderildiğin anda taş ve dal bularak ilk silahını yapmaya bak.|
|$loadscreen_tip08|Geceleri dışarıda dolaşmak genellikle pek iyi bir fikir değildir...|
|$loadscreen_tip09|Menzilli silahla geyik avlamak, iyi bir et ve deri kaynağıdır.|
|$loadscreen_tip10|Yapacağın ilk iş ilkel bir sığınak kurmak olsun!|
|$loadscreen_tip11|Düşman türleri farklı hasar türlerine göre zayıflık gösterir. Örneğin, Bozcüceler ateşi sevmezler.|
|$loadscreen_tip12|Bir yatak yaptığında yatağı sahiplenmeyi unutma.|
|$loadscreen_tip13|Çömelirken bıçak ve yayla saldırmak büyük oranda hasar bonusu sağlar.|
|$loadscreen_tip14|Kuşlar önemli bir tüy kaynağıdır. Özellikle havanın kötü olduğu zamanlarda yerde daha fazlasını bulabilirsin.|
|$loadscreen_tip15|Kuzgun Hugin işine yarayacak ipuçları verebilir.|
|$loadscreen_tip16|Bazı kaynaklar üst düzey ekipmana ihtiyaç duyar.|
|$loadscreen_tip17|Dinlendiğinde enerji ve sağlık yenilenmen artar.|
|$loadscreen_tip18|Hasar verirken sarı numaralar bir zayıflığa işaret ederken, gri sayılar direnci gösterir.|
|$loadscreen_tip19|Çapayı kullanarak zemini düzleyebilir ve böylece yapıları daha rahat inşa edebilirsin.|
|$loadscreen_tip20|Ağır eşyaları uzun mesafeler arasında taşımak için bir el arabası inşa et.|
|$loadscreen_tip21|Tekneleri, kürekle yön vererek kontrol edebilirsin.|
|$loadscreen_tip22|Kalay ve bakır, Kara Orman'da bulunabilir.|
|$loadscreen_tip23|Sağ tıklayarak harita işaretlerini türlere göre filtreleyebilirsin. (DPad Right)|
|$loadscreen_tip24|Sağ tıklayarak harita işaretlerini türlere göre filtreleyebilirsin. (DPad Right)|
|$loadscreen_tip25|Yay kullanırken herhangi bir ok elle seçilmediyse oklar otomatik olarak en sol sütundan sağa doğru sırayla seçilir.|
|$location_dvergrboss|İstila edilmiş hisar|
|$location_dvergrtown|İstila edilmiş maden|
|$location_enter|Gir|
|$location_exit|Çık|
|$location_forestcave|Trol mağarası|
|$location_forestcrypt|Defin odaları|
|$location_mausoleum|Lord Reto'nun Mezarı|
|$location_morgenhole|Kokuşmuş Delik|
|$location_mountaincave|Buz Mağaraları|
|$location_sunkencrypt|Batık mezarlar|
|$lore_ashlands_random01|Doğmamış çocuk, şunu bil ki burası bir zamanlar tüm Valheim'daki en büyük krallıktı. Burada Zümrüt Alev'in Kralı'nın oğulları ve kızları yaşardı. Onun kanatları altında gücü ve güvenliği biliyorduk. Onun gözetiminde fidanlar gibi yetiştik.<br/><br/>Şimdi bu taşı harabelerin arasına dikip doğduğumuz toprağa sırtımızı dönüyoruz. Gelecek nesiller için ibret olalım.|
|$lore_ashlands_random02|Kuzey uçtaki ormanlarda gözcülerimiz, derin karın içinde sırtı bir ağaca dayanmış çıplak bir şekilde uyuyan bir oğlana rastladı. Onu kürklerle sarıp krala getirdiler. Oğlan tüm bunlar olurken yalnızca göz kapakları hareket ederek uyumaya devam etti.<br/><br/>Ona Kış Çocuğu adını verdik ve onu bir iyilik alameti olarak kabul ettik. Tanrılar ahmaklara merhamet etsin.|
|$lore_ashlands_random03|İkisinin sık sık sohbet ediyormuş gibi birlikte oturduğu, kralın uyuyan çocuğun başında eğilip yüzünü ona yaklaştırarak mırıldanabileceği kelimeleri yakalamak için kendini zorladığı tüm bölgede biliniyordu.<br/><br/>Aralarında ne olduğunu kimse bilmiyordu ancak kraldaki değişimi herkes görüyordu. Adalet alevlenerek hiddete dönüştü ve temkin derinleşerek korku hâline geldi. Çiğnenmesi gereken kanunlar uygulandı ve kanunları çiğneyenler her seferinde daha ağır şekilde cezalandırıldı.<br/><br/>Tiranlık, büyük bir fırtına gibi bölgeyi sardı.|
|$lore_ashlands_random04|Zümrüt Alev'in Kralı, Fulings'in büyücü kralını ziyaret etmek için güneye uçtuğunda kendi baronları başkaldırıp Kış Çocuğu içindeyken sarayını ateşe verdi. O vakitte hayatta olan hepimiz yangının dokuz günü boyunca duyulan ve şehrin sokaklarında haftalarca daha yankılanan çığlığı duyduk.<br/><br/>Onlar, kralımız geri dönmeden önce hepimizin bildiği son güzel günlerdi.|
|$lore_ashlands_random05|Kral, büyük salonunun yanmış kalıntılarını gördüğünde düşmanlarına tutuşturdukları küçük kıvılcımdan çok daha büyük bir yangın göstereceğine yemin etti. Nefesiyle şehrimizi yerle bir etti ve bölgenin tamamına savaş getirdi. Halkın bir kısmı krala sadık kaldı ancak çoğu kralın öfkesini yatıştırmak istiyordu.<br/><br/>Kardeşi kardeşe kırdırdılar. Zümrüt Alev Krallığı kendini parçaladı. Şimdi geriye yalnızca ölülerin bile içinde rahat bulamadığı bu yanmış kalıntılar kaldı.|
|$lore_ashlands_random06|Taşlar bu bölgede yüzyıllar önce yaşanan büyük bir trajediyi anlatıyor. Bu harap şehirde kol gezen yaratığın bir zamanlar asil bir canavar ve bilge bir kral olduğuna inanmak güç. Yine de bu sebepten ötürü kendimizi tutmayacağız. Onu öldürdüğümüzde bunu bir merhamet sayacağız.|
|$lore_ashlands_random07|Buraya geldiğimden beri herhangi bir yerde bir günden uzun süre mutlu olmadım ancak burası hepsi arasında en kötüsü. Çoğu şey yanıyor ve balıkçılık iyi değil. Bunlar Ulf'un sözleri.|
|$lore_ashlands_random08|Büyük ejderlerin saf alevi can verip ölü toprağı diriltme gücüne sahiptir. Bir zamanlar toprağı ve denizleri arındıran alevleriyle kutsayarak geçtikleri her yere yeni yaşam getirirlerdi.<br/><br/>Bir ejderin aleviyle yananlar için ise kutsama bir lanettir. Aynı anda hem öldürülüp hem diriltilerek yaşam ve ölüm arasında sonsuza dek sıkışırlar. Bu mahvolmuş bölgede gezip vücut yerine alevlere sahip olan ve her anlarında ızdırap çeken talihsiz ruhlar böyleleridir. Onlar yok edilemez. Onlara yalnızca acınabilir.|
|$lore_ashlands_random09|Hikâyeler doğruysa tanrılar bu dünyayı uzun süre önce terk etti. Tanrıya en yakın Zümrüt Alev'in Kralı'nı bilirdik ancak ona ettiğimiz dualar ağzımızda küllere dönüştü.<br/><br/>Burası son kalemiz ve gün doğduğunda onu savunarak öleceğiz. Daha fazla dua etmiyoruz ve yalnızca temiz, alev tarafından yakılmamış özgür ölümler diliyoruz.|
|$lore_ashlands_random10|Özgür olanların tanrılara ihtiyaçları olmadığını söylemeleri ahmakça. Kimsenin tanrılara ihtiyacı yok ve tanrıların da kimseye. İnsanlığa hiçbir şey borçlu değiller.<br/><br/>Yine de bilge olanlar, asmaların tırmanacak kudretli bir ağaç aradığı gibi güçlü olanları arayacak. Kılıçlarımızı Zümrüt Alev'in Kralı'na adıyoruz çünkü o yoksa yalnızca ölüm vardır.|
|$lore_ashlands_random11|Savaş bir yangındır,<br/>İnsanlar ise yakıt.<br/><br/>Yürek alevi harlandığında,<br/>Bedenleri kuru odundur.|
|$lore_ashlands_random12|Ben Uzun Kollu Astrid'im. Buraya onca zorluktan sonra canavarla dövüşüp beni burada tutan laneti kırmak için geldim. Kuzgun bana bir anıyı geri verdi ve bu bana rahat vermeyecek. Oğlumu tekrar görmek için kaç kere ölmem gerekirse öleceğim.|
|$lore_ashlands_random13|Birçok cesur savaşçının kendilerini canavara karşı tekrar tekrar savurup yatakları boş kalana kadar her seferinde iradelerinin bir kısmını kaybetmesini izledim. Kaderim buysa varsın öyle olsun. Bunlar bir zamanlar bir çiftçi olan Björn'ün sözleri.|
|$lore_blackforest_random01|Derinlerdeki ağaçlara, zifirî karanlığa dikkat et. Gece perdesi çekildiğinde, ateşine yakın dur.|
|$lore_blackforest_random02|Ormanın kökleri, toprağın altında, büyük böğümlerle birbirlerine düğümlenirler. Bir ipliği bile koparırsan tüm kumaş harekete geçer. Bir ağacı bile kesersen tüm orman bundan haberdar olur. Adını Yürüyen'e duyurursan seni arayıp bulur.|
|$lore_blackforest_random03|Adım Harald, kıyıdan geldim. Eskiden savaşçı olduğum dışında Midgard'a dair hatırladığım bir şey yok. Rüyalarımda öldürdüğüm insanların yüzlerini görüyorum. Geride bu taşı bırakıyorum ki en azından adım yaşasın.|
|$lore_blackforest_random04|Biraz soluklan ve kendi elleriyle bu taşı yontmuş ancak yazacak aklına bir şey gelmemiş Ulf'u hatırla.|
|$lore_blackforest_random05|Ezelî Olan'a dikkat et. Kuzgunlar onun çağlar öncesinde yüce Yggdrasil'den filizlendiğini ve insanlarla ağaçların dost olduğu günlerde bilgeliğin temsili olduğunu kelam ederler. Artık baltadan nefret ediyor ve Tanrıların buyruklarına karşı geliyor. Kibrinden dolayı Odin onu buraya sürdü ve bu diyarlarda kökleri her yere uzanıyor.|
|$lore_blackforest_random06|Valkürler tarafından buraya getirilen bizler, bu diyarın ilk insanları değiliz. Yerin altına inşa ettikleri salonları ve diktikleri şehirlerin harabelerini kendi gözlerimle gördüm. Bizi de mi aynı kader bekliyor?|
|$lore_blackforest_random07|Krallığının ilk günlerinde kudretli Odin tarafından bu dünyaya yedi varlık sürüldü. Bunlar, Tanrıbaba'nın boyunduruğu altına giremeyecek kadar kibirli tanrılar ve yaratıklardı. Ormanda Ezelî Olan'ın adımlarını duydum ve o kanat çırparken Dağ Ana'nın gölgesine sığındım. Diğerleri hakkında pek bilgim yok. Daha fazlasını görürsem bu taşa ekleyeceğim.|
|$lore_blackforest_random08|Midgard trolleri, bütün insan ırkında olduğu gibi senin de hafızandan kaybolup gidiyor olabilir. Az sayıda ve yalnız olan bu canlılar nemli mağaralara sığınır, zayıf ve deliduman olanların kemiklerini kemirirlerdi. Odin'in emriyle karanlık yerlere geri püskürtülmüşlerdi.<br/><br/>Ancak Valheim'da yıldızları parladı. Midgard'da dev trollerin diyarı ayakları altında ezdiği ve eğlence için ağaçları devirdiği yüzyıllardır görülmedi. Bu harika ve asil manzaraya bakın, yabancı diyarın gezginleri! Sonra da tabanları yağlayıp kaçın...|
|$lore_blackforest_random09|Ormandaki İhtiyar tarafından günah tohumlarından yetiştirilen bozcüceler, bir zamanlar bildikleri insan formunda giyinirler ama artık yüreklerinde hiç sıcaklık kalmamıştır. Artık yalnızca Kadim Olan'a hizmet etmek ve ormanı korumak için yaşıyorlar. Onlara göstereceğin merhametin seni dizginlemesine izin verme.|
|$lore_blackforest_random10|Hangileriniz caniydi, akraba katiliydi, haindi? Kimler kardeşini sırtından bıçakladı? Kimler bebekleri annelerinin kollarından söküp aldı, yağmalanan evleri ateşe verdi? Hiçbir şey hatırlamıyorsanız bunu bir lütuf olarak görün. Her biriniz büyük birer savaşçısınız, savaşçıların da ruhları asla temiz olmaz.<br/><br/>Ancak Odin size ikinci bir şans hediye etti... Tanrıbaba'ya şükürler olsun!|
|$lore_blackforest_random11|Kendisini tekrar bu aksi dünyayla birleştirmesi için Yitikler tarafından çağrılan kudretli Yggdrasil'in uzandığı gökyüzüne bak. Muhteşem ama ürkütücü bir manzaradır. Çünkü ulu ağaç boşlukta kıvrıldıkça dalları kırılıp çatırdıyor; etkisiyse kalan dokuz dünyada depremler, büyük fırtınalar ve kutsal yangınlar olarak hissediliyor. Yitikler'in öldürülmesi ve dengenin yeniden sağlanması gerekiyor.|
|$lore_blackforest_random12|Astrid benimle ilgilenmiyor. Bu taşı yüce Freya'yı onurlandırmak için bırakıyorum. Tanrıçam, onun kalbini kazanabilmem için bana Bjorn'unki gibi bir sakal bağışla!|
|$lore_blackforest_random13|Bu dünyada birçok kez yeniden doğdum. Tekrar tekrar ölsem de yine kendi evimde uyandım. Ancak yine de kaç kez daha böyle döneceğimi merak ediyorum. Frida bir yılan tarafından bizden koparıldı ve yatağı hâlâ boş duruyor. Bir trol Bjarni'yi ısırıp ikiye böldü ve iki yarısı da o zamandan beri görülmedi...<br/><br/>Her tarafta bizden önce gelenlerin kalıntıları var ama o kalıntıları inşa eden eller nerede?<br/><br/>Belki de bir savaşçı burada kalma isteğini kaybettiğinde bir daha asla dönemiyordur...|
|$lore_bonemass|ETLERİNİ PİŞİRİN|
|$lore_caveman01|Mağaranın derinlikleri bana zenginlikler vadetti.<br/>Meğerse aşağısı yukarıya göre daha kolaymış.<br/>Göl olabildiğince yardımcı oldu ama yardımı bir yere kadar sürdü.<br/>Her şeyi almamak için güçlü kalmam gerekiyordu, senin gibi bir yorgun gezgine de aynı fırsatı sunmak için.<br/>Umarım buraya benden daha hazırlıklı gelmişsindir.<br/>Gelmediysen yanımda sadece uzanıp dinlenebilmen için yerim var.|
|$lore_caveman02|Bu uçurumu tırmandım zirveye ulaşmak umuduyla.<br/>Maalesef çok derindi mağara.<br/>...<br/>Tekrar döndüm atlamayı düşünüp taşınmaya.<br/>Ah, korkak başım yine iniyor yavaşça.|
|$lore_cavepainting_label|Kadim mağara işaretleri|
|$lore_dragonqueen|ÇOCUKLARINI KURBAN EDİN|
|$lore_drake|Bu yazıyı okuyan yabancı, Ymir'in vücudundan olagelmiş kadim türlerden Buz ejderanlara dikkat etsinler.<br/><br/>Ejderanların en yaygın türü, Ejder olarak adlandırdığımız, yavrularını koruyan erkek cinsli küçük canlılardır. Genellikle kayalık yerlerdeki yuvaların başında bekler ve yumurtalarını korurlar. Erkeklerin aksine, dişiler daha büyüktür ve onlara daha az rastlanır lakin kızdıklarında çetin bir düşmana dönüşürler.|
|$lore_drake_label|Hikâye: Ejderler|
|$lore_draugr|Çağlar evvel, Valheim dünyası mağrur ve soylu bir ırka ev sahipliği yapardı. Bu ırkın mensupları, semaya uzanan dev kuleler ve değerli madenlere ulaşmak için derinlerine inen yapılar inşa ettiler lakin kibirleri sonları oldu. Tanrılara meydan okudular ve Aesir ile Vanir'e açtıkları savaşta kendi sonlarını getirdiler. Yüce Odin şehirlerini dümdüz etti ve Loki de madenlerini alevlerle yok etti. Kudretli Thor kulelerini yıktı, Freya ise tarlalarını tuzlu göz yaşlarıyla suladı.<br/><br/>Artık yüzlerce yıllık derinlerdeler. Şarkılar onların öykülerini bundan böyle söylemiyor. Şehirleriyse toprağın ta kalbine yutuldu. Ancak kibir fani değildir, ölmez. Son muharebenin savaşçıları bedenleri toz toprağa karışmadan yeryüzünü rahat bırakmayacaklar. Draugrlar şeytani zırhlarının içerisinde dirilip pasla kaplı, çaresiz bir şekilde Valheim'i adımlıyorlar.<br/><br/>Onları yok et ve göm. Öldüklerini unutmasınlar.|
|$lore_draugr_label|Hikâye: Draugrlar|
|$lore_dvergr_label|Munin: Cüceler|
|$lore_dvergr_text|Dikkatli ol savaşçı. Nidavellir'deki soydaşlarından ayrı kaldıklarından beri yalnız olan Cüce klanlarının bir karakoluna rastladın. Burada kapana kısılmışken hâlâ sıkı çalışıyorlar, asla gelmeyecek olan bir affı bekliyorlar. Üzücü bir kader...|
|$lore_dvergrhalls_label|Munin: Cüceyuvaları|
|$lore_dvergrhalls_text|Bunlar uzun zaman önce yaldızlı çağda inşa edilen Cüceyuvaları... Onların ihtişamı Altın Salon ile rekabet ederdi!<br/><br/>Ne yazık ki hiçbir şey sonsuza dek sürmüyor. Aşağıdaki salonlar ve içlerinde kalan hazineler şimdi daha uğursuz sakinler tarafından korunuyor.|
|$lore_eikthyr|SOYDAŞLARINI AVLAYIN|
|$lore_fader|ÇANLAR ONUN İÇİN ÇALIYOR|
|$lore_gdking|GENÇ OLANLARI YAKIN|
|$lore_giants_label|Munin: Dev kalıntıları|
|$lore_giants_text|Kadim bir çağın gölgeleri. Jötunn'lar bir zamanlar 10. dünyaya hükmederdi, ta ki zamanları tükenip farklı bir güç tarafından devrilene kadar.|
|$lore_goblinking|VE CAN ÇEKİŞEN RUHU PARÇALARA BÖLÜNÜP SOYDAŞLARIYLA PAYLAŞILDI|
|$lore_greydwarfs|Beni okuyanlar, Bozcücelerin karanlıkta pusuya yatan ruhsuz varlıklar olduğunu bilsin ve dikkat etsinler. Çürükten ve yağmurdan doğan bu yaratıklar, tüten topraktan mantar misali türerler. Çeneleri boş birer kafes, gözleri boş birer küreden ibarettir ve en korkusuz olanları bile korkuturlar.<br/><br/>Bir katil veya günahkârın ruhu toprağın altında çürümeye başladığında, boşluktan olma bir kist meydana gelir ve yakınlarındaki kaya, ağaç ve yosun parçalarını kendine çeker. Çürümüş bitkiler bedenine, taşlar kemiklerine ve paçavralar da cildine can verir.<br/><br/>Yürümemesi gereken bu varlık, gece çöktüğünde ayaklanır. Eğer ki bunu okuyan kişinin elinde kılıç varsa ya onları biçmelisin ya da ateş korkularından, onları yakabilirsin.|
|$lore_greydwarfs_label|Hikâye: Bozcüceler|
|$lore_intro|Evvel zaman önce Tanrıbaba Odin, dünyaları bir araya getirdi. Alt ettiği düşmanları onuncu dünyaya sürdü. Ardından bu hapishaneyi Dünya Ağacı'na bağlayan dalları keserek Valheim'ı başıboş bir şekilde sürüklenmeye mahkûm etti. <br/><br/><br/><br/>Yüzyıllar boyunca, bu dünya tekinsiz bir uyku halindeydi ama yitip gitmedi... Buzul çağlar geçerken Tanrıların gözleri ardında krallıklar bir bir yükseldi ve aynı şekilde kayboldu.<br/><br/><br/><br/>Odin, düşmanlarının tekrar güçlenmeye başladığını duyduğunda Midgard'daki savaş meydanlarına Valkürleri gönderdi ve en güçlü düşman savaşçılarını bulmalarını istedi. Ve o zaman, bu dünyanın ölü savaşçıları yeniden doğacaklardı...<br/><br/><br/><br/>…VALHEIM'DA!|
|$lore_intro_OLD|Valhalla'ya (Vikinglerin cennetine) girmeye layık olduğunu kanıtlaman için, Valheim'e, 10. Viking dünyasına gönderildin. Tanrıların lütfunu ancak, bu diyarların azametli yaratıklarını bertaraf ederek kazanabilirsin.|
|$lore_meadows_boartaming|Bu topraklar amansız ve vahşi olabilir ama buraya getirilen bizler daha da amansızız. Sana sunulan armağanlardan teselli bul yolgezer. Sağlam odun ve taşla, ormanın meyve ve çiçekleriyle avun.<br/><br/>Bu diyarı mesken tutmuş yaban domuzlarına karşı da gözün açık olsun. Ateşten ve insanoğlundan korksalar da onlara o oğula itaat etmeyi de öğretebilirsin. Sessizce gizlen ve stoğundan yemelerine izin ver. Topraktan çıkan köklere bayılırlar.|
|$lore_meadows_boartaming_label|Hikâye: Yaban domuzları|
|$lore_meadows_random01|Çimenin yeşerdiği toprakta<br/><br/>Ve mavilik varken tam semada<br/><br/>Olacak her daim bir ocak, bir yuva|
|$lore_meadows_random02|Çağlar evvel, Tanrıbaba Odin dünyaları bir araya getirdiğinde Vanir'i, devleri ve herkesten daha kadim varlıkları alaşağı etti. Ölüm bilmeyen en kudretlileri ise Yitikler olarak ebediyen buraya, yani Valheim'a sürüldü.<br/><br/>Onlar toprağı arşınladıklarında, bütün çelimsiz yaratıklar davul derisindeki ekmek kırıntıları misali sıçrarlar. Onları gördüğün zaman, ne olduklarını anlayacaksın.|
|$lore_meadows_random03|Yağmur ve güneş için Frey'e şükret<br/><br/>Topraktan can alan filizler için<br/><br/>Bir de asmadaki meyveler için Frey'e şükret<br/><br/>Et ve kemik için Odin'e şükret<br/><br/>Tencereni kaynatan, içini ısıtan ateş için<br/><br/>En önemlisi de gücün kuvvetin için Odin'e şükret|
|$lore_meadows_random04|Dur ve uyarıma kulak ver yolcu. Senden önce pek çoğumuz, Odin'in emriyle verdiği vazifeyi yerine getirmek üzere buraya getirildik. Önünde uzanan yol zorlu ve büyük tehlikelerle dolu. Canına kıymet veriyorsan, bu çayırlardan uzaklaşma ve yuvanı ağaçlardan uzakta bir yere kur. Boynuzlu'dan kork ve onun gibilerden sakın.|
|$lore_meadows_random05|Bu yaban topraklara göçen zavallı Ulf'un sözlerine kulak ver. Burada evini inşa etmek için en iyisinden taş ve odun bulacaksın. Yağmuru uzak tutmak için bir çatı ve bu çatının çökmesini engellemek için duvar yapman gerekecek. Son olarak da bir kapın olmalı ki içeri kolayca girip çıkabilesin.<br/><br/>Ulf bu şeyleri kendi başına öğrendi. Şimdi de başkalarına yardımcı olmak için bu taşa yazıyor. Ulf'un ruhu için Odin'e dua edin.|
|$lore_meadows_random06|Ben ormanın kalkanlı savaşçı kadınlarından Astrid. Buraya gelmeden önceki hayatım hakkında hiçbir şey bilmesem de kolum kılıcı ve gözlerim okun çizdiği yolu hatırlıyor. Bana artık kuzgun rehberlik ediyor ve Odin'in şanı uğruna azametli canavarlarla savaşıyorum.|
|$lore_meadows_random07|Çamurlarda gezen mavi gözlü sürüngenler olan Neck'ler, Valheim'a özgü küçük kertenkelelerdir. Huysuz ve kötü kalpli olan bu canlılar, sana gördükleri yerde saldırırlar ve bir böcek gibi ezilmelidirler. Diyarın yaratıkları onlardan iğrenip nefret ettiği için sudan çok uzaklaşmazlar.<br/><br/>Ama hiçbir yaratık her açıdan kötü değildir. Bu canlıların da kuyrukları leziz olur.|
|$lore_meadows_random08|Kalkanduvarın kardeşleri, yüreğinizi neşelendirin! Siz bizzat Tanrıbaba tarafından cesaretiniz ve yiğitliğiniz sebebiyle seçilen en büyük savaşçılarsınız. Şimdi ölüm sizi savaşta ve zorluklarda tanrılardan lütuf ve onur kazanarak kendinizi yeniden kanıtlayabileceğiniz bir diyara getirdi! Bırakın anılarınız günahlarınızla birlikte Midgard'da çürüsün, sizin en iyi parçanız burada!|
|$lore_meadows_random09|Burada başımın üstüne bir ağaç düştü ve ben de tanrıları lanetledim. Bu yüzden bu taşı onlara bir methiye olması niyetiyle buraya bırakıyorum ki belki saygısızca dile getirdiğim sözlerim için beni affederler.|
|$lore_meadows_random10|Bekle, gezgin. On Dünya'nın en sonuncusuna ve en kanunsuzuna hoş geldin. Valheim'da hava saftır, su derin ve berraktır, ormanlar ise ganimetle dolup taşar. Burada macera, neşe ve zafer parmaklarının ucundadır!|
|$lore_meadows_random11|Tok karın,<br/><br/>Pupa yelken,<br/><br/>Elinde ağırlığını hissettiğin mızrağın,<br/><br/>Ve bir şarkı mırıldanır dudakların.<br/><br/>Bırak tüm dağlar parçalansın;<br/><br/>Denizler yalnızca tuz kalana kadar haşlansın.<br/><br/>Güneşte bir gün geçirdiysen, zaten iyi bir hayat yaşamışsın.|
|$lore_mistlands_random01|Burada Odin'in bütün soyunun en kadimi ve tüm düşmanlarının en acımasızı Jötunn'lar yatıyor. Hayattayken yıkım getirirlerdi ama şimdi ölümde yeni tomurcukları besliyorlar.<br/><br/>Yaşayanlar ölmeli ve ölenler ise yeniden doğmalı.|
|$lore_mistlands_random02|Havanın büyüyle dolu olduğu<br/>Ve toprağın yaşamı hızla beslediği yerde,<br/>Sis mucizeler doğurur.|
|$lore_mistlands_random03|Sisteki hiçbir şeye güvenme. Buradan yedi kişilik bir grupla geçiyordum ama ekibi saymak için her durduğumuzda sekiz kişi sayıyorduk. Sekizincinin kim olduğunu bilmiyoruz ama sabah olduğunda gitmişti.|
|$lore_mistlands_random04|Cüceler eskilerin büyük demircilerinin torunları, toprağın derinlerinin kazıcıları, gizli hazinenin arayışçılarıdır.<br/><br/>Valheim'da Jötunn'ların kemiklerini kazıp çıkarırlar ve büyü malzemesi eitr damıtırlar. Yalnız başlarına bırakılırlarsa kendi planlarıyla mutludurlar ve başkalarının hayatını pek umursamazlar. Ancak kızdırılırlarsa kendilerini savunurlar.|
|$lore_mistlands_random05|Kuzgun, sakalımı yakan asayı nasıl yapacağımı bana gösterdi. Bir daha böyle şeylere burnumu sokmayacağım.<br/><br/>Sis ve büyüden bıkan Ulf bu taşı oydu. Şimdi gidip daha az dertli bir yer bulacağım.|
|$lore_mistlands_random06|Ulf'un sözlerine kulak verin ve kısaların kafasından şapkalarını almayın. Şakadan anlamıyorlar ve onları kızdırdıktan sonra tek çare öldürmek oluyor. Ayrıca şapkaları da sıradan kafalara uymuyor.|
|$lore_mistlands_random07|Sisteki gezginler,<br/>Nereye gidiyorsunuz?<br/>Nerede olduğunuzu bilmeden<br/>Ve nereye gittiğinizi görmeden.|
|$lore_mountains_fenring|Dolunay'da gelir<br/><br/>Gecelerin avcısı<br/><br/>Adımları sessizdir<br/><br/>Dişlerinde gıcırtısı.<br/><br/>Her daim peşindedir<br/><br/>Çabucak saldırıverir.|
|$lore_mountains_fenring_label|Hikâye: Fenring|
|$lore_mountains_random01|Pek azımız bu dağların yolunu bulabildi. Sayımız yirmiydi ama şimdi iki kişi kaldık. Agda ölüyor, bir Draugr oku kalbinin yakınına saplandı. Bu taşın yanı başında ona veda edip küllerini rüzgâra savuracağım. Sonra da yüzümü en yüksek tepeye çevirip Kanatlı Varlık'ı arayacağım. Odin bana ya ölüm ya da zafer bahşetsin.|
|$lore_mountains_random02|Ben, Ulf, tam da bu noktada, Valheim'daki hayatımın üçüncü yazında ulu ejderi ilk kez burada gördüm. Burada inip, geriye geyik ve domuz kemikleri ile Bozcüce kafatası bulunan koca bir dışkı yığını bıraktı. Bir daha bu yerin yanına bile yaklaşmam.|
|$lore_mountains_random03|Dur ve kulak kesil, yolgezer. Valheim'ın en yüksek tepelerinde hava ince ve kırılgandır. Buradayken bazen başka diyarlardan gelen sesler duyabilirsin. Midgard'daki bir muharebenin çarpışma seslerini, Valhalla'da verilen bir ziyafetin kahkahalarını ya da bir dünyadan diğerine geçen bir Valkür'ün haykırışlarını işitebilirsin.|
|$lore_mountains_random04|Burada bir keresinde tepemde uçan bir ulu ejder görmüş ve gözden kaybolana kadar çalılarda saklanmıştım. Bu taşı Cesur Ulf oydu.|
|$lore_mountains_random05|Bunu okuyanlar, evlerinin sıcağından ve çayırların güvenli alanlarından çok uzaktaki bu bayırlarda dolanmamaları gerektiğini bilsinler. Bu dağlarda kol gezen vahşi ve insafsız yaratıklar ete açtır ve bu yaratıklardan kötüsü de ayazın ta kendisidir. Canına değer veriyorsan kalın kürkünü giy, dayanıklı ateşler inşa et ve ulu ejderin yumurtalarına ilişme.|
|$lore_mountains_random06|Mahir şehirler kendi kendilerine yükselmezler<br/><br/>Yabancı toprakların göçmeni, kalbini böyle eyle<br/><br/>Yağız ol, ser verme|
|$lore_mountains_random07|Bu taş, Valheim'daki yedinci yılımda ben, yani Astrid tarafından yapıldı. Tam burada, Tanrıbaba benimle konuştu. Derin bir uykudan uyandığımda Tanrıbaba'nın kelamını etrafımdaki toprağa dağılmış, derin soğukla donmuş çakıllarda buldum. Çakılları elimde ısıttığımda, bana Tanrıbaba'nın kelamını bir bir ilettiler.<br/><br/>Yitikler'i alt et. Başlarını zincirle. Geleceğim.|
|$lore_mountains_random08|Valheim'da ilk gözlerimi açtığımda Odin'e kendisini bana göstermesi için yalvardım. Neredeydi o? Neden beni terk etmişti?<br/><br/>Uzun bir zaman boyunca bir yanıt almak uğruna fırtınalara ve vahşi yaratıklara karşı koydum. Tanrılar neredeydi? Benim ödülüm bu mu olacaktı? Odin neredeydi?<br/><br/>Ama artık biliyorum. Bozcüceleri bir araziden yabani ot ayıklar gibi söküp attığımda veya alazları köz gibi topladığımda Odin'in işini ben yaptım. Bir büyük ejderayla gücümü sınayıp kellesini tek darbeyle uçurduğumda içimde Odin'in neşesini derinden ve gururla hissettim.<br/><br/>Biz Odin'in elleriyiz, her birimiz... ve hayattaki kıymetimiz onun arzularını yerine getirmektir.|
|$lore_mountains_random09|Burada Otho ve Bjorn kimin en iyi sakala sahip olduğunu kararlaştırmak için muazzam bir düello yaptı. Şimdi ben, Bjorn, Otho'nun sakalının bir yaz günündeki mısır tarlası kadar parlak ve güzel olduğunu belirtecek şekilde bu taşı oyacağım. Yalan söylüyorsam yüce Thor kendi sakalımı suratımdan koparıp alsın.|
|$lore_mountains_random10|Eski Midgard'da tıpkı Vanir'in annelerine meydan okuyup onu Valheim'a gönderdiği gibi insan ordusu da ejderleri dağlardan püskürtmüştü. Ancak burada güçlerini ve sayılarını geri kazandılar. Şimdi muazzam kanatlarının çırpılma sesi zirvelerde yankılanıyor.<br/><br/>Savaşmaya layık gerçek birer düşman olan ejderler acımasız ve asil yaratıklardır. Kılıcınla yüzleşmekten kaçarlarsa onları oklarınla karşıla.|
|$lore_mountains_random11|Bu dağlarda rastgele mızrak fırlatsam beni öldürmeye çalışan bir şeylere denk geliyor. Ancak tepelerdeyken hayatın daha kolay olacağı güneşli ovalar gördüm. Şimdi orada yuva kurmaya gidiyorum. Ulf'un sözleri bunlardır.|
|$lore_mountains_random12|Valheim'da sana dost olacaklar vardır ama onları düşmanlarının arasından bulman gerekir. Eski dostların, ateş soyu, ay şarkıcıları. Asırlarca yanında avlandılar, şimdi kendi halkınla onların arasında yapılan eski anlaşmayı onlara hatırlatmalısın. Et onların sevinç kaynağıdır ve senin de ikramın olmalıdır.|
|$lore_munin_ashlands_label|Kül Diyarı bekliyor...|
|$lore_munin_ashlands_text|İyi dövüştün savaşçı! Kardeşim ve ben bu adakla bir ziyafet çekeceğiz! Ancak hâlâ yapılacak işlerin var... Kül Diyarı'nda kara bulutlar gökyüzünü kapladı ve kral közden yatağında dönüp duruyor. Bugüne kadar sadece çatışmalara katıldın. Şimdi kendini savaşa hazırlaman gerekiyor...|
|$lore_munin_label|Munin: Tanışma|
|$lore_munin_text|Gaak! Tanıştığımıza sevindim yolgezer... Benim adım Munin, Hugin'in kardeşiyim. Tanrıbaba'nın selamlarını getirdim. Tanrıbaba benim gözümden görür, kelamını dilimde taşırım. Tanrıbaba'nın yolundan sapmazsan seninle tekrar görüşeceğiz…|
|$lore_plains_random01|Yağmurlar yağmasa<br/><br/>Ekinler baş vermese de<br/><br/>Topraktan ümidini kesme.|
|$lore_plains_random02|Burası, Ulf için fazla sıcak. Ulf'un sakal kaşıma alışkanlığı var. Ulf bu taşı oydu ve yoluna devam etti.|
|$lore_plains_random03|Zihnini dindir, odaklan, yolcu. Odin, onu dinleyenlere kelam eder. Bu çorak diyarda Yitikler'den biri süregelir. Bu Yitik, bu öfkenin esiri olan kadim büyücü, bitik vücudundan geride kalanları kumlar boyu sürükler durur. Bu illeti katledecek savaşçıya Odin'in tebessüm edeceği besbellidir.|
|$lore_plains_random04|Buranın yakınlarında, bir yıldızın gecenin dev perdesinden kopup kuma düştüğünü gördüm. Yaklaştığımda kızıl bir ışık yayıyordu ama sabah olduğunda söndü. İçerisinde Tanrılardan kutsal bir kılıç tabanı, fırınlanmış yemek ziyafeti ve gümüş hediyesi vardı!|
|$lore_plains_random05|Ey dost, elini bu taşa koyduğunda taşı oyan Harald'ı hatırla. Midgard'daki savaş meydanında can verdim ama Valheim'da yeniden hayat buldum. Henüz savaşım bitmedi.<br/><br/>Yeşil çayırlarda Ulu Geyik Eikthyr ile çarpıştım ve gözüm, boynuzlarına kurban gitti.<br/><br/>Ormanın derinlerindeyken, kalkan tutan elimi Ormandaki İhtiyar'a verdim ve kaburgalarımın altına bir draugr oku yedim. Parmaklarım hâlâ hisseder.<br/><br/>Dağ yamacında Anne Ejder'le savaştım ve dizimden aşağısını kopardı. Artık bir asayla yürümem gerekiyor.<br/><br/>Artık nihai savaşım yaklaşıyor. Bu kez uyuduğumda nerede uyanacağım?|
|$lore_plains_random06|Rüyamda Odin, elinde bastonuyla, kafasında geniş kenarlı şapkasıyla bir ihtiyar olarak bana göründü. Kelamını taşıyan kuzgunlara güvenmemi ve başkaları da onun kudretini bilebilsin diye bu taşı oymamı söyledi. Valhalla'daki salonunda yerimizi kazanmak için Yitikler'i öldürmeliyiz.|
|$lore_plains_random07|Bastığın toprağın altında, Tanrıbaba'dan çok önce Valheim'da yaşamış ve tarihe gömülmüş kadim kabilelere mensup erkekler ve kadınlar yatıyor. Hazineler için derinlere inebilirsin ama unutma, onlardan bazıları hâlâ oralarda ve ölümden bihaberler.|
|$lore_plains_random08|Dağın tepesinden bu ova huzurlu ve güzel görünüyordu. Ama burası, diğer yerlerin tümünden daha kötü. Sürekli şu kahrolası böceklerin vızıltısını duyuyorum. Sakalım kar beyazı oldu. Bir tekne yapacağım.|
|$lore_plains_random09|Ne büyük zevk evden uzakta dolaşabilmek;<br/><br/>Rüzgâr saçına vururken<br/><br/>Masmavi gökyüzüne karşı ilerlemek.<br/><br/><br/>Ve ne büyük zevk,<br/><br/>Düşmanlarına karşı dik dururken<br/><br/>Dünyaya açıkça konuşabilmek.<br/><br/><br/>Ve sonra ne büyük zevk,<br/><br/>Yolculuğundan eve dönmüşken<br/><br/>Şöminede ateşi hâlâ parlarken görmek.|
|$lore_plains_random10|Bu ovalarda yalnızca en güçlüler hayatta kalarak sözlerimi okuyup tavsiyeme kulak verebilir. Bu yüzden şimdi bana kulak verin.<br/><br/>Yüce Odin sevgi dolu, yumuşak ve nazik bir baba değildir; sevimli sözler etmez. Ama esirlerine ölüm tehdidiyle emir veren, kolları altın kaplı katı bir köle sahibi da değildir.<br/><br/>Sizi burada düşmanlarıyla savaşmaya ondan korktuğunuz veya onu sevdiğiniz için göndermedi, onun düşmanları sizin de düşmanlarınız olduğu için gönderdi. Onun neşesi ve onuru sizin de neşeniz ve onurunuz. Siz de tıpkı bir zamanlar onun da yaptığı gibi savaşıp zafer alacaksınız.<br/><br/>Odin'in oğulları ve kızları, sevinin!|
|$lore_plains_random11|Bu ovalarda bir zamanlar kibirleri sebebiyle Odin onları cezalandırana dek insanlarınkiyle rekabet etmek için kuleler ve şehirler inşa eden kadim ırk Fuling'lerin yuvaları görebilirsin. Odin, zırhlarını ayaklarının altında salyangoz kabuğu gibi parçalayarak Fuling ordularını çiğneyip geçti ve şehirlerini yerle bir etti.<br/><br/>Yalnızca ulu büyücü Yagluth, Odin'in öfkesi karşısında diz çökmedi. Bu yüzden Odin, Fuling halkından geriye kalanlar arkasından yaş dökerken Yagluth'un vücudunu ikiye böldü ve kanının toprağa karışıp etinin toz olduğu Valheim'a gönderdi.<br/><br/>Şimdi Fuling'ler Valheim ovalarında kalplerinde yalnızca nefret ve korku taşıyarak acımasız ve vahşi bir şekilde hüküm sürüyorlar. Ancak Fuling şamanlarının çarpık büyüleri için faydalandığı görünmez bir güç olarak Yagluth hâlâ onlarla birlikte ve halkı dünyaya yayılırken tekrar güçleniyor...|
|$lore_plains_random12|Lox'lar, Yitikler bu diyardan kovulmadan çok uzun zaman önce de Valheim ovalarında dolaşıp toprağı sarsan kudretli yaratıklardır. Çabuk sinirlenirler ve savunmasız bir şekilde onlara yaklaşacak kadar aptal olanları ezerler. Ancak insan elini tanırlar ve tekrar sevmek üzere eğitilebilirler.|
|$lore_plains_random13|Bil ki, gezgin, Valheim dünyasında ölemiyor olsan da yeniden doğmamaya başlayabilirsin. Senden önce nicesi Odin'in emrini yerine getirmek üzere bu diyara gelip kendi iradesine söz geçirmekten âciz düştü...|
|$lore_queen|Kapıyı mühürleyip anahtarı attık. Kendi haline bırak.|
|$lore_surtlings|Beni okuyanlar Alazları bilsin ve o şeytani varlıklara kurban gitmesinler.<br/><br/>Evvel zaman önce büyük iblis Surtr, İlk İnsanlara, artık geriye sadece yankıları kalmış dev savaşçılara yenildi ve yok edildi. Kılıç ve çekiçleriyle Surtr'un bedenini öylesine dövdüler ki özündeki ateş, pek çok küçük parçaya ayrıldı.<br/><br/>Ateşten yaratıldığı gibi, her bir parçası da yine ateşe dönüştü.. Surtr'un beyni veya Surtr'un ciğeri diyebileceğimiz bir şey yoktu çünkü her parça Surtr'a aitti. Sözün özü, ateşten doğanın her parçası hâlâ Surtr'du ama güç, düşünce ve anı olarak çok daha zayıftı ve tekrar bir bütün olamazdı.<br/><br/>Bu parçalara Alaz diyoruz ve bu varlıklarla karşılaşıldığında onları tıpkı ateşten artakalan közler misali söndürmeli veya alevlerini kökünden kurutmak için suya boğmalısın. Sana uzaktan ateş fırlatacak, kaos ve yıkımdan huzur bulacaklardır. Surtr, bin parçaya bölünmüş bu haliyle bile İlk İnsanların çocuklarından intikam alma peşindedir.|
|$lore_surtlings_label|Hikâye: Alazlar|
|$lore_swamp_random01|Zavallı Ulf'un sözlerine kulak ver ve evini bu tehlikeli, bulanık sulara yakın inşa etme. Öyle ki eline kötü rüyalardan ve ıslak bir yataktan başka hiçbir şey geçmeyecektir. Bu taşı uyarı amaçlı bırakıyor ve yuvamı yüksek bir yere yapmak üzere yola koyuluyorum.|
|$lore_swamp_random02|Oyalanma yolgezer. Hava hastalıklı, sularsa zehirli. Draugrlar etrafta cirit atıyor ve ismini zikretmeyeceğim o berbat kemik ve günah yığını suyun altında çırpınıyor. Dikkatli ve hızlı ilerle, yüzünde güneşi hissedene dek durma.|
|$lore_swamp_random03|Bu kasvetli bölgede hâlâ parıldayan bir şey bulabilirsin. Savaştan kalma kemik, savaşçı altınları, demirci ocağının kalıntıları... Parlak demirler onu alacakları bekliyor!|
|$lore_swamp_random04|Ey yolgezer, beni unutma. Ben bir zamanlar evi Midgard'ın dağlarında olan bir adamım. İstirahati hak ettiğimi düşündüğümde, Valheim'da yaşamdan sonra yaşamı bulmak için buraya getirildim. Bozcüceler gelmeden evvel yedi kişiydik. Şimdilerde artık ne Odin beni dinliyor ne de denizler beni kabul ediyor. Pek yakında geriye kalan tek şey bu taş olacak.|
|$lore_swamp_random05|Alazlara dikkat et. Evvel zamanda söndürülüp küle çevrilmiş büyük bir ateşin közleri onlar. Bataklık onları sürüler halinde kendine çekiyor ama ışıklarını uzaktan görmek mümkün. Yükseklerden sapma.|
|$lore_swamp_random06|Draugrlar asırlar öncesinde bu topraklarda dolaşıyordu. Tıpkı senin şu an yaptığın gibi. Onlara acı çünkü bir zamanlar tanıdıkları dünyanın gölgesinde, yaşamla ölüm arasına sıkışıp kalmışlar. Onları yok etmek, gösterebileceğin en büyük merhamettir.|
|$lore_swamp_random07|Sis ve kasvetin altında<br/><br/>Kemiklerin fısıltısı<br/><br/>Ağzında tek bir cümlesi<br/><br/>Ete duyulan özlemin sesi.|
|$lore_swamp_random08|Odin'in gözü çağlardır Valheim'a bakmaz. Ancak tanrılar görmezden gelse de bazı yaratıklar bu unutulmuş dünyaya sızarken bazıları da çatlaklar arasından düştü. Troller, goblinler ve insanlar bu gizli yollardan ilerledi. Krallıklar yükselip çöktü ve yerin derinliklerine gömüldü.<br/><br/>Yürürken ölülerin arasında, bir mezara basarsın her adımında.|
|$lore_swamp_random09|Burada yedi draugr öldürüp uzun yıllar süren savaşlarına ve ıstıraplarına son verdim. Şimdi benimkine kim son verecek?|
|$lore_swamp_random10|Ben Gudrun, hiçbir adamın karısı da değilim, hiçbir babanın kızı da. Bana kalan yalnızca adımdır. Ancak bazen uyandığımda göğsümde bir bebeğin ağırlığını hissedip ağlıyorum. Yüce Freya, bana merhamet göster.|
|$lore_swamp_random11|Tanrılar seni terk etti diye korkma. Valheim dünya ağacından uzaklaşsa ve Vanir silahlanıp bu dünyaya gelemese de onlar seni uzaktan izliyor. Onları hep aklında tut. İnancın güçlüyse sana bir işaret gönderebilirler.|
|$lore_swamp_random12|Peşinden yeni bir hayat getirmeyen bir ölüm yoktur. Kurtçuklar savaş alanında beslenirken Odin'in düşmanlarından oluşan yığınla ceset de Valheim'a yeni bir hayat getirdi. Küçük yaratıklar onların etlerinden beslenip hâlâ onlara tutunan kadim büyüyle doydular. Ruhlarının bir kısmı toprağı canlandırmak için yerin içine daldı, diğerleriyse kendilerine yeni bedenler aramaya koyuldu.<br/><br/>Ölüleri hor görme, sen de zaten bir ölüsün.|
|$lore_wraith|Dinlen yolcu ve heyulalara dikkat et.<br/><br/>Bir savaşçı öldüğünde ruhunun serbest kalıp toprağa karıştığını ve orada yeni bir cana dönüşüp ardından gelenlere güç verdiğini biliyoruz.<br/><br/>Lakin kahramanlıklarıyla adını yüceltememiş bir kahraman ölürse ruhu ince ve hafif olacak, bacadan tüten duman misali yukarı doğru süzülecektir. Bu ruh kırıntıları bazen ağaç dallarına belli bir süre takılı kaldığında heyulalara, tek güdüleri öfke ve kıskançlık olan sefil gölgelere dönüşürler. Karamsarlık dolu halleriyle diyarda dolanan bu varlıklar, yalnızca yaşayanların peşindedirler.<br/><br/>Bir heyulaya rastlayacak olursan ona merhamet et ve öldür.|
|$lore_wraith_label|Hikâye: Heyulalar|

|Key M-Z |Turkish Text |
|---|---|
|$mac_gamecenter_required_online_header|Sign in to Game Center for online features|
|$mac_gamecenter_required_online_text|Valheim uses Game Center for authentication - sign in to Game Center and restart Valheim in order to use online features. |
|$menu_acceptedinvite|Bir daveti kabul ettin!|
|$menu_addserver|Sunucu ekle|
|$menu_addserverfailed|Sunucu eklenemedi!|
|$menu_addserverfaileddnslookup|DNS araması başarısız oldu!|
|$menu_addserverfailedincorrectformatting|Girdi, geçerli bir IP, URL veya davet kodu değil!|
|$menu_addserverfailedresolvejoincode|Katılma kodu çözümlenemedi|
|$menu_back|Geri|
|$menu_backupcount|$1 yedek|
|$menu_backuprestorefailedheader|Yedek geri yüklenemedi!|
|$menu_backuprestorepromptheader|Yedek geri yüklensin mi?|
|$menu_backuprestorepromptrecover|Silinen $1 dosyası, $2 yedeğinden geri yüklenecek.|
|$menu_backuprestorepromptreplace|Mevcut $1 sürümü yedeklenecek ve yerini $2 alacak.|
|$menu_barber_title|Berber istasyonu|
|$menu_beard|Sakal|
|$menu_blondness|Sarışınlık|
|$menu_cancel|İptal|
|$menu_changelog|Değişiklik Günlüğü|
|$menu_characterexist|Bu isim kullanılıyor.|
|$menu_characters|Karakterler|
|$menu_checklogfile|Lütfen daha fazla bilgi için Player.log dosyasına göz at|
|$menu_close|Kapat|
|$menu_closemenu|Menüyü Kapat|
|$menu_cloudsavesdisabled|Tercihlerinde bulut kayıtlarını devre dışı bıraktın. Bulutta kayıtlı herhangi bir dünya veya karakter burada görünmez. Bu dosyalara tekrar erişmek için bulut kayıtlarını etkinleştir.|
|$menu_cloudstoragefull|Bulut depon dolu!|
|$menu_cloudstoragefullcharacterprompt|Bu karakteri yerel depona kaydetmek ister misin?|
|$menu_cloudstoragefullmovingsave|Veri kaybını önlemek için mevcut oturumun kayıt verileri yerel depona taşınabilir.|
|$menu_cloudstoragefulloperationfailed|Bu işlem, bulut depon dolu olduğu için başarısız oldu. Lütfen bazı dünyaları yerel depona taşıyıp tekrar dene.|
|$menu_cloudstoragefullworldprompt|Bu dünyayı yerel depona kaydetmek ister misin?|
|$menu_cloudstoragelow|Bulut depolama alanı azaldı|
|$menu_cloudstoragelownextsave|Bulut depolamadaki alanın azaldığı için dünyanın veya karakterinin kayıtlarının alınabileceğini garanti edemiyoruz. Lütfen ana menüdeki Kayıtları yönet ekranından boş yer açmaya çalış.|
|$menu_cloudstorageused|Kullanılan bulut deposu: $1/$2|
|$menu_collapse|Daralt|
|$menu_combat|Çatışma|
|$menu_combat_tooltip|Ne kadar hasar alacağını ve vereceğini belirler.|
|$menu_community|Topluluk|
|$menu_communityserver|Topluluk Sunucusu|
|$menu_connect|Bağlan|
|$menu_connecting|Bağlanılıyor|
|$menu_continue|Devam et|
|$menu_corruptsaverestore|Bu dünya kaydı arızalı. Son alınan yedeği kullanarak dosyayı geri yüklemek ister misin?|
|$menu_craft_tab|ÜRETİM|
|$menu_credits|Emeği geçenler|
|$menu_crossplayserver|Çapraz Oynanış|
|$menu_customize|Özelleştir|
|$menu_death_casual_tooltip|Kuşanılmış ekipmanını düşürmezsin ve normal moda kıyasla yeteneklerin çok daha az azalır.|
|$menu_death_easy_tooltip|Tüm eşyalarını düşürürsün ve normal moda kıyasla yeteneklerin daha az azalır.|
|$menu_death_hard_tooltip|Kuşanılmış eşyaları düşürürsün, envanterinin geri kalanı sonsuza dek kaybolur ve yetenekler daha hızlı azalır.|
|$menu_death_hardcore_tooltip|Taşıdığın tüm eşyaları ve yeteneklerini sonsuza dek kaybedersin.|
|$menu_death_normal_tooltip|Tüm eşyalarını düşürürsün ve tüm yetenekler normal bir şekilde azalır.|
|$menu_death_veryeasy_tooltip|Tüm eşyalarını düşürürsün ve normal moda kıyasla yeteneklerin çok daha az azalır.|
|$menu_deathpenalty|Ölüm Cezası|
|$menu_deathpenalty_tooltip|Öldüğünde ne olacağını belirler.|
|$menu_deleted|Silinmiş|
|$menu_deletefailedheader|Dosya silinemedi!|
|$menu_disallowedcharactername|Bu isimle bir karakter oluşturulamaz|
|$menu_disallowedworldname|Bu isimle bir dünya oluşturulamaz|
|$menu_done|Tamam|
|$menu_duplicatefilepromptheader|Kayıt taşınamıyor|
|$menu_duplicatefileprompttext|Zaten hedef konumda $1 isminde bir kayıt bulunuyor.|
|$menu_enteripinvite|IP veya davet kodu gir|
|$menu_entername|Ad girin|
|$menu_enterpassword|Şifre Gir|
|$menu_enterseed|Seed gir|
|$menu_events|Baskın Oranı|
|$menu_events_tooltip|Düşmanların ne sıklıkla üssüne baskın yapacağını belirler.|
|$menu_exit|Çıkış|
|$menu_exitconfirm|Oyundan çıkacak mısın?|
|$menu_expand|Genişlet|
|$menu_favorite|Favori|
|$menu_feedback|Hata bildirmek, genel bir öneri vermek veya havalı bir fikrini bize yazmak ister misin? Şu şık butonu kullanarak bize bir mesaj gönderebilirsin.|
|$menu_female|Kadın|
|$menu_filter|Filtre|
|$menu_firehazards|Yangın Tehlikeleri|
|$menu_firehazards_tooltip|Yalnızca Kül Diyarı'nda değil tüm dünyada odun alev alıp yangın yayılabilir.|
|$menu_friendinvite_accepted_header|Friend invite accepted|
|$menu_friendinvite_accepted_text|Waiting for friend to start the game...|
|$menu_friendinvite_selectcharacter_header|Accepted invite from |
|$menu_friendinvite_selectcharacter_text|Please select the character you would like to use to join their game.|
|$menu_friends|Arkadaşlar|
|$menu_gamepreviewpopupheader|Valheim, Oyun Önizlemesi'nde|
|$menu_gamepreviewpopuptext|Bazı platform veya oyun özellikleri düzgün çalışmayabilir ve oyun çökebilir.|
|$menu_generating|Generating...|
|$menu_hair|Saç|
|$menu_hairtone|Saç Rengi|
|$menu_invite_friends_macos|Invite Friends|
|$menu_ipportorinvitecode|[IP/URL]:[port] veya [6 haneli davet kodu]|
|$menu_joincode|Katılma Kodu $1|
|$menu_joincode_copied|Katılma kodu panoya kopyalandı!|
|$menu_joindifferentserver|Farklı bir sunucuya katılmak üzeresin!|
|$menu_joingame|Oyuna Katıl|
|$menu_joinip|IP ile bağlan|
|$menu_kick_player|$1 adlı oyuncuyu gerçekten oyundan atmak istiyor musun?|
|$menu_kick_player_title|Oyuncu atılsın mı?|
|$menu_legacynotice|Lütfen bu dosyanın şu anda eski dosya düzeninde saklandığını ve bunun da eşitleme çatışmalarına ve veri kaybına sebep olabileceğini unutma. Dosya, sıradaki kaydında buluta taşınacak. Bulut kayıtlarını devre dışı bıraktıysan eşitlenmeyen yeni klasöre kaydedilecek. Dosyalar taşındığında her zaman yerel bir yedek saklanır.|
|$menu_legacynotice_worlds|Dosyayı eşitlenmeyen yerel bir klasörde saklamayı tercih edersen <color=orange>Yerele Taşı</color> butonunu kullan. Bu başlıca özel sunucular için ya da aynı bilgisayarda farklı hesaplarla aynı dünyada oynamak istiyorsan önerilir. Bulutu elle devre dışı bıraktıysan dosya eşitlenmeyen yeni yerel klasöre taşınır.|
|$menu_less|Daha az|
|$menu_loading|Yükleniyor...|
|$menu_logging_in_played_failed_not_signed_in_to_platform|Çevrim içi oynamayı denemeden önce oynamakta olduğun platforma giriş yaptığından emin ol.|
|$menu_logging_in_played_failed_not_signed_in_to_platform_gamecenter|Çevrim içi oynamayı denemeden önce Oyun Merkezi'ne giriş yaptığından emin ol. Değişikliklerin uygulanması için oyunu yeniden başlatman gerekebilir.|
|$menu_logging_in_playfab_failed_header|Could not log in to PlayFab. |
|$menu_logging_in_playfab_task_header|Logging in to PlayFab...|
|$menu_loginfailedtext|Giriş yapılamadı. Lütfen internete bağlı olduğundan emin ol.|
|$menu_loginheader|Arka uç hizmetine giriş yapılıyor...|
|$menu_loginretrycountdowntext|$1 saniye içinde tekrar denenecek...|
|$menu_logintext|Giriş yapılıyor...|
|$menu_loginwithplayfab_header|Log in with PlayFab|
|$menu_loginwithplayfab_text|Valheim uses PlayFab to connect you with other players online. Sign in to PlayFab? This automatically creates an anonymous PlayFab account for you based on which platform you use to play on.|
|$menu_logout|Oturumu kapat|
|$menu_logoutconfirm|Oturumu kapatacak mısın?|
|$menu_logoutprompt|Mevcut oturumdan çıkış yapmak istediğine emin misin?|
|$menu_lowdisk_block_exitanyway_header|No disk space left - Exit anyway?|
|$menu_lowdisk_block_exitanyway_prompt|You do not have enough space left on the disk to save your game. Exit anyway? (Your world and character will NOT be saved!)|
|$menu_lowdisk_header_block|Diskte alan kalmadı|
|$menu_lowdisk_header_warn|Uyarı: Diskte boş alan azalıyor|
|$menu_lowdisk_message_block|Oyunu kaydetmek için diskte boş alan kalmadı. Lütfen diskte yer açıp tekrar kayıt almayı dene.|
|$menu_lowdisk_message_warn|Diskte boş alan azalıyor. Oyunun kaydedilecek ancak diskte yer açmak faydalı olabilir.|
|$menu_male|Erkek|
|$menu_managesaves|Kayıtları yönet|
|$menu_manualsave|Kaydet|
|$menu_manualsavetime|Son Kayıt: $1 dak. önce|
|$menu_missingmetarestore|Bu dünya kaydının meta dosyası eksik. Son alınan yedeği kullanarak dosyayı geri yüklemek ister misin?|
|$menu_mod_notice|Valheim'ın modlu bir sürümünde oynuyorsun.<br/>Lütfen bunun, orijinal oyun deneyimini değiştirebileceğini unutma. Herhangi bir sorun yaşıyorsan bildirmeden önce lütfen bütün modları kaldır. Yakın zamanda bir yama olduysa daha önce çalışan bazı modlar güncellenene kadar bozulabilir.|
|$menu_modifier_casual|Rahat|
|$menu_modifier_casual_tooltip|Rahat modda çatışmalar çok daha kolaydır, sen düşmanlara saldırmadıkça onlar da sana saldırmaz ve üssüne hiç baskın yapılmaz. Kaynaklar çok daha boldur, öldüğünde eşyalarını düşürmez ve yeteneklerini kaybetmezsin.|
|$menu_modifier_custom|Özel niteleyiciler|
|$menu_modifier_default|Normale döndür|
|$menu_modifier_default_tooltip|Valheim'ı amaçlandığı şekilde ve zorlukta oynamanın yolu. En iyi deneyim için ilk oynanışta şiddetle tavsiye edilir.|
|$menu_modifier_easy|Kolay|
|$menu_modifier_easy_tooltip|Çatışmalar daha kolaydır ve üssüne daha az baskın yapılır.|
|$menu_modifier_hammer|Çekiç Modu|
|$menu_modifier_hammer_tooltip|Çekiç modunda bir tarifi keşfettiğinde tüm yapı parçalarını ücretsiz bir şekilde inşa edebilirsin. Üssüne baskın yapılmaz ve sen düşmanlara saldırmadıkça sana saldırmazlar. Diğer zorluk ayarları her zamanki gibidir.|
|$menu_modifier_hard|Zor|
|$menu_modifier_hard_tooltip|Çatışmalar daha zordur ve canavarlar üssüne daha fazla baskın yapar.|
|$menu_modifier_hardcore|Aşırı Zor|
|$menu_modifier_hardcore_tooltip|Çatışmalar çok daha zordur ve düşmanlar üssüne daha sık baskın yapar. Öldüğünde tüm eşyaların ve yeteneklerin sonsuza dek kaybolur. Haritan yoktur ve geçitleri kullanamazsın.|
|$menu_modifier_immersive|Sürükleyici|
|$menu_modifier_immersive_tooltip|Sürükleyici modda haritan olmaz ve geçitleri kullanamazsın. Dünyada nerede olduğunu çözmek için kendi aklını kullanman gerekir. Diğer zorluk ayarları her zamanki gibidir.|
|$menu_modifier_normal|Normal|
|$menu_modifier_normal_tooltip|Normal zorluk, ilk oynayışın için önerilir ve genel olarak en dengelisidir.|
|$menu_modifier_popup_text|Lütfen bu ayarların Valheim tecrübeni bozabileceğini unutma. İlk oyun deneyiminde orijinal ayarlar ile oynaman şiddetle tavsiye edilir. Böylece oyunu amaçlandığı şekilde oynayabilirsin.|
|$menu_modifier_popup_title|Uyarı: Dünya Niteleyicileri|
|$menu_modifier_veryeasy|Çok kolay|
|$menu_modifier_veryhard|Çok zor|
|$menu_more|Daha fazla|
|$menu_movefailedheader|Dosya taşınamadı!|
|$menu_movetocloud|Buluta Taşı|
|$menu_movetolocal|Yerele Taşı|
|$menu_muchless|Çok daha az|
|$menu_muchmore|Çok daha fazla|
|$menu_name|İsim|
|$menu_ndaaccept|Kabul Et|
|$menu_ndadecline|Reddet|
|$menu_new|Yeni|
|$menu_newcharacter|Yeni karakter|
|$menu_newworld|Yeni Dünya|
|$menu_newworldalreadyexists|Bu dünya zaten var!|
|$menu_newworldalreadyexistsmessage|$1 isminde bir dünya zaten var|
|$menu_no|Hayır|
|$menu_nobossportals|Baş düşman geçidi yok|
|$menu_nobossportals_tooltip|Bir baş düşman etkinken geçit kullanılamaz veya baş düşman zindanlarından çıkılamaz.|
|$menu_nobuildcost|Ücretsiz İnşaat|
|$menu_nobuildcost_tooltip|Yapı parçalarının inşaatı için malzeme bedelleri kalkar. Yine de her zaman olduğu gibi tarifleri keşfetmen gerekir.|
|$menu_nomap|Haritasız|
|$menu_nomap_tooltip|Hiçbir haritaya erişimin olmayacak. Bu, oyunu amaçlanandan daha zor yapar.|
|$menu_none|Asla|
|$menu_noportals|Geçitsiz|
|$menu_noportals_tooltip|Dünyada ışınlanmak için geçitleri kullanamayacaksın.|
|$menu_ok|Tamam|
|$menu_onlineprivilegetext|Platform hesap ayarların çevrimiçi çok oyunculu moda katılmanı önlüyor.|
|$menu_onlysameversion|Yalnızca $1 sürümünde çalışan sunuculara katılabilirsin|
|$menu_passivemobs|Pasif düşmanlar|
|$menu_passivemobs_tooltip|Düşmanlar sen onlara saldırmadıkça sana saldırmaz.|
|$menu_password|Şifre|
|$menu_passwordfieldempty|Boş|
|$menu_passwordinvalid|Geçersiz şifre|
|$menu_passwordshort|Şifre çok kısa|
|$menu_playerbasedevents|Oyuncuya dayalı baskınlar|
|$menu_playerbasedevents_tooltip|Baskınlar sunucuda öldürülen baş düşmanlar yerine her oyuncunun kendi ilerlemesine dayalı olur. Bu ayar, oyunun farklı ilerlemelere sahip oyunculara karşı daha dostane olmasını istiyorsan önerilir.|
|$menu_players|Oyuncu Listesi|
|$menu_pleasewait|Lütfen Bekle|
|$menu_portals|Geçitler|
|$menu_portals_tooltip|Oyunda geçitlerin çalışma şeklini değiştirir.|
|$menu_presets|Hazır Ayarlar|
|$menu_privilegerequiredheader|Platform ayrıcalığı gerekli|
|$menu_public|Herkese Açık|
|$menu_recent|En Son|
|$menu_reloadfailed|Kayıtlar yeniden yüklenemedi|
|$menu_remove|Sil|
|$menu_removebackup|Yedek kaldırılsın mı?|
|$menu_removecharacter|Karakter silinsin mi?|
|$menu_removeserver|Sunucu kaldırılsın mı?|
|$menu_removeworld|Dünya silinsin mi?|
|$menu_resettutorial|Eğitimi yeniden başlat|
|$menu_resettutorial_descr|Odin'in kuzgunları sana anlattıkları her şeyi unuttu! Gördüğün ipuçlarını tekrar göreceksin.|
|$menu_resources|Kaynak Oranı|
|$menu_resources_tooltip|Dünyadan ve düşmanlardan kazanacağın kaynakların miktarını belirler.|
|$menu_restorebackup|Yedeği geri yükle|
|$menu_savefilecorrupt|Kayıt dosyası arızalı!|
|$menu_savesuccesfull|Kayıt başarılı!|
|$menu_seed|Seed|
|$menu_selectcharacter|Karakter seçin|
|$menu_selectserver|Sunucu seçin|
|$menu_selectworld|Dünya Seçin|
|$menu_server_info|Herhangi bir çevrimiçi sunucuya bağlanmak, oyun karakterinizi tehlikeye atar. Değerli eşyalarınızı kaybetme riskini azaltmak için çevrimiçi oturumlarda farklı bir karakter yaratmanızı tavsiye ederiz. Ayrıca çevrimiçi oyunlarda hilecilerle ve oyunun huzurunu bozanlarla karşılaşabileceğini unutma. Bol şans!|
|$menu_server_warning|Uyarı|
|$menu_serveroptions|Dünya Niteleyicileri|
|$menu_serverpassword|Sunucu Şifresi|
|$menu_settings|Ayarlar|
|$menu_sex|Cinsiyet|
|$menu_skintone|Ten Rengi|
|$menu_skipcutscene|Girişi atla|
|$menu_start|Başla|
|$menu_startgame|Oyuna Başla|
|$menu_startserver|Sunucuyu Başlat|
|$menu_teleportall|Geçit eşyaları|
|$menu_teleportall_tooltip|Daha rahat bir oynanış tarzı için geçitlerden tüm eşyalarını geçirebilmeni sağlar. Bu, oyunun amaçlanan ilerleme ve dünyayı keşfetme deneyimini bozar.|
|$menu_textcontainsbannedword|Metin yasaklı kelimeler içeriyor|
|$menu_tryagainorrestart|Lütfen tekrar dene veya sorun devam ediyorsa oyunu yeniden başlat|
|$menu_ugcwarningheader|Kullanıcı tarafından oluşturulan içerik engellendi|
|$menu_ugcwarningtext|Platform erişim ayarların, kullanıcı tarafından oluşturulan içerikleri görmeni önlüyor. Diğer oyunculardan gelen metinler yoğun bir şekilde filtrelenecek.|
|$menu_upgrade_tab|GELİŞTİRME|
|$menu_vulkancrashwarning_header|Valheim, Vulkan kullanılırken sisteminde çökmeye sebep olabilir|
|$menu_vulkancrashwarning_text|Şu anda Vulkan kullanıyorsun. Belirli sistemlerde Vulkan kullanılırken oluşan bilinen bir çökme sorununu inceliyoruz ve sisteminin etkilenme ihtimali olduğunu tespit ettik. Oyunda performans sorunları yaşanıyorsa lütfen varsayılan görüntüleme API'ını (DirectX 11) kullan. Sorun yaşadığın için üzgünüz!|
|$menu_worldnametip|Minimum 5 karakter|
|$menu_worlds|Dünyalar|
|$menu_yes|Evet|
|$msg_added|Eklendi|
|$msg_alreadysynced|Kopyalayacak yeni bir şey yok|
|$msg_bedenemiesnearby|Yakınında düşmanlar var|
|$msg_bedneedroof|Yatağa çatı gerekiyor|
|$msg_bednofire|Yatağın yakınında ateş gerekiyor|
|$msg_bedtooexposed|Yatak fazla açıkta|
|$msg_bedwet|Islandın|
|$msg_biome_found|$1 ortamını keşfettin|
|$msg_blocked|Engellendi|
|$msg_blockedbyboss|Habis bir güç tarafından engellendi.|
|$msg_broke|$1 kırıldı|
|$msg_broke_OLD|kırık|
|$msg_cantaddmore|Daha fazla $1 ekleyemezsin|
|$msg_cantconsume|Şu an bunu tüketemezsin|
|$msg_cantdrop|Bunu bırakamazsın|
|$msg_cantoffer|Adağına yanıt verilmedi|
|$msg_cantopen|Bunu açamazsın|
|$msg_cantpickup|Bunu alamazsın|
|$msg_cantremovenow|Şu an kaldırılamaz|
|$msg_cantsleep|Şu an uyuyamazsın|
|$msg_cantuseon|$2 üzerine $1 kullanamazsın|
|$msg_cart_incorrectposition|Doğru pozisyonda değil|
|$msg_cart_storage|Depo|
|$msg_cart_use|Use|
|$msg_consumed|Tüketildi|
|$msg_cookingstationtooweak|Bu pişirme istasyonu şunun için fazla zayıf:|
|$msg_dlcrequired|DLC gerekiyor|
|$msg_doesnotneedrepair|eşyasının onarıma ihtiyacı yok|
|$msg_donthaveany|Şundan elinde hiç yok:|
|$msg_door_needkey|Anahtar gerekiyor: $1|
|$msg_door_usingkey|Anahtar kullanıyorsun: $1|
|$msg_dropped|Bıraktın|
|$msg_extensionmissingstation|Uygun atölyenin yakınına yerleştirilmesi gerekiyor|
|$msg_fireadding|Ateşe $1 ekleniyor|
|$msg_firenotburning|Yanmıyor|
|$msg_fireportalalready|Geçit zaten etkin|
|$msg_fishing_catched|Yakalandı|
|$msg_fishing_hooked|Oltaya takıldı|
|$msg_fishing_linebroke|Misina koptu|
|$msg_fishing_lost|Balık kaçtı|
|$msg_fishing_wrongbait|Yeme gelmiyor.|
|$msg_food_done|Bir lokma daha yiyebilirsin|
|$msg_goodmorning|Günaydın|
|$msg_goodnight|İyi geceler|
|$msg_harpoon_escaped|kaçtı|
|$msg_harpoon_harpooned|zıpkınlandı|
|$msg_harpoon_linebroke|Misina koptu|
|$msg_harpoon_released|serbest bırakıldı|
|$msg_harpoon_targettoofar|Hedef çok uzak|
|$msg_incompleteoffering|Eksik adak|
|$msg_incompleteoffering_fader|Çanlar yerinde değil|
|$msg_inuse|Başkası kullanıyor|
|$msg_invalidplacement|Geçersiz yerleştirme|
|$msg_isfull|Zaten doydun|
|$msg_itsfull|Dolu|
|$msg_level|seviye|
|$msg_mapnodata|Masa boş|
|$msg_mapsaved|Keşiflerini masaya kaydettin|
|$msg_mapsynced|Masadan bazı keşifleri kopyaladın|
|$msg_missingitem|Doğru eşyaya sahip değilsin|
|$msg_missingrequirement|Eksik malzeme|
|$msg_missingstation|Gerekli atölye eksik|
|$msg_needcultivated|İşlenmiş toprak gerekiyor|
|$msg_needdirt|Bu bölge işlenemez.|
|$msg_needfire|Pişirme istasyonunun altında ateş olması gerekiyor|
|$msg_needspace|Daha fazla alan gerekiyor|
|$msg_newday|$1. Gün|
|$msg_newdish|Yeni yemek|
|$msg_newitem|Yeni eşya|
|$msg_newmaterial|Yeni malzeme|
|$msg_newpiece|Yeni yapı parçası|
|$msg_newrecipe|Yeni eşya tarifi|
|$msg_newstation|Yeni atölye|
|$msg_newstation_level|Yeni istasyon seviyesi|
|$msg_newtext|$1, külliyata eklendi|
|$msg_newtrophy|Av ganimeti topladın|
|$msg_nobuildzone|Bu bölgedeki mistik bir güç seni engelliyor|
|$msg_nocookitems|You don't have any cookable items|
|$msg_nocookroom|Boş yer yok|
|$msg_nomore|Daha fazla $1 yiyemezsin|
|$msg_noprocessableitems|İşlenebilir eşyan yok|
|$msg_noroom|Envanterde boş yer yok|
|$msg_noteleport|Envanterindeki bir eşya geçiti kullanmana engel oluyor|
|$msg_noteleportarea|Sadece taş çember içinde inşa edilebilir|
|$msg_notindungeon|Zindanlara yerleştirilemez|
|$msg_noturretammo|Balista oku bulunmuyor|
|$msg_offerdone|Adak adadın|
|$msg_offerdone_fader|Çanları çaldın|
|$msg_offerwrong|Hiçbir şey olmadı|
|$msg_outof|Tükendi:|
|$msg_pin_added|Harita konumu eklendi|
|$msg_pin_exist|Harita konumu zaten ekli|
|$msg_portal_blocked|Engellendi|
|$msg_privatezone|Nazarlık seni engelliyor|
|$msg_pvpoff|PVP hasarı kapalı|
|$msg_pvpon|PVP hasarı açık|
|$msg_removed|Çıkarıldı|
|$msg_repaired|$1 onarıldı|
|$msg_ship_ladder|Merdiven|
|$msg_skills_lowered|Yeteneklerin zayıfladı|
|$msg_skillup|Yeteneğin gelişti|
|$msg_snapping|Birleştirme:|
|$msg_snapping_auto|Otomatik|
|$msg_snapping_manual|Manuel|
|$msg_softdeath|Tanrılar sana merhamet etti|
|$msg_sold|$2 karşılığı $1 sattın|
|$msg_spawnpointset|Canlanma noktası belirlendi|
|$msg_stackall|Biriktirilen eşyalar:|
|$msg_stackall_hover|(Yığın biriktirmek için basılı tut)|
|$msg_stackall_none|Yığın biriktirecek eşya yok|
|$msg_stationneedroof|Atölyeye çatı gerekiyor|
|$msg_stationtooexposed|İstasyon fazla açıkta|
|$msg_throwinfire|Ateşe biraz $1 attın|
|$msg_toofew|Çok az|
|$msg_toohard|Fazla sert|
|$msg_turretotherammo|Balistaya zaten şu yüklendi:|
|$msg_useonwhat|$1 eşyasını neyin üzerine kullanacaksın?|
|$msg_walk|Yürü|
|$msg_wontwork|İşe yaramaz|
|$msg_worldsaveblockedonserver|The server does not have enough space left on disk to save your game.|
|$msg_worldsaved|Dünya kaydedildi|
|$msg_worldsavedcloudstoragefull|Bulut deposu dolu! Dünya yerel depoya taşındı ve kaydedildi.|
|$msg_worldsavewarning|Dünyanın kaydedilmesine|
|$msg_worldsavewarningonserver|The server is running out of disk space, consider clearing up space to be able to save in the future.|
|$msg_wrongbiome|Bu ortama yerleştirilemez|
|$msg_wrongitem|Yanlış eşya|
|$msg_wrongtool|Yanlış alet|
|$msg_youdied|ÖLDÜN!|
|$name_mistlands_bosshint|Gizemli bir metin|
|$NOT USED tutorial_stemple3_label|[NOT] USED tutorial_stemple3_label|
|$NOT USED tutorial_stemple3_text|[NOT] USED tutorial_stemple3_text|
|$npc_blabar|Blåbär|
|$npc_bogwitch|Bataklık Cadısı|
|$npc_bogwitch_random_buy1|Lhm!|
|$npc_bogwitch_random_buy2|Lhm gkr tlk.|
|$npc_bogwitch_random_buy3|Gkr, gkr!|
|$npc_bogwitch_random_goodbye1|Gnk.|
|$npc_bogwitch_random_goodbye2|Gnk nk.|
|$npc_bogwitch_random_goodbye3|Nk. Ktrr!|
|$npc_bogwitch_random_greet1|Gk!|
|$npc_bogwitch_random_greet2|Kgk.|
|$npc_bogwitch_random_greet3|Krrgk.|
|$npc_bogwitch_random_sell1|Mm, Ipr...|
|$npc_bogwitch_random_sell2|Lkm.|
|$npc_bogwitch_random_sell3|Kng.|
|$npc_bogwitch_random_starttrade1|Mg, Ihm?|
|$npc_bogwitch_random_starttrade2|Ptl mgg.|
|$npc_bogwitch_random_starttrade3|Hk! Mg, mg.|
|$npc_bogwitch_random_talk1|Hm...|
|$npc_bogwitch_random_talk2|Mhm...|
|$npc_bogwitch_random_talk3|Ngk...|
|$npc_dvergr_ashlands_random_aggravated1|Bunu cidden yapmamalıydın!|
|$npc_dvergr_ashlands_random_aggravated2|Sana bunun hesabını soracağım!|
|$npc_dvergr_ashlands_random_aggravated3|Bittin sen!|
|$npc_dvergr_ashlands_random_aggravated4|Seni kahrolası bir kül domuzuna yem edeceğim!|
|$npc_dvergr_ashlands_random_aggravated5|Seni küçük...|
|$npc_dvergr_ashlands_random_goodbye1|Hadi kendine iyi bak.|
|$npc_dvergr_ashlands_random_goodbye2|Orada dikkatli ol.|
|$npc_dvergr_ashlands_random_goodbye3|Sonra görüşürüz.|
|$npc_dvergr_ashlands_random_goodbye4|Sonra görüşürüz. Belki de görüşmeyiz.|
|$npc_dvergr_ashlands_random_goodbye5|Kaçtım.|
|$npc_dvergr_ashlands_random_greet1|Kim var orada?|
|$npc_dvergr_ashlands_random_greet2|Yaşayan bir kişi mi? Bu güzel bir hava değişikliği.|
|$npc_dvergr_ashlands_random_greet3|Selamlar.|
|$npc_dvergr_ashlands_random_greet4|Merhabalar.|
|$npc_dvergr_ashlands_random_greet5|Dost musun, düşman mı?|
|$npc_dvergr_ashlands_random_private_area_alarm1|Yapma!|
|$npc_dvergr_ashlands_random_private_area_alarm2|Kes şunu.|
|$npc_dvergr_ashlands_random_private_area_alarm3|Ellerine hâkim ol.|
|$npc_dvergr_ashlands_random_private_area_alarm4|Böyle gidersen kendini lavın içine fırlattırırsın.|
|$npc_dvergr_ashlands_random_private_area_alarm5|Yerinde olsam cidden bunu yapmazdım.|
|$npc_dvergr_ashlands_random_talk_in_faction_base1|O zar oyununa asla bahis oynamamalıydım...|
|$npc_dvergr_ashlands_random_talk_in_faction_base10|Bir evcil hayvan edinmeyi düşünüyordum. Sence de güzel olmaz mıydı?|
|$npc_dvergr_ashlands_random_talk_in_faction_base11|Annem hep derdi ki biraz terlemek sana iyi gelir!|
|$npc_dvergr_ashlands_random_talk_in_faction_base12|Falan filan falan filan.|
|$npc_dvergr_ashlands_random_talk_in_faction_base13|Umarım çocuklar evde iyilerdir. Onları son görüşümün üstünden artık çok zaman geçti...|
|$npc_dvergr_ashlands_random_talk_in_faction_base14|Eve geri döndüğümüzde sanırım yeni bir hobi edineceğim.|
|$npc_dvergr_ashlands_random_talk_in_faction_base15|Sanırım çizmemin onarıma ihtiyacı var.|
|$npc_dvergr_ashlands_random_talk_in_faction_base16|Ah, yüzümde sisin serin rahatlığını hissetmek... O görev yerini geride bırakmak ahmaklıktı.|
|$npc_dvergr_ashlands_random_talk_in_faction_base17|Yarının beraberinde nasıl keşifler getireceğini merak ediyorum. O Aksakallar ne kaçırdıklarını bilmiyorlar!|
|$npc_dvergr_ashlands_random_talk_in_faction_base18|Asla yalnızca bir kazı alanında aylarca görev yapamazdım. Biz Kızılsakalların gezmesi gerekir!|
|$npc_dvergr_ashlands_random_talk_in_faction_base19|Eminim ki Aksakallar başarımızı duyduklarında kıskanacaklar!|
|$npc_dvergr_ashlands_random_talk_in_faction_base2|Ama yine de sıcaklık o kadar da kötü değil!|
|$npc_dvergr_ashlands_random_talk_in_faction_base20|O duman mantarları şahsen hiç umurumda değil. Onları gerçekten anca kül domuzları yer!|
|$npc_dvergr_ashlands_random_talk_in_faction_base3|Birinin yalnızca iç çamaşırları ve birkaç kemerle etrafta koşuşturduğunu gördüm. İyi ve makul görünüyordu...|
|$npc_dvergr_ashlands_random_talk_in_faction_base4|O kardeşler iyi düşünmüştü. Havalı bir ormanda kendi iş macerana atılmış olduğunu hayal etsene!|
|$npc_dvergr_ashlands_random_talk_in_faction_base5|Burada ne bulacağımızı merak ediyorum. Eminim buna değecektir.|
|$npc_dvergr_ashlands_random_talk_in_faction_base6|Keşke güzel, soğuk bir banyo yapabilsem.|
|$npc_dvergr_ashlands_random_talk_in_faction_base7|Sence burada ne bulacağız?|
|$npc_dvergr_ashlands_random_talk_in_faction_base8|Dün o korlara epey iyi hasar verdik, değil mi?|
|$npc_dvergr_ashlands_random_talk_in_faction_base9|Bu taraflarda eskiden Zümrüt Alev'e taparlardı. Ne tür bir hami olduğunu merak ettiriyor doğrusu...|
|$npc_dvergr_ashlands_random_talk1|Sanırım terlemekten sakalımı yitireceğim.|
|$npc_dvergr_ashlands_random_talk10|Şu an bir bardak soğuk bira cidden iyi gelirdi...|
|$npc_dvergr_ashlands_random_talk11|Düşününce bu sıcaklık iyi bir demir ocağının sıcaklığından kötü değil.|
|$npc_dvergr_ashlands_random_talk12|Terini silmeyi unutma! Yoksa bir morgen seni koklayıp akşam yemeği yapmaya karar verebilir.|
|$npc_dvergr_ashlands_random_talk13|Kan taşı, kan taşı, kan taşı, kan taşı...|
|$npc_dvergr_ashlands_random_talk14|Büyük bir kemik yığını görürsen dikkat et. Bu çoğu zaman şu morgen yaratıklarından birinin yakınlarda olduğu anlamına gelir.|
|$npc_dvergr_ashlands_random_talk15|O alçak hortlaklar gerçekten de değerli taşlar topluyor...|
|$npc_dvergr_ashlands_random_talk16|Burası hayret verici! Keşfedilecek hâlâ çok yer var!|
|$npc_dvergr_ashlands_random_talk17|Sıcak olabilir ancak yaptığımız keşifler başlı başına bir ödül.|
|$npc_dvergr_ashlands_random_talk18|Ha! Bir Aksakal buraya gelmeye asla cüret etmez.|
|$npc_dvergr_ashlands_random_talk19|Aksakallar bu sıcağa asla dayanamazdı. Buna yetecek yürekleri yok!|
|$npc_dvergr_ashlands_random_talk2|Yerin altında, havalı kayaların arasında yaşamaya uygun biriyim. Bu şiddetli sıcağa değil.|
|$npc_dvergr_ashlands_random_talk20|Dün koca bir kül domuzu sürüsü bulunan bir harabeye denk geldim! Düşünüyorum ki orada yemek arıyorlardı.|
|$npc_dvergr_ashlands_random_talk3|Dikkat et, buradaki su bile kaynar sıcaklıkta.|
|$npc_dvergr_ashlands_random_talk4|Kuzenim sis diyarlarında görev yapıyor. Şanslı Aksakal...|
|$npc_dvergr_ashlands_random_talk5|Buralarda hiç gölge olup olmadığını merak ediyorum. Yardımcı olacağından değil de...|
|$npc_dvergr_ashlands_random_talk6|Hm. Çürük yumurta gibi kokuyor.|
|$npc_dvergr_ashlands_random_talk7|Adımına dikkat et. Erkek kardeşim bir süre önce bir ayak parmağını lava kaybetti.|
|$npc_dvergr_ashlands_random_talk8|Kız kardeşim bir kül domuzunu evcilleştirmeye çalıştı, anlarsın ya. Normalde et yememelerine rağmen kardeşimin elini ısırıp kopardı!|
|$npc_dvergr_ashlands_random_talk9|Asma yemişleri, asma yemişleri... Buralarda biraz gördüğümden eminim...|
|$npc_dvergrmage_random_aggravated1|Büyünün neler yapabileceğini göreceksin!|
|$npc_dvergrmage_random_aggravated10|Artık Odin sana yardım edemez!|
|$npc_dvergrmage_random_aggravated11|Seni uyarmıştık!|
|$npc_dvergrmage_random_aggravated2|Hel'in dibine git!|
|$npc_dvergrmage_random_aggravated3|Bunu cidden yapmamalıydın!|
|$npc_dvergrmage_random_aggravated4|Cüceler, yanıma!|
|$npc_dvergrmage_random_aggravated5|Haşlanmış et kızarmaya başladı, ona göre!|
|$npc_dvergrmage_random_aggravated6|Bize bulaşmaman gerektiğini öğreteceğim sana!|
|$npc_dvergrmage_random_aggravated7|Şimdi borunun döndüğü yeri göreceksin!|
|$npc_dvergrmage_random_aggravated8|Pislik insan!|
|$npc_dvergrmage_random_aggravated9|Nidhögg'ü boyla!|
|$npc_dvergrmage_random_goodbye1|Elveda.|
|$npc_dvergrmage_random_goodbye2|Gjall'lara dikkat et.|
|$npc_dvergrmage_random_goodbye3|Şimdilik görüşürüz.|
|$npc_dvergrmage_random_goodbye4|Belki tekrar görüşürüz, belki de görüşmeyiz.|
|$npc_dvergrmage_random_goodbye5|Güvenli yolculuklar dilerim.|
|$npc_dvergrmage_random_greet1|Hatırladığımdan uzunmuşsun.|
|$npc_dvergrmage_random_greet2|Demek bunca kargaşayı sen çıkarıyordun.|
|$npc_dvergrmage_random_greet3|Selam.|
|$npc_dvergrmage_random_greet4|İnsan mı o?|
|$npc_dvergrmage_random_greet5|Merhaba. Uzun yol gelmişsin.|
|$npc_dvergrmage_random_private_area_alarm1|Hop! Dikkat et.|
|$npc_dvergrmage_random_private_area_alarm2|Kes şunu!|
|$npc_dvergrmage_random_private_area_alarm3|Yapma şunu, dur!|
|$npc_dvergrmage_random_private_area_alarm4|Biz sadece iyi bir misafir olduğun sürece iyi ev sahibi oluruz.|
|$npc_dvergrmage_random_private_area_alarm5|Ziyaretinin süresi dolmak üzere Kuzeyli.|
|$npc_dvergrmage_random_private_area_alarm6|Kes şunu!|
|$npc_dvergrmage_random_private_area_alarm7|Bunu bir daha yaparsan yaşanacaklardan hoşlanmayacaksın.|
|$npc_dvergrmage_random_private_area_alarm8|Tehlikeli sulardasın.|
|$npc_dvergrmage_random_talk_in_faction_base1|I hope this mine isn't as infested as the last.|
|$npc_dvergrmage_random_talk_in_faction_base10|It's damp as a troll's armpit here, but at least it's not as painfully hot as the south.|
|$npc_dvergrmage_random_talk_in_faction_base2|I wonder if we can bring this shipment home soon.|
|$npc_dvergrmage_random_talk_in_faction_base3|We built these mines, you know? And now look at us...|
|$npc_dvergrmage_random_talk_in_faction_base4|Do you ever wonder what happened to Haldor?|
|$npc_dvergrmage_random_talk_in_faction_base5|Sometimes I wonder if the eitr is worth all of this.|
|$npc_dvergrmage_random_talk_in_faction_base6|I look forward to seeing what the fire will show me next.|
|$npc_dvergrmage_random_talk_in_faction_base7|Now where did I put that bottle...?|
|$npc_dvergrmage_random_talk_in_faction_base8|I swear we had a spare Extractor here somewhere...|
|$npc_dvergrmage_random_talk_in_faction_base9|Just a bit more of this mining, and then we can take it back to Myrkheim.|
|$npc_dvergrmage_random_talk1|Umarım bu maden sonuncu gibi istila edilmemiştir.|
|$npc_dvergrmage_random_talk10|Midgard'da hiç büyü olmadığı doğru mu?|
|$npc_dvergrmage_random_talk2|Acaba bu sevkiyatı yakın zamanda eve götürebilecek miyiz?|
|$npc_dvergrmage_random_talk3|Haldor'a ne olduğunu hiç merak ediyor musun?|
|$npc_dvergrmage_random_talk4|Şu arayıcılara bulaşmamak gerek...|
|$npc_dvergrmage_random_talk5|La-di-di, la-di-da...|
|$npc_dvergrmage_random_talk6|Hâlâ hatırlıyorum, eskilerin devlerini...|
|$npc_dvergrmage_random_talk7|Bu madenleri biz inşa ettik, biliyor musun? Şimdi halimize bak...|
|$npc_dvergrmage_random_talk8|Bu haşereler korktuğumdan daha fazla yayılmışlar. Bu hızla giderse eski madenlerimizin tamamını ele geçirecekler.|
|$npc_dvergrmage_random_talk9|İnsan olmak tuhaf olmalı. Küçük yerlerden sürünmen gerektiğini düşünsene!|
|$npc_dvergrrogue_random_aggravated1|İşin bitti!|
|$npc_dvergrrogue_random_aggravated10|Dizilin!|
|$npc_dvergrrogue_random_aggravated11|Yandın sen!|
|$npc_dvergrrogue_random_aggravated2|Kuzeyliyi öldürün!|
|$npc_dvergrrogue_random_aggravated3|Şanslı gününde misin sence?|
|$npc_dvergrrogue_random_aggravated4|Buna pişman olacaksın!|
|$npc_dvergrrogue_random_aggravated5|Son şalgamını ektin!|
|$npc_dvergrrogue_random_aggravated6|Kıçına tekmeyi basacağım!|
|$npc_dvergrrogue_random_aggravated7|Kendi topuğuna sıkmaktan zevk mi alıyorsun sen? Yardımcı olayım!|
|$npc_dvergrrogue_random_aggravated8|Dayak mı istiyorsun sen?|
|$npc_dvergrrogue_random_aggravated9|Bağırsaklarınla yerleri sileceğim!|
|$npc_dvergrrogue_random_goodbye1|Sonra görüşürüz.|
|$npc_dvergrrogue_random_goodbye2|Kendine dikkat et!|
|$npc_dvergrrogue_random_goodbye3|Hoşça kal.|
|$npc_dvergrrogue_random_goodbye4|Kaybolayım deme.|
|$npc_dvergrrogue_random_goodbye5|O peri ışığını yanından ayırma, kaybetmek istemezsin.|
|$npc_dvergrrogue_random_goodbye6|Tettå|
|$npc_dvergrrogue_random_greet1|Kimsin?|
|$npc_dvergrrogue_random_greet2|Kuzeyli.|
|$npc_dvergrrogue_random_greet3|Merhabalar.|
|$npc_dvergrrogue_random_greet4|Selam.|
|$npc_dvergrrogue_random_greet5|Hoş geldin.|
|$npc_dvergrrogue_random_greet6|Åtett.|
|$npc_dvergrrogue_random_private_area_alarm1|Senin yerinde olsam bunu yapmazdım.|
|$npc_dvergrrogue_random_private_area_alarm2|Ellerine hâkim ol!|
|$npc_dvergrrogue_random_private_area_alarm3|Buraya ilk biz gelmiştik!|
|$npc_dvergrrogue_random_private_area_alarm4|Çek ellerini!|
|$npc_dvergrrogue_random_private_area_alarm5|Bir daha dokunursan o elini koparırım.|
|$npc_dvergrrogue_random_private_area_alarm6|Bak seni uyarıyorum!|
|$npc_dvergrrogue_random_private_area_alarm7|Kimse bizimle dalga geçemez!|
|$npc_dvergrrogue_random_private_area_alarm8|Çok dikkatli olsan iyi olur. Yoksa işler senin için kötüye gidecek.|
|$npc_dvergrrogue_random_talk_in_faction_base1|Dizlerim ağrıyor ama en azından yemeğimiz ve ateşimiz var.|
|$npc_dvergrrogue_random_talk_in_faction_base10|I heard we're venturing down south again. Maybe I'll apply for the expedition this time.|
|$npc_dvergrrogue_random_talk_in_faction_base2|Ne ekersen onu biçersin.|
|$npc_dvergrrogue_random_talk_in_faction_base3|Bu küçük hisar evdeki kalelerimize kıyasla hiçbir şey...|
|$npc_dvergrrogue_random_talk_in_faction_base4|Yemek vakti ne zaman? Acıktım.|
|$npc_dvergrrogue_random_talk_in_faction_base5|Biramız bitiyor mu? Galiba biramız bitiyor...|
|$npc_dvergrrogue_random_talk_in_faction_base6|Burada biraz tamirat yapılsa fena olmazdı.|
|$npc_dvergrrogue_random_talk_in_faction_base7|Madenlerden ayrılırken neden her şeyi yanlarına almadıklarını anlamıyorum. Çekirdekleri neden orada bıraktılar ki?|
|$npc_dvergrrogue_random_talk_in_faction_base8|Lanet olası arayıcılar Kara Çekirdeklerimizi parçaladılar! Derinlere dönme zamanı galiba...|
|$npc_dvergrrogue_random_talk_in_faction_base9|Halkımız en son buraya geldiğinden beri bu dünya çok değişti, birkaç yüzyıl sonra nasıl görüneceğini bir düşünsene!|
|$npc_dvergrrogue_random_talk1|Siste nelerle karşılaşacağını asla bilemezsin.|
|$npc_dvergrrogue_random_talk10|Biz buraya ulaşabiliyorsak başka neler ulaşabilir acaba?|
|$npc_dvergrrogue_random_talk2|Tetikte olsan iyi olur.|
|$npc_dvergrrogue_random_talk3|Peki Midgard nasıl? Ben sadece hikâyelerini duydum.|
|$npc_dvergrrogue_random_talk4|Bütün insanlar sana mı benziyor? Sen biraz... Neyse, boş ver.|
|$npc_dvergrrogue_random_talk5|Burada hangi günde olduğunu unutmak kolaydır...|
|$npc_dvergrrogue_random_talk6|Biz cücelerin soydaşlarımıza göz kulak olması gerekiyor.|
|$npc_dvergrrogue_random_talk7|Kahrolası arayıcılar! Ezilip ölseniz keşke hepiniz.|
|$npc_dvergrrogue_random_talk8|Bir keresinde kenenin biri vücudumda neredeyse kan bırakmıyordu... Ama cüceler öyle kolay ölmez.|
|$npc_dvergrrogue_random_talk9|Buraların bir zamanlar büyük bir savaş alanı olduğunu düşünmek biraz tuhaf geliyor...|
|$npc_giveitem|Eşyayı ver|
|$npc_giveitem_no1|Teşekkür ederim ama buna ihtiyacım yok.|
|$npc_giveitem_no10|HAYIR!|
|$npc_giveitem_no11|Hayır demem gerekecek.|
|$npc_giveitem_no12|Buna kesinlikle hayır.|
|$npc_giveitem_no2|Teşekkür ederim ama almayayım.|
|$npc_giveitem_no3|Bununla ne yaparım emin değilim.|
|$npc_giveitem_no4|Ona üç metrelik demirden bir çubukla bile dokunmam.|
|$npc_giveitem_no5|Hayır. Sadece hayır.|
|$npc_giveitem_no6|Aradığım şey bu değil.|
|$npc_giveitem_no7|Bundan daha iyisini yapabilirsin.|
|$npc_giveitem_no8|Sanırım ihtiyaçlarımı yanlış anladın.|
|$npc_giveitem_no9|Hayır. Hayır.|
|$npc_haldor|Haldor|
|$npc_haldor_buy01|Bunlar güzel mallardır, yerden toplayabileceğin cinsten şeyler değiller|
|$npc_haldor_buy02|Parasını ödemeyeceksen dokunma.|
|$npc_haldor_buy03|Mallarıma bakmak serbest ama dokunmak yasak.|
|$npc_haldor_buy04|Buradan alacağın bir şeyle hayatını biraz daha uzatabilirsin. Gerçi ne değiştirecekse.|
|$npc_haldor_goodbye01|Fazla uzaklaşmamaya çalış. Öldürüldüğünde mallarını ben almak istiyorum.|
|$npc_haldor_goodbye02|Yol al. Yapacak işlerim var.|
|$npc_haldor_goodbye03|Kaybol.|
|$npc_haldor_greeting01|Hah! Valkürlerden bir et parçası daha...|
|$npc_haldor_greeting02|Ee?|
|$npc_haldor_greeting03|Ne istiyorsun?|
|$npc_haldor_random_buy1|İyi bir seçim!|
|$npc_haldor_random_buy2|Sana yakıştı!|
|$npc_haldor_random_buy3|Senindir.|
|$npc_haldor_random_buy4|Seninle iş yapmak bir zevk.|
|$npc_haldor_random_buy5|Güle güle kullan.|
|$npc_haldor_random_goodbye1|Yine bekleriz!|
|$npc_haldor_random_goodbye2|Görüşürüz!|
|$npc_haldor_random_goodbye3|Sakın öleyim deme.|
|$npc_haldor_random_greet1|Selam.|
|$npc_haldor_random_greet2|Merhabalar!|
|$npc_haldor_random_greet3|Deyim yerindeyse mekânıma hoş geldin.|
|$npc_haldor_random_greet4|Ah! Bir müşteri!|
|$npc_haldor_random_sell1|Anlaştık.|
|$npc_haldor_random_sell2|Al bakalım.|
|$npc_haldor_random_sell3|Bu biraz altın eder.|
|$npc_haldor_random_sell4|Alacağım.|
|$npc_haldor_random_starttrade1|Mallarıma bir göz at.|
|$npc_haldor_random_starttrade2|Bunlara bir bak!|
|$npc_haldor_random_starttrade3|Neye ihtiyacın varsa eminim elimde vardır.|
|$npc_haldor_random_starttrade4|Her şeyden biraz var elimde.|
|$npc_haldor_random_starttrade5|Mallarım aşırı özel değil ama iş görürler.|
|$npc_haldor_random_talk1|Bir cüceye göre biraz kısayım.|
|$npc_haldor_random_talk2|Radjdadaj...|
|$npc_haldor_random_talk3|Senin türünle daha önce karşılaştım.|
|$npc_haldor_random_talk4|Ne bakıyorsun? Daha önce hiç cüce görmedin mi?|
|$npc_haldor_random_talk5|Halstein, yine mi gaz çıkardın?|
|$npc_haldor_random_talk6|Madencilik çok sıkıcı! Neyse ki girişimcilerle dolu bir aileden geliyorum.|
|$npc_haldor_random_talk7|Süslü şeyler beni pek ilgilenmiyor, o daha çok kız kardeşimin alanı.|
|$npc_haldor_sell01|Bakalım ne hurdalar toplamışsın...|
|$npc_haldor_sell02|Hmm, bakalım burada ne varmış?|
|$npc_haldor_sell03|Acaba taşıdığın bu çer çöpün içinde parlak bir şeyler var mı...|
|$npc_haldor_sell04|Nelerin var göreyim.|
|$npc_haldor_smalltalk01|Burada hepimiz hiçliğe yüz tuttuk. Tanrılar, Valheim'ı uzun süre evvel kaderine terk etti.|
|$npc_haldor_smalltalk02|Yitikler'i avlamak için geldin herhalde? Ne ben onları rahatsız ederim, ne de onlar beni.|
|$npc_haldor_smalltalk03|Savaşmak, inşa etmek, keşfetmek... Hepsinin sonu mezar.|
|$npc_haldor_smalltalk04|Senden önce gelenlerden hiç farkın yok, savaşçı. Sen de onlarla aynı kaderi tadacaksın.|
|$npc_haldor_smalltalk05|Yaratıklarla aramızdaki tek fark, ticaret.|
|$npc_haldor_trade01|Çene çalmakla değil, ticaretle ilgileniyorum. Bir şey alacak mısın bari?|
|$npc_haldor_trade02|Gel, hadi bir anlaşma yapalım.|
|$npc_haldor_trade03|Şansına bu aralar havamdayım. Hadi ticaret yapalım.|
|$npc_hallon|Hallon|
|$npc_halstein|Halstein|
|$npc_hildir|Hildir|
|$npc_hildir_chest1_recieved|Yaşasın, bulmuşsun! Mallarıma bir de şimdi bak!|
|$npc_hildir_chest2_recieved|Bunu bulmak zorlayıcı olmuş olmalı ama buna pişman olmayacaksın!|
|$npc_hildir_chest3_recieved|Bunu geri getirebileceğini düşünmemiştim. Etkilendim!|
|$npc_hildir_random_alreadyhavechest1|Bana bunu zaten vermiştin.|
|$npc_hildir_random_alreadyhavechest2|Güzel taklitmiş. Benim değil ama.|
|$npc_hildir_random_alreadyhavechest3|Sende kalsın. Benim bir işime yaramaz.|
|$npc_hildir_random_alreadyhavechest4|İyi de... bu sandığı bana verdin zaten. Bunu nereden buldun?|
|$npc_hildir_random_alreadyhavechest5|Bu, benim değil... Birisi tarzımı gerçekten beğeniyor anlaşılan.|
|$npc_hildir_random_anylockedchest1|Yardımın için teşekkür ederim, gerçekten minnettarım. Vaktin varsa hâlâ kayıp mallarım var.|
|$npc_hildir_random_anylockedchest2|Burası geçmişte daha barışçıl bir yerdi. Bir kadın her şeyi çalınmadan rahatça yürüyebilirdi mesela.|
|$npc_hildir_random_anylockedchest3|Stokumun bir kısmına yeniden sahip olmak güzel. Hepsini geri alabilirsen gerçekten çok müteşekkir olurum.|
|$npc_hildir_random_anylockedchest4|Sıkıntı olmaz, değil mi? Çalınan mallarımı bulmak yani. Yardımın için teşekkür ederim.|
|$npc_hildir_random_anylockedchest5|Büyük lox'un adı Hallon, küçük olanın da Blåbär.|
|$npc_hildir_random_anylockedchest6|Buraya gelmenin iyi bir fikir olmadığı konusunda beni uyarmıştı. Ama onu dinlemedim ve şimdi de stokumun bir kısmı zindanın tekinde çürüyor.|
|$npc_hildir_random_anylockedchest7|Hakkında söylentiler var, biliyor muydun? Acaba gerçeklikle eşleşiyorlar mı?|
|$npc_hildir_random_anylockedchest8|İlerleme kat ediyorsundur diye umuyorum. Üstünde görmeyi çok istediğim bir kumaş var ama önce onu geri getirmen lazım tabii.|
|$npc_hildir_random_anylockedchest9|En azından dükkân artık tamamen boş değil.|
|$npc_hildir_random_buy1|Aa, bu sana çok yakışacak!|
|$npc_hildir_random_buy2|Bu, en iyi eserlerimden biri.|
|$npc_hildir_random_buy3|Yalnızca senin için dikildi.|
|$npc_hildir_random_buy4|Bunu yaparken çok eğlendim.|
|$npc_hildir_random_buy5|Gerçekten zevklisin.|
|$npc_hildir_random_goodbye1|Umarım yakında yine görüşürüz!|
|$npc_hildir_random_goodbye2|Uğramaktan çekinme.|
|$npc_hildir_random_goodbye3|Başka bir şeye ihtiyacın olursa bir süre buralarda olacağım.|
|$npc_hildir_random_goodbye4|Kendine iyi bak.|
|$npc_hildir_random_greet1|Dükkânıma hoş geldin. Kardeşiminkine kıyasla biraz daha şık.|
|$npc_hildir_random_greet2|Selam.|
|$npc_hildir_random_greet3|Giysilerin biraz iyileştirilebilirmiş gibi duruyorsun.|
|$npc_hildir_random_greet4|Kızlar, bakın kimler gelmiş.|
|$npc_hildir_random_greet5|Selamlar viking.|
|$npc_hildir_random_lockedchest1|Neredeyse bütün mallarım çalındı! İçinde satılacak hiçbir şey olmayan bir dükkân neye yarar?|
|$npc_hildir_random_lockedchest2|Sandıkları bulursan onları kendin açmaya çalışma. Cüce kilitleri maymuncukla açılamaz ve sandıklar, patlamaya karşı bile dayanıklı.|
|$npc_hildir_random_lockedchest3|O kadar el emeğinin senden öylece çalınmasının ne kadar sinir bozucu olduğunu biliyor musun?|
|$npc_hildir_random_lockedchest4|Mallarımı bana geri getirirsen elimdekileri sana indirimli satarım. Ödemen farz et.|
|$npc_hildir_random_lockedchest5|Sandıklarımı bulduğunda bana geri getir. Kilitlerini açacağım, biz de işimize bakacağız.|
|$npc_hildir_random_lockedchest6|Sandıkların artık nerede olduğunu bildiğine göre, sakın ola onları kendine saklamayı düşünme. Doğru anahtar olmadan onları açamazsın.|
|$npc_hildir_random_map1|Çoğu malım benden çalındı. Belki onları bulmama yardım etmek istersin diye haritada yerlerini işaretledim.|
|$npc_hildir_random_map2|Bazılarını geri almama yardım edersen seninle iş yapmaktan mutluluk duyarım.|
|$npc_hildir_random_map3|Gidip kendim alırdım ama daha fazlasını kaybetmeyi risk edemem. Bu kıza bir yardım eli uzatabilir misin?|
|$npc_hildir_random_map4|Kaçabildim ama mallarım o kadar şanslı değildi. En azından olayın nerede olduğunu biliyorum, ne işime yarayacaksa artık.|
|$npc_hildir_random_map5|O ufak açgözlü şerefsizlerin beni pusuya düşürdüğü yer burası. Benim için onların peşinden gitmek istemezsin, değil mi?|
|$npc_hildir_random_sell1|Tabii, alabilirim.|
|$npc_hildir_random_sell2|Hım, bunu belki bir şeylere kullanabilirim.|
|$npc_hildir_random_sell3|Ne derler bilirsin, birinin çöpü başkasının hazinesidir.|
|$npc_hildir_random_sell4|Neden olmasın? Eminim bir ara elden çıkarırım.|
|$npc_hildir_random_sell5|Anlaştık.|
|$npc_hildir_random_starttrade1|Gel, istediğine bir bak.|
|$npc_hildir_random_starttrade2|Eminim elimde hoşuna gidecek bir şeyler vardır.|
|$npc_hildir_random_starttrade3|Eğer solucan ve yumurta arıyorsan yanlış dükkâna geldin.|
|$npc_hildir_random_starttrade4|Beğendiğin bir şey olursa haber ver!|
|$npc_hildir_random_starttrade5|Mallarım olabilecek en iyi kumaştan üretilmiştir.|
|$npc_hildir_random_starttrade6|Emin ol, dükkânımda yalnızca en yüksek kaliteden ürünler bulunur.|
|$npc_hildir_random_talk1|Sorun yok, istiyorsan lox'u sevebilirsin.|
|$npc_hildir_random_talk10|Bu geçici ikametler ne kadar eğlenceli olursa olsun, ailemin yanına eve dönmek güzel olacak.|
|$npc_hildir_random_talk2|Burası geçmişte daha barışçıl bir yerdi. Bir kadın her şeyi çalınmadan rahatça yürüyebilirdi mesela.|
|$npc_hildir_random_talk3|Büyük lox'un adı Hallon, küçük olanın da Blåbär.|
|$npc_hildir_random_talk4|Vagona giremezsin, üzgünüm. Bazı meslek sırları gizli kalmalı.|
|$npc_hildir_random_talk5|Hakkında söylentiler var, biliyor muydun? Acaba gerçeklikle eşleşiyorlar mı?|
|$npc_hildir_random_talk6|Onarım işi yapmıyorum, üzgünüm.|
|$npc_hildir_random_talk7|Hım... Bir gün sana özelleştirilmiş bir parça dikmeyi isterim.|
|$npc_hildir_random_talk8|Geçmişte burada kalıcı bir dükkân açmayı düşünmüştüm. Sorun şu ki, bunun için yeterince müşterim yok.|
|$npc_hildir_random_talk9|Şöyle her şeyi bir göz önünden geçirdim de burası o kadar da kötü değil. Belki gelecek sefere yanımda biriyle gelirim.|
|$npc_hugin|Hugin|
|$npc_munin|Munin|
|$npc_munin_ashlands_bellshard|Kraa! Burada hâlâ üzücü bir anı bulunuyor... Havada kalan sönük bir yankı... Bu çanlar, zamanında onun ölümünü kutlamak için neşeyle çalıyordu ama o şimdi ne sesine ne de hatırasına dayanamıyor. Onu getirmek istiyorsan çanların yeniden çalmasını sağla.|
|$npc_munin_ashlands_general01|Kraa! Buradakiler zavallı artıklar. Alevlerin içinde kavrulmuş boş kafatasları. En ufak bir hatıra bile kalmamış…|
|$npc_munin_ashlands_general02|Merhaba küçük savaşçı! Etrafına bak, burası Zümrüt Alev'in Çorak Toprakları... Ateşin kendini tükettiği bu yerde tüm kibir ayaklar altına alınır... Adımlarına dikkat et!|
|$npc_munin_ashlands_general03|Kraa! Herkesin Babası'ndan bir mesaj getirdim. Şeref, ateşle dövülür. Örse sert vurun!|
|$npc_munin_ashlands_general04|Kraahaha! Bir leşçil kuş dost! Zavallı ölülerin kalıntılarıyla uğraşan şu hâline bir bak...|
|$npc_munin_ashlands_general05|Hmmm... Nasıl hâlâ hayatta olabilirsin savaşçı? Burada, fırında kendini bulmayı mı başardın yoksa? İlginç...|
|$npc_munin_general01|Gaak! Tanıştığımıza sevindim yolgezer... Benim adım Munin, Hugin'in kardeşiyim. Tanrıbaba'nın selamlarını getirdim. Tanrıbaba benim gözümden görür, kelamını dilimde taşırım. Tanrıbaba'nın yolundan sapmazsan seninle tekrar görüşeceğiz…|
|$npc_munin_general02|Gaak! Tüm gözler bunun üzerinde. Büyük gözler, küçük gözler, yukarıdakiler, aşağıdakiler. Ağaçların bile gözleri vardır…|
|$npc_munin_general03|Hâlâ hayatta mısın savaşçı? Cesedinden, çürümüş etinden, ufak bir lokma alır, atıştırmalık yaparım diyordum… Yine de hayatta olman, Tanrıbaba'nın hoşuna gidecek.|
|$npc_munin_general04|Sisin içinde kayıp mı oldun savaşçı? Bir zamanlar bildiğin şeyleri, yürüdüğün yolları ve sevdiğin yüzleri görüyorum. Zavallı savaşçı…|
|$npc_munin_general05|Gaak! Görevini unutma yolgezer! Tanrıbaba'nın seni buraya neden gönderdiğini hatırla. Yitikler yok edilmeli. Unutma!|
|$npc_munin_general06|Hah, yağız bir savaşçı olmuşsun! Nezakettendir, aklında bulunsun. Öldüğünde Hugin sol, ben de sağ gözünü alacağım. Yine de, elbette, bol şans dileriz!|
|$npc_munin_general07|Gaak! Tanrıbaba'ya iyi hizmet edersen anılarını sana geri verebileceğimi söyledi. Bundan önceki hayatını hatırlıyor musun? O seni hatırlıyor...|
|$npc_munin_general08|Bu kadar uzun süre hayatta kalabileceğini düşünmezdim… Lakin kibirden sakın dostum. Savaş meydanları benim şölen soframdır ve her büyük savaşçının sonunu bilirim...|
|$npc_munin_general09|Gak! Bana kulak ver, artık bilmece yok. Ben, anıların koruyucusu, yüce Munin'im. Tanrıbaba'nın sol omzunda oturur, on dünyadaki işlerini görürüm. Pek çok büyük savaşçının yükseliş ve düşüşüne tanıklık ettim ama sen en kudretlilerindensin. Bil ki, bizzat Odin seni izliyor...|
|$npc_munin_general10|Gaak! Tanrıbaba'dan haber var! Ulu köprüyü göndermek için senden haber bekliyor. Yitikler'i öldür ve yolu temizle!|
|$OLD hud_altplace|[OLD] hud_altplace|
|$OLD inventory_stationlevel|[OLD] inventory_stationlevel|
|$OLD inventory_stationlevelreq|[OLD] inventory_stationlevelreq|
|$OLD inventory_upgrade|[OLD] inventory_upgrade|
|$OLD menu_exit|[OLD] menu_exit|
|$online_kickedfromsession_header|Oturumdan atıldın|
|$online_kickedfromsession_suspendresume_xbox_text|Konsolun Uyku moduna geçmesi, ağ etkinliğini duraklattığından çevrim içi oturumdan atıldın.|
|$piece_ancient_pot|Kadim Çömlek|
|$piece_archerytarget|Okçuluk Hedefi|
|$piece_archerytarget_hits|hits|
|$piece_archerytarget_lastscores|İsabetler|
|$piece_archerytarget_reset|Okları al ve sıfırla|
|$piece_archerytarget_total|Toplam|
|$piece_armorstand|Zırh askısı|
|$piece_armorstand_cantattach|Bu eşya oraya sığmıyor|
|$piece_artisan_ext1|Zanaatkâr Presi|
|$piece_artisanstation|Zanaatkâr masası|
|$piece_ashwood_archedwall|Kül Odunu Kemerli Duvar|
|$piece_ashwood_beam_1m|Kül Odunu Kiriş 1 m|
|$piece_ashwood_beam_2m|Kül Odunu Kiriş 2 m|
|$piece_ashwood_bed|Kül Odunu Yatak|
|$piece_ashwood_decowall|Kül Odunu Süs Duvarı|
|$piece_ashwood_decowall_divider|Kül Odunu Bölücü|
|$piece_ashwood_decowall_tree|Kül Odunu Süs Penceresi|
|$piece_ashwood_door|Kül Odunu Kapı|
|$piece_ashwood_floor_1x1|Kül Odunu Zemin 1x1|
|$piece_ashwood_floor_2x2|Kül Odunu Zemin 2x2|
|$piece_ashwood_floor_deco|Kül Odunu Süs Zemini|
|$piece_ashwood_halfwall|Yarım Kül Odunu Duvar|
|$piece_ashwood_pole_1m|Kül Odunu Sütun 1 m|
|$piece_ashwood_pole_2m|Kül Odunu Sütun 2 m|
|$piece_ashwood_quarterwall|Çeyrek Kül Odunu Duvar|
|$piece_ashwood_wall|Kül Odunu Duvar|
|$piece_ashwoodarch_big|Kül Odunu Kemer|
|$piece_ashwoodarch_bottom|Kül Odunu Alt Kemer|
|$piece_ashwoodarch_top|Kül Odunu Tepe Kemeri|
|$piece_ashwoodbeam26|Kül Odunu Kiriş, 26°|
|$piece_ashwoodbeam45|Kül Odunu Kiriş, 45°|
|$piece_ashwoodcross26|Kül Odunu Çatı Haçı, 26°|
|$piece_ashwoodcross45|Kül Odunu Çatı Haçı, 45°|
|$piece_ashwoodstair|Kül Odunu Merdiven|
|$piece_ashwoodwallroof_26|Kül Odunu Duvar Çatısı 26°|
|$piece_ashwoodwallroof_26_upsidedown|Kül Odunu Duvar Çatısı 26° (Ters)|
|$piece_ashwoodwallroof_45|Kül Odunu Duvar Çatısı 45°|
|$piece_ashwoodwallroof_45_upsidedown|Kül Odunu Duvar Çatısı 45° (Ters)|
|$piece_asksvinskeleton|Kül Domuzu İskeleti|
|$piece_banner01|Kara sancak|
|$piece_banner02|Mavi sancak|
|$piece_banner03|Ak ve kızıl çizgili sancak|
|$piece_banner04|Kızıl sancak|
|$piece_banner05|Yeşil sancak|
|$piece_banner06|Mavi|
|$piece_banner07|Beyaz ve mavi şeritli flama|
|$piece_banner08|Sarı sancak|
|$piece_banner09|Mor sancak|
|$piece_banner10|Turuncu sancak|
|$piece_banner11|Beyaz sancak|
|$piece_barber|Berber istasyonu|
|$piece_barber_description|En son viking modasından bir tarz edinmene yardım eder.|
|$piece_bathtub|Küvet|
|$piece_bed|Yatak|
|$piece_bed_claim|Sahiplen|
|$piece_bed_currentspawn|Mevcut canlanma noktası|
|$piece_bed_setspawn|Canlanma noktası belirle|
|$piece_bed_sleep|Uyu|
|$piece_bed_unclaimed|Sahipsiz yatak|
|$piece_bed02|Ejderha yatağı|
|$piece_beehive|Arı kovanı|
|$piece_beehive_area|Arılar bu ortamdan hoşlanmaz|
|$piece_beehive_check|Arılara göz at|
|$piece_beehive_extract|Topla|
|$piece_beehive_freespace|Arılar için daha fazla açık alan gerekli|
|$piece_beehive_happy|Arılar mutlu|
|$piece_beehive_sleep|Arılar uyuyor|
|$piece_bench01|Bank|
|$piece_benchlog|Bank kütük|
|$piece_blackforge|Kara ocak|
|$piece_blackforge_ext1|Kara ocak söndürücü|
|$piece_blackforge_ext2|Mengene|
|$piece_blackforge_ext3|Metal Kesici|
|$piece_blackforge_ext4|Mücevher Kesici|
|$piece_blackmarble_arch|Siyah mermer kemer|
|$piece_blackmarble_base1|Siyah mermer kaide|
|$piece_blackmarble_base2|Siyah mermer geniş kaide|
|$piece_blackmarble_basecorner|Siyah mermer kaide, köşe|
|$piece_blackmarble_bench|Siyah mermer bank|
|$piece_blackmarble_bronze_head1|Tunç kafa 1|
|$piece_blackmarble_bronze_head2|Tunç kafa 2|
|$piece_blackmarble_column_1|Siyah mermer sütun, küçük|
|$piece_blackmarble_column_2|Siyah mermer sütun, geniş|
|$piece_blackmarble_column_3|Siyah mermer sütun, uzun|
|$piece_blackmarble_equilateral_triangle_floor|Siyah mermer zemin, eşkenar üçgen|
|$piece_blackmarble_floor|Siyah mermer zemin|
|$piece_blackmarble_floor_triangle|Siyah mermer zemin, üçgen|
|$piece_blackmarble_floor4x4|Siyah mermer zemin, 4x4|
|$piece_blackmarble_head_big1|Siyah mermer büyük kafa 1|
|$piece_blackmarble_head_big2|Siyah mermer büyük kafa 2|
|$piece_blackmarble_inverted_slope1x2|Siyah mermer eğim, ters|
|$piece_blackmarble_out1|Siyah mermer korniş|
|$piece_blackmarble_out2|Siyah mermer korniş, geniş|
|$piece_blackmarble_outcorner|Siyah mermer korniş, köşe|
|$piece_blackmarble_slope1x2|Siyah mermer eğim|
|$piece_blackmarble_stair|Siyah mermer merdiven|
|$piece_blackmarble_stair_corner_left|Siyah mermer merdiven, sol|
|$piece_blackmarble_stair_corner_right|Siyah mermer merdiven, sağ|
|$piece_blackmarble_table|Siyah mermer masa|
|$piece_blackmarble_throne|Siyah mermer taht|
|$piece_blackmarble_tile_floor_1x1|Siyah mermer zemin fayansı, küçük|
|$piece_blackmarble_tile_floor_2x2|Siyah mermer zemin fayansı, büyük|
|$piece_blackmarble_tile_wall_1x1|Siyah mermer duvar fayansı, küçük|
|$piece_blackmarble_tile_wall_2x2|Siyah mermer duvar fayansı, büyük|
|$piece_blackmarble_tile_wall_2x4|Siyah mermer duvar fayansı, uzun|
|$piece_blackmarble_tip|Siyah mermer köşe külahı|
|$piece_blackmarble1x1|Siyah mermer 1x1x1|
|$piece_blackmarble2x1x1|Siyah mermer 2x1x1|
|$piece_blackmarble2x2_enforced|Sağlam siyah mermer|
|$piece_blackmarble2x2x1|Siyah mermer 2x2x1|
|$piece_blackmarble2x2x2|Siyah mermer 2x2x2|
|$piece_blackwoodbench01|Kül Odunu Bank|
|$piece_blackwoodstack|Kül Odunu Yığını|
|$piece_BlackwoodStakewall|Kül Odunu Kazık Duvar|
|$piece_blastfurnace|Yüksek fırın|
|$piece_bone_throne|Kemik Taht|
|$piece_bonestack|Kemik Yığını|
|$piece_bonfire|Büyük Ateş|
|$piece_brazierceiling01|Asılı mangal|
|$piece_brazierfloor01|Ayaklı mangal|
|$piece_brazierfloor02|Ayaklı mavi mangal|
|$piece_candle|Reçine Mumu|
|$piece_cartographytable|Haritacılık masası|
|$piece_catapult_legsdown|Ayakları indir|
|$piece_catapult_legsup|Ayakları kaldır|
|$piece_catapult_placeitem|Eşyayı yerleştir|
|$piece_catapult_wontfit|Bu öge mancınığa sığmıyor|
|$piece_cauldron|Kazan|
|$piece_cauldron_ext1_spice|Baharatlık|
|$piece_cauldron_ext3_butchertable|Et kesme masası|
|$piece_cauldron_ext4_pans|Çanak çömlek|
|$piece_cauldron_ext5_mortarandpestle|Havan ve tokmak|
|$piece_cauldron_ext6_rollingpins|Oklavalar ve Kesme Tahtaları|
|$piece_celebrationgarland|Çiçek Çelengi|
|$piece_chair|Sandalye|
|$piece_charcoalkiln|Kömür fırını|
|$piece_charredballista|Gölg|
|$piece_charredchest|Kor Sandık|
|$piece_chest|Güçlendirilmiş sandık|
|$piece_chestbarrel|Fıçı|
|$piece_chestblackmetal|Kara metal sandık|
|$piece_chestprivate|Kişisel sandık|
|$piece_chesttreasure|Hazine sandığı|
|$piece_chestwood|Sandık|
|$piece_clothdoor|Kırmızı jüt perde|
|$piece_coalpile|Kömür öbeği|
|$piece_container_empty|BOŞ|
|$piece_container_open|Aç|
|$piece_cookingstation|Pişirme istasyonu|
|$piece_cookingstation_iron|Demir pişirme istasyonu|
|$piece_craftingstation|Atölye|
|$piece_crate|Kasa|
|$piece_crate_long|Cüce malzeme kasası|
|$piece_crystalwall1x1|Kristal duvar 1x1|
|$piece_cstand_cook|Eşyayı pişir|
|$piece_cultivate|İşle|
|$piece_darkwoodarch|Kara ahşap kemeri|
|$piece_darkwoodbeam|Kara ahşap kiriş 2m|
|$piece_darkwoodbeam_26|Kara ahşap kiriş 26°|
|$piece_darkwoodbeam_45|Kara ahşap kiriş 45°|
|$piece_darkwoodbeam4|Kara ahşap kiriş 4m|
|$piece_darkwoodchair|Kara ahşap sandalye|
|$piece_darkwooddecowall|Oymalı Kara Ahşap bölücü|
|$piece_darkwoodgate|Kara ahşap kapı|
|$piece_darkwoodpole|Kara ahşap direk 2m|
|$piece_darkwoodpole4|Kara ahşap direk 4m|
|$piece_darkwoodraven|Kuzgun dekoru|
|$piece_darkwoodroof26|Padavra çatı 26°|
|$piece_darkwoodroof45|Padavra çatı 45°|
|$piece_darkwoodrooficorner|Padavra çatı iç köşe 26°|
|$piece_darkwoodrooficorner45|Padavra çatı iç köşe 45°|
|$piece_darkwoodroofocorner|Padavra çatı dış köşe 26°|
|$piece_darkwoodroofocorner45|Padavra çatı dış köşe 45°|
|$piece_darkwoodrooftop|Padavra çatı sırtı 26°|
|$piece_darkwoodrooftop45|Padavra çatı sırtı 45°|
|$piece_darkwoodwolf|Kurt dekoru|
|$piece_deposit_copper|Bakır yatağı|
|$piece_deposit_obsidian|Obsidiyen yatağı|
|$piece_deposit_silver|Gümüş yatağı|
|$piece_deposit_silvervein|Gümüş yatağı|
|$piece_deposit_tin|Kalay yatağı|
|$piece_door_close|Kapat|
|$piece_door_open|Aç|
|$piece_dv_gate|Eski cüce kapısı|
|$piece_dvergr_door|Cüce kapısı|
|$piece_dvergr_lantern|Cüce duvar feneri|
|$piece_dvergr_lantern_pole|Cüce sokak lambası|
|$piece_dvergr_metal_wall|Cüce metal duvarı|
|$piece_dvergr_pole|Cüce direği|
|$piece_dvergr_sharpstakes|Cüce sivri kazıkları|
|$piece_dvergr_spiralstair|Cüce sarmal sol merdiven|
|$piece_dvergr_spiralstair_right|Cüce sarmal sağ merdiven|
|$piece_dvergr_stake_wall|Cüce kazık duvarı|
|$piece_dvergr_woodwall|Cüce duvarı|
|$piece_dvergrchest|Cüce hazine sandığı|
|$piece_eitrrefinery|Eitr rafinerisi|
|$piece_fairylightgarland|Süs Işıkları|
|$piece_fermenter|Mayalandırıcı|
|$piece_fermenter_add|Eşya ekle|
|$piece_fermenter_exposed|Çok açıkta|
|$piece_fermenter_fermenting|Mayalanıyor|
|$piece_fermenter_needroof|Requires a roof|
|$piece_fermenter_ready|Tamam|
|$piece_fermenter_tap|Dök|
|$piece_fire|Ateş|
|$piece_fire_fuel|Yakıt|
|$piece_firepit|Kamp Ateşi|
|$piece_firepit_iron|Demir ateş çukuru|
|$piece_flametal_beam|Flametal Beam|
|$piece_flametal_pillar|Flametal Pillar|
|$piece_flametalgate|Oddemir Kapı|
|$piece_forge|Ocak|
|$piece_forge_ext1|Körük|
|$piece_forge_ext2|Örsler|
|$piece_forge_ext3|Biley taşı|
|$piece_forge_ext4|Demirci örsü|
|$piece_forge_ext5|Ocak soğutucu|
|$piece_forge_ext6|Ocak aleti askılığı|
|$piece_giant_bone|Taşlaşmış kemik|
|$piece_giant_brain|Yumuşak doku|
|$piece_giant_helmet|Kadim zırh|
|$piece_giant_sword|Kadim kılıç|
|$piece_grausten_archmedium|Gritaş Orta Boy Kemer|
|$piece_grausten_archsmall|Gritaş Küçük Kemer|
|$piece_grausten_beammedium|Gritaş Orta Boy Kiriş|
|$piece_grausten_beamsmall|Gritaş Küçük Kiriş|
|$piece_grausten_floor1x1|Gritaş Zemin 1x1|
|$piece_grausten_floor2x2|Gritaş Zemin 2x2|
|$piece_grausten_floor4x4|Gritaş Zemin 4x4|
|$piece_grausten_pillarmedium|Gritaş Orta Boy Sütun|
|$piece_grausten_pillarsmall|Gritaş Küçük Sütun|
|$piece_grausten_pillartapered|Gritaş Konik Sütun|
|$piece_grausten_pillartaperedinverted|Gritaş Konik Sütun (Ters)|
|$piece_grausten_roof45|Gritaş Çatı|
|$piece_grausten_roof45_arch|Gritaş Kemerli Çatı|
|$piece_grausten_roof45_archcorner|Gritaş Kemerli Çatı Köşesi|
|$piece_grausten_roof45_archcorner2|Gritaş Kemerli Çatı Köşesi|
|$piece_grausten_roof45_corner|Gritaş Çatı Köşesi|
|$piece_grausten_roof45_corner2|Gritaş Çatı Köşesi|
|$piece_grausten_stair|Gritaş Merdiven|
|$piece_grausten_stoneladder|Gritaş Dik Merdiven|
|$piece_grausten_wall1x2|Gritaş Duvar 1x2|
|$piece_grausten_wall2x2|Gritaş Duvar 2x2|
|$piece_grausten_wall4x2|Gritaş Duvar 4x2|
|$piece_grausten_wallarch|Gritaş Duvar Kemeri|
|$piece_grausten_wallarchinv|Gritaş Ters Duvar Kemeri|
|$piece_grausten_window2x2|Gritaş Pencere 2x2|
|$piece_grausten_window4x2|Gritaş Pencere 4x2|
|$piece_graustenpile|Gritaş Yığını|
|$piece_groundtorch|Sabit demir meşale|
|$piece_groundtorchblue|Mavi renkte yanan sabit demir meşale|
|$piece_groundtorchdemister|Peri meşalesi|
|$piece_groundtorchgreen|Yeşil renkte yanan sabit demir meşale|
|$piece_groundtorchwood|Sabit ahşap meşale|
|$piece_guardstone|Nazarlık|
|$piece_guardstone_activate|Etkinleştir|
|$piece_guardstone_active|Aktif|
|$piece_guardstone_add|Seç|
|$piece_guardstone_additional|İzin verildi|
|$piece_guardstone_deactivate|Devre dışı bırak|
|$piece_guardstone_description|Yakın çevreye büyülü bir mühür yayarak diğer oyuncuların yapı inşa etmesine ya da kapı açmasına engel olur.|
|$piece_guardstone_inactive|Pasif|
|$piece_guardstone_owner|Sahibi|
|$piece_guardstone_remove|Vazgeç|
|$piece_hanging_cloth_blue1|Mavi jüt güneşlik|
|$piece_hanging_cloth_blue2|Mavi jüt perde|
|$piece_harvest|Topla|
|$piece_hearth|Ocak tabanı|
|$piece_hexagonalgate|Altıgen kapı|
|$piece_horizontal|Yatay|
|$piece_incinerator|Yok edici|
|$piece_incinerator_conversion|Thor sana bir hediye bahşediyor!|
|$piece_incinerator_empty|Thor seni onaylamıyor|
|$piece_incinerator_fail|Yok edici kullanılıyor|
|$piece_incinerator_success|Eşyalar yok edildi!|
|$piece_ironfloor|Kafes zemin 2x2|
|$piece_ironfloorSmall|Kafes zemin 1x1|
|$piece_irongate|Demir kapı|
|$piece_ironwall|Kafes duvar 2x2|
|$piece_ironwallSmall|Kafes duvar 1x1|
|$piece_itemstand|Eşya ayaklığı|
|$piece_itemstand_attach|Eşyayı bağla|
|$piece_itemstand_cantattach|Bu eşyayı bağlayamazsın|
|$piece_itemstand_missingitem|Gerekli eşya eksik|
|$piece_itemstand_take|Eşyayı al|
|$piece_jackoturnip|Turp Fener|
|$piece_jute_carpet|Kırmızı jüt halı|
|$piece_juteblue_carpet|Mavi jüt kilim|
|$piece_lavalantern|Lav Feneri|
|$piece_levelground|Toprağı düzle|
|$piece_logbeam2|Kütük kiriş 2m|
|$piece_logbeam4|Kütük kiriş 4m|
|$piece_logpole2|Kütük direk 2m|
|$piece_logpole4|Kütük direk 4m|
|$piece_lorestone|Rün Taşı|
|$piece_magetable|Galdr masası|
|$piece_magetable_ext|Efsun masası|
|$piece_magetable_ext2|Sönmeyen mumlar|
|$piece_magetable_ext3|Tüylü Çelenk|
|$piece_maptable_hildir|Hildir'in harita masası|
|$piece_maptable_hildir_use|Sandık konumlarını kaydet|
|$piece_marblepile|Siyah mermer yığını|
|$piece_maypole|Çiçekli Direk|
|$piece_meadcauldron|Mead Ketill|
|$piece_mistletoe|Ökseotu|
|$piece_mudpile|Çamurlu hurda yığını|
|$piece_noaccess|Erişim yok|
|$piece_offerbowl_bonemass|Kavurucu ecel|
|$piece_offerbowl_eikthyr|Gizemli sunak|
|$piece_offerbowl_fader|Zümrüt Alev Sunağı|
|$piece_offerbowl_hive|Kovan sunağı|
|$piece_offerbowl_makeoffer|Adak ada|
|$piece_offerbowl_offeritem|Eşya ada|
|$piece_offerbowl_offeritem_fader|Çanları çal|
|$piece_offerbowl_yagluth|Gizemli sunak|
|$piece_oven|Taş fırın|
|$piece_oven_additem|Pişirme eşyası|
|$piece_path|Yol yap|
|$piece_pavedroad|Döşenmiş yol|
|$piece_plant_healthy|Sağlıklı|
|$piece_plant_nospace|Yetiştirebilmek için daha fazla alan gerekiyor|
|$piece_plant_nosun|Üstü açık bir yer gerekiyor|
|$piece_plant_notcultivated|İşlenmiş toprak gerekiyor|
|$piece_plant_nowall|Yakınında duvar bulunmalı|
|$piece_plant_toocold|Çok soğuk|
|$piece_plant_toohot|Çok sıcak|
|$piece_plant_wrongbiome|Bu ortamda yetişemez|
|$piece_portal|Geçit|
|$piece_portal_connected|BAĞLI|
|$piece_portal_description|Adsız ya da aynı ada sahip diğer geçitlere bağlanır.|
|$piece_portal_settag|Ad belirle|
|$piece_portal_stone|Geçit - Taş|
|$piece_portal_stone_description|Kuvvetli enerji kaynağı en değerli eşyalarla bile içinden geçmene izin veriyor.|
|$piece_portal_tag|Ad|
|$piece_portal_unconnected|BAĞLI DEĞİL|
|$piece_pot_large_cracked|Büyük Kil Çömlek|
|$piece_pot_large_green|Büyük Yeşil Çömlek|
|$piece_pot_large_red|Büyük Kırmızı Çömlek|
|$piece_pot_medium_cracked|Orta Boy Kil Çömlek|
|$piece_pot_medium_green|Orta Boy Yeşil Çömlek|
|$piece_pot_medium_red|Orta Boy Kırmızı Çömlek|
|$piece_pot_small_cracked|Küçük Kil Çömlek|
|$piece_pot_small_green|Küçük Yeşil Çömlek|
|$piece_pot_small_red|Küçük Kırmızı Çömlek|
|$piece_preptable|Food Preparation Table|
|$piece_pulllever|Kolu çek|
|$piece_queendoor|Cüce kasası|
|$piece_raise|Zemini yükselt|
|$piece_readmap|Haritayı oku|
|$piece_register_location|Konumu kaydet|
|$piece_repair|Onar|
|$piece_replant|Çimen|
|$piece_rug_asksvin|Kül Domuzu Kilimi|
|$piece_rug_bjorn|Ayıderisi Kilim|
|$piece_rug_deer|Geyik postu kilim|
|$piece_rug_hare|Yabantavşanı kilimi|
|$piece_rug_lox|Lox postu kilim|
|$piece_rug_straw|Hasır|
|$piece_rug_wolf|Kurt postu kilim|
|$piece_rune_read|Oku|
|$piece_sapcollector|Özsu çıkarıcı|
|$piece_sapcollector_description|Gizemli dallardan özsu çıkar.|
|$piece_sapcollector_draining|Çıkarılıyor|
|$piece_sapcollector_drainingslow|Yavaşça çıkarılıyor|
|$piece_sapcollector_extract|Topla|
|$piece_sapcollector_isfull|Dolu|
|$piece_sapcollector_notconnected|Çıkarılacak bir şey yok|
|$piece_sapling_barley|Arpa|
|$piece_sapling_carrot|Havuç|
|$piece_sapling_carrot_description|Havuç yetiştirmek için havuç tohumları ek.|
|$piece_sapling_flax|Keten|
|$piece_sapling_jotunpuffs|Jötunn puf tomurcukları|
|$piece_sapling_magecap|Büyücü mantarı tomurcukları|
|$piece_sapling_onion|Soğan|
|$piece_sapling_seedcarrot|Havuç tohumu|
|$piece_sapling_seedcarrot_description|Daha fazla tohum elde etmek için havuç ek.|
|$piece_sapling_seedonion|Soğan filizi|
|$piece_sapling_seedonion_description|Daha çok tohum almak için soğan tohumu ek.|
|$piece_sapling_seedturnip|Şalgam tohumu|
|$piece_sapling_seedturnip_description|Daha fazla tohum elde etmek için şalgam ek.|
|$piece_sapling_turnip|Şalgam|
|$piece_sapling_vineash|Kül Asması|
|$piece_sapling_vineash_description|Tutunabileceği bir şey gerekiyor.|
|$piece_sapling_vinegreen|Sarmaşık|
|$piece_sapling_vinegreen_description|Tutunabileceği bir şey gerekiyor.|
|$piece_sconce|Meşalelik|
|$piece_secretdoor|Gizli kapı|
|$piece_sharpstakes|Sivri kazıklar|
|$piece_shieldgenerator|Kalkan Oluşturucu|
|$piece_shieldgenerator_add|Kemik ekle|
|$piece_shieldgenerator_charging|Yükleniyor|
|$piece_shieldgenerator_description|Hava koşullarından ve menzilli saldırılardan koruması için bir kalkan oluşturur. Yakıt olarak tüm kemik türleri kullanılabilir.|
|$piece_shieldgenerator_fuelname|kemikler|
|$piece_shieldgenerator_ready|Dolduruldu ve hazır|
|$piece_shieldgenerator_use|Etkinleştir|
|$piece_shieldgenerator_waiting|Yakıt gerekiyor|
|$piece_ship_ladder|Merdiven|
|$piece_ship_rudder|Dümeni kullan|
|$piece_sign|Tabela|
|$piece_sign_input|Metin girin|
|$piece_skullpile|Kafatası Yığını|
|$piece_smelter|Eritme fırını|
|$piece_smelter_add|Ekle|
|$piece_smelter_additem|Eşya ekle|
|$piece_smelter_empty|Boş|
|$piece_smelter_ready|İşlendi|
|$piece_smelter_reqroof|Çatı gerekiyor|
|$piece_spinningwheel|Çıkrık|
|$piece_stakewall|Kazık duvar|
|$piece_stonearch|Taş kemer|
|$piece_stonecutter|Taş kesici|
|$piece_stonefloor2x2|Taş zemin 2x2|
|$piece_stonefloor4x4|Taş zemin 4x4|
|$piece_stonepile|Taş yığını|
|$piece_stonepillar|Taş sütun|
|$piece_stonestair|Taş basamaklar|
|$piece_stonethrone|Taş taht|
|$piece_stonewall1x1|Taş duvar 1x1|
|$piece_stonewall2x1|Taş duvar 2x1|
|$piece_stonewall4x2|Taş duvar 4x2|
|$piece_stool|Tabure|
|$piece_table|Masa|
|$piece_table_oak|Uzun ağır masa|
|$piece_table_round|Yuvarlak masa|
|$piece_tarpile|Katran varili|
|$piece_throne01|Kuzgun tahtı|
|$piece_tombstone|Burada yatan:|
|$piece_tombstone_container|Grave|
|$piece_tombstone_recovered|Eşyalar kurtarıldı|
|$piece_toofar|Çok uzak|
|$piece_trainingdummy|Savaş Kardeşi|
|$piece_trap|Tuzak|
|$piece_trap_arm|Tuzak kur|
|$piece_trap_armed|Kurulu|
|$piece_trap_cooldown|Trol tuzağı henüz hazır değil|
|$piece_treasure_pile|Sikke öbeği|
|$piece_treasure_stack|Sikke yığını|
|$piece_turret|Balista|
|$piece_turret_addammo|Balista oku ekle|
|$piece_turret_armed|Dolduruldu ve hazır|
|$piece_turret_description|Yoluna çıkan her şeye balista oku fırlatan bir savunma aleti.|
|$piece_turret_noammo|Mühimmat yok|
|$piece_turret_target|Hedef:|
|$piece_turret_target_everything|her şey|
|$piece_turret_target_set|Av ganimeti ile hedef belirle|
|$piece_turret_target_set_msg|Balista hedefleri|
|$piece_UnstableMine|Dengesiz Mayın|
|$piece_upgrade|geliştirme|
|$piece_use|Kullan|
|$piece_useitem|Eşyayı kullan|
|$piece_vegvisir|Vegvisir|
|$piece_vertical|Dikey|
|$piece_windmill|Yel değirmeni|
|$piece_wisplure|Peri çeşmesi|
|$piece_wisplure_description|Perileri çeker, genellikle geceleri gelirler... genellikle.|
|$piece_wisplure_full|Periler dans ediyor|
|$piece_wisplure_light|Periler karanlık talep ediyor|
|$piece_wisplure_nospace|Periler daha fazla açık alan istiyor|
|$piece_wisplure_ok|Periler geliyor|
|$piece_woodbeam1|Ahşap kiriş 1m|
|$piece_woodbeam2|Ahşap kiriş 2m|
|$piece_woodbeam26|Ahşap kiriş 26°|
|$piece_woodbeam45|Ahşap kiriş 45°|
|$piece_woodcorestack|Kütük ahşap yığını|
|$piece_wooddoor|Ahşap kapı|
|$piece_wooddragon|Ahşap ejder başı|
|$piece_woodfence|Yuvarlak direk çit|
|$piece_woodfinestack|Kaliteli ahşap yığını|
|$piece_woodfloor1x1|Ahşap zemin 1x1|
|$piece_woodfloor2x2|Ahşap zemin 2x2|
|$piece_woodgate|Ahşap kapı|
|$piece_woodironbeam|Demir destekli ahşap kiriş|
|$piece_woodironbeam_26|Demir destekli ahşap kiriş 26°|
|$piece_woodironbeam_45|Demir destekli ahşap kiriş 45°|
|$piece_woodironpole|Demir destekli ahşap direk|
|$piece_woodledge|Ahşap çıkıntı|
|$piece_woodlog26|Kütük kiriş 26°|
|$piece_woodlog45|Kütük kiriş 45°|
|$piece_woodpole|Ahşap direk 1m|
|$piece_woodpole2|Ahşap direk 2m|
|$piece_woodroof26|Saman çatı 26°|
|$piece_woodroof45|Saman çatı 45°|
|$piece_woodrooficorner|Saman çatı iç köşe 26°|
|$piece_woodrooficorner45|Saman çatı iç köşe 45°|
|$piece_woodroofocorner|Saman çatı dış köşe 26°|
|$piece_woodroofocorner45|Saman çatı dış köşe 45°|
|$piece_woodrooftop|Saman çatı sırtı 26°|
|$piece_woodrooftop45|Saman çatı sırtı 45°|
|$piece_woodstack|Ahşap yığını|
|$piece_woodstair|Ahşap basamaklar|
|$piece_woodstepladder|Ahşap merdiven|
|$piece_woodwall|Ahşap duvar|
|$piece_woodwallhalf|Yarım ahşap duvar|
|$piece_woodwallquarter|Ahşap duvar 1x1|
|$piece_woodwallroof|Ahşap duvar 26°|
|$piece_woodwallroof_upsidedown|Ahşap duvar 26° (ters)|
|$piece_woodwallroof45|Ahşap duvar 45°|
|$piece_woodwallroof45_upsidedown|Ahşap duvar 45° (ters)|
|$piece_woodwallrooftop|Çapraz ahşap çatı kirişi 26°|
|$piece_woodwallrooftop45|Çapraz ahşap çatı kirişi 45°|
|$piece_woodwindowshutter|Ahşap panjur|
|$piece_workbench|Tezgâh|
|$piece_workbench_ext1|Doğrama kütüğü|
|$piece_workbench_ext2|Tabaklama askılığı|
|$piece_workbench_ext3|Keser|
|$piece_workbench_ext4|Alet rafı|
|$piece_writemap|Keşifleri kaydet|
|$piece_yggdrasilstack|Yggdrasil odunu yığını|
|$piece_yulecrown|Yule çelengi|
|$piece_yulegarland|Yule askısı|
|$piece_yuleklapp|Yule hediyesi|
|$piece_yuletree|Yule ağacı|
|$placeofmystery|Gizemli Konum|
|$playfab_couldnotloadplayfabparty_header|PlayFab could not load|
|$playfab_couldnotloadplayfabparty_text_linux|It is necessary for using crossplay. Make sure you have installed the dependencies necessary for PlayFab Party on Linux, listed here: https://learn.microsoft.com/en-us/gaming/playfab/features/multiplayer/networking/linux-specific-requirements|
|$playfab_couldnotloadplayfabparty_text_linux_steamdeck|It is necessary for using crossplay. Until a fix is available, you can use the Proton Compatibility layer on your Steam Deck to access crossplay features.|
|$playfab_couldnotloadplayfabparty_text_otherplatforms|It is necessary for using crossplay. If this error persists, please report it as a bug.|
|$playfab_couldnotloadplayfabparty_text_windows|It is necessary for using crossplay. This is usually because you do not have Microsoft Visual C++ redistributables installed. Download here: https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist|
|$prop_ancienttree|Kadim ağaç|
|$prop_ancienttreeroot|Kadim kök|
|$prop_ashlandstree|Kor Ağaç|
|$prop_beech|Akgüren|
|$prop_beech_sapling|Akgüren fidanı|
|$prop_birch|Huş|
|$prop_birch_sapling|Huş Fidanı|
|$prop_dragonsummoningbowl_name|Kurban sunağı|
|$prop_eggcup|Adak kâsesi|
|$prop_eldersummoningbowl_name|Kadim Kâse|
|$prop_eldersummoningbowl_use|Eşyayı yak|
|$prop_fir|Köknar|
|$prop_fir_sapling|Köknar fidanı|
|$prop_oak|Meşe|
|$prop_oak_sapling|Meşe Fidanı|
|$prop_offerbowl_makeoffer|Adak sun|
|$prop_pine|Çam|
|$prop_pine_sapling|Çam fidanı|
|$prop_treelog|Kütük|
|$prop_treestump|Ağaç kökü|
|$prop_yggashoot|Yggdrasil filizi|
|$radial_all|All|
|$radial_armor_utility|Armor & Utilities|
|$radial_back|Geri|
|$radial_close|Kapat|
|$radial_consumables|Consumables|
|$radial_drop|Drop|
|$radial_drop_multiple|Drop Multiple|
|$radial_emotes|İfadeler|
|$radial_half|Half|
|$radial_hammer|Equip Hammer|
|$radial_handitems|Hand Items|
|$radial_hold|(Basılı Tut)|
|$radial_hotbar|Hotbar|
|$radial_interact|Kullan/Aç|
|$radial_speed_fast|Hızlı|
|$radial_speed_medium|Orta|
|$radial_speed_off|Kapalı|
|$radial_speed_slow|Yavaş|
|$radial_weapons_tools|Weapons & Tools|
|$raven_interact|Konuş|
|$rock_abyssalbarnacle|Abyssal Barnacle|
|$rotate_build_mode|Çevir (İnşa Modu)|
|$se_adrenaline|Adrenalin kullanımı|
|$se_adrenaline_upfront|Adrenaline gain|
|$se_adrenalinerush|Adrenalin Patlaması|
|$se_adrenalinerush_tooltip|Adrenalini damarlarında hissediyorsun|
|$se_ashlandsmediumarmorseteffect_name|Ask's Endurance|
|$se_ashlandsmediumarmorseteffect_tooltip|A lighter armour lets you move more freely, every move requiring less energy.|
|$se_attackstamina|Saldırı enerji tüketimi|
|$se_beltstrength_start|Daha kuvvetli hissediyorsun|
|$se_beltstrength_tooltip|Azami ağırlık taşıma kapasitesi arttı.|
|$se_berserker_undead_effect_name|Menfurkemiği Öfke|
|$se_berserker_undead_effect_tooltip|Hasarı ve yenilenmeyi artırır fakat seni fiziksel hasara karşı zayıflatır.|
|$se_berserkereffect_name|Cinnet|
|$se_berserkerteffect_tooltip|Hasarı ve yenilenmeyi artırır fakat seni fiziksel hasara karşı zayıflatır.|
|$se_blockstamina|Bloklama enerji tüketimi|
|$se_blockstaminaflat|Enerji kullanımı|
|$se_blockstaminaflat_minus|Block stamina return|
|$se_bonemass_name|Kemik kütlesi|
|$se_bonemass_start|Kütlekemik'in ruhu artık içinde yaşıyor.|
|$se_bonemass_tooltip|Fiziksel hasara karşı direnç.|
|$se_burning_name|Yanıyorsun|
|$se_burning_start|Yanıyorsun|
|$se_burning_tooltip|Alev aldın!|
|$se_cold_name|Soğuk|
|$se_cold_repeat|Üşüdün|
|$se_cold_start|Üşüyorsun|
|$se_cold_stop|Isınmaya başladın|
|$se_cold_tooltip|Enerji ve sağlık yenilenmesi zayıf.|
|$se_coldres_name|Soğuk direnci|
|$se_corpserun_name|İskelet atikliği|
|$se_corpserun_tooltip|Daha hızlı koşup fiziksel saldırılardan büyük ölçüde az hasar alıyorsun.|
|$se_cozy_name|Rahat|
|$se_cozy_start|Rahat hissediyorsun|
|$se_demister_start|Sadık periyi serbest bıraktın|
|$se_dodgestamina|Kaçınma enerji tüketimi|
|$se_eikthyr_name|Eikthyr|
|$se_eikthyr_start|Eikthyr'in gücünü etkinleştirdin.|
|$se_eikthyr_tooltip|Koşma ve zıplama kabiliyetin gelişti.|
|$se_eitr|Eitr|
|$se_eitr_upfront|Eitr kazanımı|
|$se_eitrregen|Eitr yenilenmesi|
|$se_encumbered_name|Aşırı yük|
|$se_encumbered_repeat|Çok fazla şey taşıyorsun|
|$se_encumbered_start|Çok fazla şey taşıyorsun|
|$se_encumbered_tooltip|Aşırı yükten dolayı koşamıyorsun.|
|$se_fader_name|Sönümleyici gücü|
|$se_fader_start|Zümrüt Alev hâlâ yanıyor|
|$se_fader_tooltip|Kuvveti ve hızı artırır.|
|$se_fenringseteffect_name|Fenris lütfu|
|$se_fenringseteffect_tooltip|Fenris zırhı ateşten geçebilmen için ayaklarını hızlandırır ve yumrukların yaratığın gücünü hisseder.|
|$se_fire_name|Ateş|
|$se_fire_tooltip|İçini ısıtan bir ateş.|
|$se_fishinghat_description|Vikingler seni istiyor, balıklar senden korkuyor.|
|$se_freezing_name|Donuyorsun|
|$se_freezing_repeat|Donuyorsun|
|$se_freezing_start|Donuyorsun|
|$se_freezing_stop|Isınmaya başladın|
|$se_freezing_tooltip|Donuyorsun, enerji yenilenmesi çok zayıf ve can yenilenmesi durdu.|
|$se_frost_name|Buz|
|$se_frost_tooltip|Hipotermi geçiriyorsun.|
|$se_frostres_name|Donma direnci|
|$se_frostres_tooltip|Soğuğa karşı korunuyorsun.|
|$se_harpooned_name|Zıpkınlandın|
|$se_harpooned_tooltip|Zıpkın yedin.|
|$se_harvesterseteffect_name|Hasatçı|
|$se_harvesterseteffect_tooltip|Doğru kıyafetleri giymek günlük işleri çok daha kolay hâle getirir.|
|$se_health|Sağlık|
|$se_health_upfront|Can kazanımı|
|$se_healthpotionmedium_name|Sağlık iksiri|
|$se_healthpotionminor_name|Zayıf sağlık iksiri|
|$se_healthregen|Sağlık yenileme|
|$se_healthupgrade_name|Sağlık geliştirmesi|
|$se_healthupgrade_stop|Daha kuvvetli hissediyorsun|
|$se_immobilized|Hareketsiz|
|$se_immobilized_tooltip|Sıkıştın|
|$se_jumpheight|Zıplama yüksekliği|
|$se_jumplength|Zıplama mesafesi|
|$se_jumpstamina|Zıplama enerji tüketimi|
|$se_lightfooteffect_name|Çeviklik|
|$se_lightning_name|Şimşek|
|$se_lightning_tooltip|Şimşek hasarı.|
|$se_mageseteffect_name|Eitr enerjili|
|$se_mageseteffect_tooltip|Eitr ile birleştin (Arttırılmış yenilenme + Element Büyüsü yeteneği)|
|$se_max_carryweight|Azami taşıma ağırlığı|
|$se_mead_name|Şarap|
|$se_moder_name|Moder|
|$se_moder_start|Moder'ın yakarışlarını ölüm bile dindiremiyor.|
|$se_moder_tooltip|Tekneyle açıldığında daima kuyruk rüzgârından faydalan.|
|$se_noisemod|Gürültü|
|$se_poison_name|Zehir|
|$se_poison_start|Zehirlendin|
|$se_poison_tooltip|Zehir hasarı.|
|$se_poisonres_name|Zehir direnci|
|$se_potion_barleywine_tooltip|Ateşten daha az hasar alıyorsun.|
|$se_potion_bugrepellent_tooltip|Bu iksirin taze kokusu, bazı istenmeyen yoldaşları uzak tutar.|
|$se_potion_bzerker_tooltip|Kısa bir süreliğine deli ve çıldırmış bir yaratık gibi vahşice savaşırsın.|
|$se_potion_eitrlingering_tooltip|Zaman içinde eitr yenilenmesi.|
|$se_potion_eitrminor_tooltip|Zaman içinde eitr.|
|$se_potion_frostresist_tooltip|Soğuğa karşı korunuyorsun.|
|$se_potion_hasty_tooltip|Büyü içine dolarak hiç olmadığı kadar hızlı hareket etmeni sağlar.|
|$se_potion_healthlingering_tooltip|Zaman içinde can yenilenmesi.|
|$se_potion_healthmajor_tooltip|Zaman içinde sağlık.|
|$se_potion_healthmedium_tooltip|Zaman içinde sağlık.|
|$se_potion_healthminor_tooltip|Zaman içinde sağlık.|
|$se_potion_lightfoot_tooltip|O kadar çevik ve hafif hissediyorsun ki sanki doğrudan Yggdrasil'in dallarına zıplayabilirsin.|
|$se_potion_poisonresist_tooltip|Zehirden daha az hasar alıyorsun.|
|$se_potion_staminalingering_tooltip|Zaman içinde enerji yenilenmesi.|
|$se_potion_staminamedium_tooltip|Hızlı enerji yenileyici.|
|$se_potion_staminaminor_tooltip|Hızlı enerji yenileyici.|
|$se_potion_start|İksiri içiyorsun|
|$se_potion_strength_tooltip|İçinde inanılmaz bir güç akar ama sadece kısa bir süreliğine...|
|$se_potion_swimmer_tooltip|Dalgalara göğüs gererken deniz tanrısının gücü seninle olsun.|
|$se_potion_tamer_tooltip|Hayvanlar, seni sürülerine daha kolay kabul eder.|
|$se_potion_tasty_tooltip|Enerji ve sağlık yenilenmesi zayıf.|
|$se_potion_trollpheromones_tooltip|Yakındaki troller, çiftleşme dönemlerinin de etkisiyle bölgelerini korumak için öfkeye kapılacak.|
|$se_puke_name|Kötü hissediyorsun|
|$se_puke_start|Kötü hissediyorsun|
|$se_puke_tooltip|Kötü hissettiğin için yemeğini tutamıyorsun.|
|$se_queen_name|Kraliçe|
|$se_queen_start|Kraliçe'nin gücü serbest kaldı|
|$se_queen_tooltip|Daha hızlı madencilik ve fazladan Eitr yenilenmesi.|
|$se_rested_comfort|Rahat|
|$se_rested_name|Dinlenmiş|
|$se_rested_start|Dinlenmiş hissediyorsun|
|$se_rested_stop|Artık dinç hissetmiyorsun|
|$se_rested_tooltip|Dinlenmiş hissediyorsun ve sağlık ile enerji yenilenmesi daha yüksek.|
|$se_resting_name|Dinleniyorsun|
|$se_resting_start|Dinleniyorsun|
|$se_resting_tooltip|Şu anda dinleniyorsun ve sağlık ile enerji yenilenmesi büyük ölçüde daha yüksek.|
|$se_rootseteffect_name|Geliştirilmiş okçuluk|
|$se_rootseteffect_tooltip|Kadim kökler yay yeteneğini odaklamana yardımcı olur.|
|$se_runstamina|Koşma enerji tüketimi|
|$se_shelter_name|Sığınak|
|$se_shelter_start|Sığınaktasın|
|$se_shelter_stop|Açıktasın|
|$se_shelter_tooltip|Hava şartlarından korunuyorsun.|
|$se_shield|Büyü Bariyeri|
|$se_shield_damage|Hasar emilimi (yeteneğe bağlı):|
|$se_shield_description|Hasar emen büyülü bir kalkan|
|$se_shield_ttl|Süre:|
|$se_slimed_name|Çamurlu|
|$se_slowfall_name|Tüy düşüşü|
|$se_slowfall_tooltip|Yerçekimi, entropinin bir etkisi değil de ne ki?|
|$se_smoked_name|Dumandan etkilendin|
|$se_smoked_start|Dumandan nefes alamıyorsun|
|$se_smoked_tooltip|Duman solumak sağlığa zararlıdır.|
|$se_sneakmod|Gizlilik|
|$se_sneakstamina|Gizlenme enerji tüketimi|
|$se_softdeath_name|Yetenek puanı kaybı engellendi|
|$se_softdeath_tooltip|Ölürsen yetenek puanı kaybetmeyeceksin.|
|$se_spirit_name|Ruh|
|$se_spirit_tooltip|Ölügezerlere karşı fazladan hasar.|
|$se_stagger|Sersemleme direnci|
|$se_stamina|Enerji|
|$se_stamina_upfront|Enerji kazanımı|
|$se_staminapotion_name|Enerji iksiri|
|$se_staminaregen|Enerji yenileme|
|$se_staminaupgrade_name|Enerji yükseltmesi|
|$se_staminaupgrade_stop|Daha kuvvetli hissediyorsun|
|$se_swimstamina|Yüzme enerji tüketimi|
|$se_tared_name|Katran bulaştı|
|$se_tared_start|Cılk katranla kaplısın|
|$se_tared_tooltip|Cılk katran seni yavaşlatıyor|
|$se_theelder_name|Kadim Olan|
|$se_theelder_start|Kadim Olan'ın gücünü damarlarında hissediyorsun.|
|$se_theelder_tooltip|Daha çabuk odun kesme.|
|$se_trinketblackdamagedealth|Demir Deri|
|$se_trinketblackdamagedealth_description|Daha fazla gürz hasarı ve hızlı bir sağlık yenilenmesi.|
|$se_trinketblackdtamina|Gölge Örtüsü|
|$se_trinketblackdtamina_description|Daha yüksek kaçınma yeteneği ve daha düşük bloklama enerji bedeli.|
|$se_trinketbronzehealth|Ormanın Kalbi|
|$se_trinketbronzehealth_description|Daha yüksek can yenilenmesi.|
|$se_trinketbronzestamina|Bronz Tılsım|
|$se_trinketbronzestamina_description|Daha yüksek enerji yenilenmesi.|
|$se_trinketcarapaceeitr|Titreşen Küpeler|
|$se_trinketcarapaceeitr_description|Daha yüksek eitr yenilenmesi.|
|$se_trinketchitinswim|Kader Yüzgeci|
|$se_trinketchitinswim_description|Daha düşük yüzme enerji bedeli ve daha yüksek yüzme hızı.|
|$se_trinketflametaleitr|Jörmundcuk|
|$se_trinketflametaleitr_description|Daha yüksek element ve kan büyüsü yeteneği, ve hızlı bir eitr yenilenmesi patlaması.|
|$se_trinketflametalstaminahealth|Ateş Kalp|
|$se_trinketflametalstaminahealth_description|Hızlı bir can ve enerji yenilenmesi.|
|$se_trinketironhealth|Demir Broş|
|$se_trinketironhealth_description|Daha yüksek zırh ve hızlı bir sağlık yenilenmesi.|
|$se_trinketironstamina|Halhal|
|$se_trinketironstamina_description|Daha yüksek koşma hızı ve hızlı bir enerji yenilenmesi.|
|$se_trinketscalestaminadamage|Çınlayan Pranga|
|$se_trinketscalestaminadamage_description|Daha yüksek kesme hasarı ve hızlı bir enerji yenilenmesi.|
|$se_trinketsilverdamage|Kurt Bakışı|
|$se_trinketsilverdamage_description|Daha yüksek yay yeteneği, mızrak yeteneği ve delme hasarı.|
|$se_trinketsilverresist|Kristal Kalp|
|$se_trinketsilverresist_description|Künt, kesici, ve delici hasara karşı daha fazla dayanıklılık.|
|$se_trollseteffect_name|Sinsi|
|$se_trollseteffect_tooltip|Daha iyi gizlenmeni sağlar.|
|$se_ttl|Etki süresi|
|$se_warm_name|Sıcak|
|$se_warm_start|Sıcak hissediyorsun|
|$se_warm_tooltip|Isındın.|
|$se_wet_name|Islak|
|$se_wet_repeat|Islandın|
|$se_wet_start|Islandın|
|$se_wet_stop|Kurulandın|
|$se_wet_tooltip|Sağlık ve enerji yenilenmen düşüyor.|
|$se_windrun_name|Rüzgâr Koşusu|
|$se_windrun_tooltip|Rüzgâr eşliğinde koşarken daha hızlı ol. Rüzgâr seni zafere götürsün!|
|$se_wishbone_name|Lades kemiği|
|$se_wishbone_start|Gizli nesneleri sezebiliyorsun|
|$se_wishbone_tooltip|Gizli hazineleri bulmana yardım eder. Uyarının arttığı yöne doğru git.|
|$se_yagluth_name|Yagluth|
|$se_yagluth_start|Yagluth'un kozmik güçleri.|
|$se_yagluth_tooltip|Büyü hasarı ve şimşek hasarına karşı direnç.|
|$settings_3dresolution|3B Çözünürlük Limiti|
|$settings_accessibility|Erişilebilirlik|
|$settings_account|Account|
|$settings_altkeys|(Alternatif Düğmeler)|
|$settings_altplace|Alternatif Yerleştirme|
|$settings_animateradial|Dairesel Menü Animasyonu|
|$settings_animateradial_descr|Dairesel menüyü açıp kapatırken ve bir simgeyi seçerken animasyon efekti gösterilir.|
|$settings_antialiasing|Kenar yumuşatma|
|$settings_apply|Uygula|
|$settings_attack|Saldır|
|$settings_audio|Ses|
|$settings_autobackup|Otomatik yedek geçmişi|
|$settings_autobackup_descr|Kaç kayıt alınacağını seç.|
|$settings_automatic|Otomatik|
|$settings_autopickup|Oto-Topla'yı Aç / Kapa|
|$settings_autorun|Otomatik koşma|
|$settings_autorun_descr|Karakter koşmaya başladıktan sonra durana kadar veya enerjisi tükenene kadar koşmaya devam eder.|
|$settings_backward|Geri|
|$settings_barber_look|Etrafa bak|
|$settings_bigpicturetext|* Oyun Kumandası metin giriş kutucukları ve sohbet yalnızca Steam Geniş Ekran modunda oynarken kullanılabilir.|
|$settings_bilinear|Düzleştirme|
|$settings_block|Savunma|
|$settings_bloom|Bloom|
|$settings_buildcat|İnşaat Kategorisi|
|$settings_buildmenu|İnşaat Menüsü|
|$settings_buildnext|Sonraki inşa seçeneği|
|$settings_buildprev|Önceki inşa seçeneği|
|$settings_ca|Renk sapması|
|$settings_camera_sensitivity|Kamera Hassasiyeti|
|$settings_camerashake|Kamera Sarsılması|
|$settings_camerashake_descr|Karakter hasar aldığında ekran sallanır.|
|$settings_cameratilt|Sürükleyici kamera|
|$settings_cameratilt_descr|Teknedeyken kamera hareketleri dalgalardan etkilenir.|
|$settings_camzoom|Zoom in/out view|
|$settings_camzoomin|Görüntüyü Yakınlaştır|
|$settings_camzoomout|Görüntüyü Uzaklaştır|
|$settings_chat|Sohbet|
|$settings_close_radial|Dairesel Menüyü / İfade Çarkını Kapat|
|$settings_cloud_storage_warning|Yedekleri yüksek bir değere ayarlamak depolama alanının tükenmesine yol açabilir.|
|$settings_cloudsave|Bulut Kaydı|
|$settings_console|Konsolu Etkinleştir|
|$settings_console_desc|Özel hata ayıklama komutlarına ve özelliklerine erişim için kullanılan oyun içi konsolun açılmasını sağlar. -console komut satırı argümanı bu ayarı geçersiz kılar.|
|$settings_continousmusic|Sürekli müzik|
|$settings_controller_classic|Klasik|
|$settings_controller_default|Konsol|
|$settings_controls|Kontroller|
|$settings_crouch|Çömel|
|$settings_deleteplayfab_task_header|Deleting PlayFab account...|
|$settings_deleteplayfabaccount|Delete PlayFab account|
|$settings_deleteplayfabaccount_failure|Could not delete PlayFab account. Reason: |
|$settings_deleteplayfabaccount_ingamewarning|Can't delete PlayFab account while in a world. Please go to the main menu in order to delete your account.|
|$settings_deleteplayfabaccount_not_logged_in|You are not logged in to PlayFab|
|$settings_deleteplayfabaccount_success|PlayFab account deleted successfully!|
|$settings_deleteplayfabaccount_text|Are you sure you want to delete your PlayFab account and all associated data? Your worlds, characters, and any User-Generated Content such as buildings will NOT be removed. You can easily create a new account next time you try to access online services inside Valheim.|
|$settings_detailedparticlesystems|Ayrıntılı Partikül Sistemleri|
|$settings_distantshadows|Uzaktaki Gölgeler|
|$settings_dodge|Kaçın|
|$settings_dof|Alan Derinliği|
|$settings_double_tap|Çift Dokun|
|$settings_double_tap_descr|Aynı simgeye doğru çubuğa hızla iki kez dokunulduğunda bu simgeyle etkileşime geçilir.|
|$settings_draw|Ger|
|$settings_fishingcast|Olta At|
|$settings_fishingpull|Çek|
|$settings_flick_interact|Hızlı Etkileşim|
|$settings_flick_interact_descr|Çubuk, bir simgeye doğru hızlıca itildiğinde o simgeyle etkileşime geçilir.|
|$settings_forward|İleri|
|$settings_fpslimit|Kare Hızı Sınırı|
|$settings_fullscreen|Tam ekran|
|$settings_gamepadenabled|Oyun Kumandası Etkin|
|$settings_gameplay|Oynanış|
|$settings_glyphs|Glyphs|
|$settings_glyphs_ps|Alternatif İşaretler|
|$settings_gp|Yitik Güç|
|$settings_graphic_mode|Grafik Modu|
|$settings_graphics|Grafik|
|$settings_guicale_note|Minimum ekran çözünürlüğü 2560x1440|
|$settings_guiscale|Kullanıcı arayüzünü ölçekle|
|$settings_hide|Silahı Gizle/Göster|
|$settings_high|Yüksek|
|$settings_hotbarright|Arayüz Çubuğunda Dolaş|
|$settings_hotbaruse|Arayüz Çubuğu Eşyası Kullan|
|$settings_hoverselectspd|Simge Seçim Hızı|
|$settings_hoverselectspd_descr|Bir simgenin üzerindeyken ne kadar hızlı seçileceğini belirler. Simgenin üzerine gelerek seçmeyi devre dışı bırakmak için Kapalı olarak ayarla.|
|$settings_infinite|Bilgi|
|$settings_inventory|Envanter|
|$settings_invert_camera_x|Kamera X Eksenini Ters Çevir|
|$settings_invert_camera_y|Kamera Y Eksenini Ters Çevir|
|$settings_invertcamera|Kamerayı Ters Çevir|
|$settings_invertmouse|Fareyi Ters Çevir|
|$settings_joysens|Oyun Kumandası Hassasiyeti|
|$settings_jump|Zıpla|
|$settings_keyboardmouse|Klavye ve Fare|
|$settings_langchange_community|*Topluluk Çevirisi|
|$settings_langchange_notice|Oyun dilinin değişmesi için yeniden başlatınız.|
|$settings_language|Dil|
|$settings_left|Sol|
|$settings_legacysave|Eski Kayıt|
|$settings_lights|Partikül ışıkları|
|$settings_localsave|Yerel Kayıt|
|$settings_lod|Çizim mesafesi/Ayrıntı seviyesi|
|$settings_look|Bak|
|$settings_low|Düşük|
|$settings_map|Harita|
|$settings_mapzoom|Zoom in/out minimap|
|$settings_mastervol|Ses ayarı|
|$settings_medium|Orta|
|$settings_menu|Menü|
|$settings_minimapzoomin|Zoom minimap|
|$settings_misc|Diğer|
|$settings_motionblur|Hareket bulanıklığı|
|$settings_mousesens|Fare Hassasiyeti|
|$settings_move|Taşı|
|$settings_musicvol|Müzik Sesi|
|$settings_native|Varsayılan|
|$settings_nearestneighbor|Pikselli|
|$settings_nextsnap|Sonraki birleştirme noktası|
|$settings_open_emote_wheel|İfade Çarkını Aç|
|$settings_open_radial|Dairesel Menüyü Aç|
|$settings_persistentbackbtn|Persistent Back Button|
|$settings_persistentbackbtn_descr|When scrolling through a radial menu with multiple pages, the top-most element will always be a back button.|
|$settings_place|Yerleştirme|
|$settings_playfab_timed_out_header|PlayFab timed out! Make sure you are connected to the internet.|
|$settings_pointlights|Aktif Nokta Işıkları|
|$settings_pointlightshadows|Aktif Nokta Işık Gölgeleri|
|$settings_presskey|Bir tuşa bas|
|$settings_prevsnap|Önceki birleştirme noktası|
|$settings_quality_mode_balanced|Dengeli|
|$settings_quality_mode_balanced_description|Dengeli mod performans ve kalite modları arasındaki orta yoldur.|
|$settings_quality_mode_custom|Özel|
|$settings_quality_mode_customized|Özelleştirilmiş|
|$settings_quality_mode_not_supported|*Lütfen cihazını 120Hz grafik çıkışını destekleyecek şekilde ayarla.|
|$settings_quality_mode_performance|Performans|
|$settings_quality_mode_performance_description|Performans modu daha yüksek kare sayısı sağlayarak daha akıcı bir oynanış sağlar.|
|$settings_quality_mode_quality|Kalite|
|$settings_quality_mode_quality_description|Kalite modu kare sayısı yerine oyunun daha iyi görünmesine öncelik verir.|
|$settings_quality_mode_very_low|Çok düşük|
|$settings_quality_mode_very_low_description|Kalite çok düşüktür ancak zayıf bir sistemde de oynanabilir.|
|$settings_quickselect|Sağ Tuş ile İnşaat Seçimi|
|$settings_quickselect_descr|Farenin sağ düğmesiyle yapı parçası seçmeyi mümkün kılar.|
|$settings_radial|Dairesel Menü|
|$settings_radial_big|Büyük|
|$settings_radial_small|Küçük|
|$settings_radial_small_edge|Küçük Kenar|
|$settings_radialback|Geri (Dairesel Menü)|
|$settings_radialclose|Kapat (Dairesel Menü)|
|$settings_radialinteract|Etkileşime Geç (Dairesel Menü)|
|$settings_radialsecondaryinteract|Drop (Radial)|
|$settings_radialsize|Dairesel Menü Boyutu|
|$settings_radialsize_descr|Dairesel menünün boyutunu ve konumunu ayarlar.|
|$settings_reducebg|Arka plan performansını azalt|
|$settings_reducebg_descr|Oyun ekrana odaklı değilken (simge durumuna küçültüldüğünde) daha az kaynak kullanır.|
|$settings_reduceflashinglights|Yanıp sönen ışıkları azalt|
|$settings_reduceflashinglights_descr|Şimşek efektlerinin gücünü azaltır.|
|$settings_release_to_use_mode|Release To Use Mode|
|$settings_release_to_use_mode_descr|TODO-TRANSLATE|
|$settings_remove|Yık|
|$settings_renderscale|İşleme ölçeği|
|$settings_renderscale_descr|Arayüz haricindeki oyun elementlerinin çözünürlüğü. Performansı artırmak veya oyuna daha retro bir görünüş sağlamak için kullanılabilir.|
|$settings_requirerestart|Yeniden başlatmanız gerek|
|$settings_res|Çözünürlük|
|$settings_resetcontrols|Kontrolleri varsayılana döndür|
|$settings_resok|Düzgün çalışıyor mu?|
|$settings_right|Sağ|
|$settings_rotate|Çevir|
|$settings_run|Koş|
|$settings_secondaryattack|İkincil Saldırı|
|$settings_sfxvol|Efekt Seviyesi|
|$settings_shadowquality|Gölge kalitesi|
|$settings_shipcameratilt|Atmosferik Gemi Kamerası|
|$settings_show_button_hints|Buton İpuçlarını Göster|
|$settings_show_button_hints_descr|Menülerdeki kontroller hakkında bilgi sağlayan ipuçlarını etkinleştirir.|
|$settings_showgamepadmap|Oyun Kumandasını Göster|
|$settings_showkeyhints|Tuş İpuçlarını Göster|
|$settings_sit|Otur|
|$settings_softpart|Yumuşak partiküller|
|$settings_spiral_normal|Normal|
|$settings_spiral_off|Kapalı|
|$settings_spiral_slight|Hafif|
|$settings_spiraleffect|Spiral Merdiven Efekti|
|$settings_spiraleffect_descr|Spiral merdiven efektinin yoğunluğunu belirler. Birden çok sayfalı bir dairesel menüde gezinirken seçili simgenin yanındaki simgeler, spiral merdiven benzeri bir efekt oluşturacak şekilde ölçeklenip hareket ettirilir. Bu efekt kapatıldığında simgeler yavaşça kaybolur. Bu ayar için Dairesel Menü Animasyonu etkinleştirilmelidir.|
|$settings_ssao|Ekran uzayı ortam örtmesi|
|$settings_sunshafts|Güneş Işınları|
|$settings_swap_triggers|Tetikleri değiştir|
|$settings_tesselation|Tesselasyon|
|$settings_test|Test et|
|$settings_toggle_attack_towards_look_direction|Baktığın yöne doğru saldır|
|$settings_toggle_attack_towards_look_direction_tooltip|Kapatıldığında saldırılar kameranın baktığı yöne doğru gerçekleşir.|
|$settings_togglehud|Arayüzü aç/kapa|
|$settings_togglerun|Koşmayı aç/kapa|
|$settings_tutorialsenabled|Kuzgun ipuçlarını etkinleştir|
|$settings_tutorialsenabled_descr|Odin atıldığın maceralarda sana rehber olsun diye kuzgunlarını gönderecek.|
|$settings_unlimited|Unlimited|
|$settings_upscalingalgorithm|Çözünürlük Artırma Yöntemi|
|$settings_use|Kullan|
|$settings_useradial|Dairesel Menüyü Kullan|
|$settings_useradial_descr|Envanterde gezinmek için dairesel menüyü kullanmayı sağlar.|
|$settings_vegetation|Bitki örtüsü kalitesi|
|$settings_veryhigh|Çok Yüksek|
|$settings_vsync|Dikey eşitleme|
|$settings_walk|Yürü/Koş|
|$settings_zoomin|Yakınlaştır|
|$settings_zoomout|Uzaklaştır|
|$sfx_arrow_hit|Arrow thunking|
|$sfx_bird_death|Bird dying|
|$sfx_crow_idle|Crow cawing|
|$sfx_piercing_dmg|Weapon piercing|
|$sfx_ship_impact|Ship impact|
|$sfx_wishbone_closer|ringing sharper|
|$sfx_wishbone_far|giving off a dull ring|
|$sfx_wishbone_further|ringing with a low tone|
|$sfx_wishbone_med|ringing|
|$sfx_wishbone_near|ringing sharply|
|$ship_ashlandocean_warning|Fokurdayan sular geminin gövdesini aşındırıyor, geri dön!|
|$ship_cargo|Kargo|
|$ship_holdfast|Tutun|
|$ship_karve|Karve|
|$ship_longship|Viking Yelkenlisi|
|$ship_longship_ashlands|Drakkar|
|$ship_raft|Sal|
|$skill_axes|Baltalar|
|$skill_axes_description|Baltalarla verilen hasar.|
|$skill_blacksmithing|Demircilik|
|$skill_blocking|Bloklama|
|$skill_blocking_description|Silah veya kalkanla bloklarken emilen hasar.|
|$skill_bloodmagic|Kan büyüsü|
|$skill_bloodmagic_description|Kan büyüsü kullanırken hasar, eitr ve can tüketimi.|
|$skill_bows|Yaylar|
|$skill_bows_description|Yaylarla verilen hasar.|
|$skill_clubs|Sopalar|
|$skill_clubs_description|Sopalarla verilen hasar.|
|$skill_cooking|Aşçılık|
|$skill_cooking_description|Pişirme hızı ve ek getiri ihtimali.|
|$skill_crafting|Üretim|
|$skill_crafting_description|Üretim hızı ve inşa enerji kullanımı.|
|$skill_crossbows|Arbaletler|
|$skill_crossbows_description|Arbaletlerle isabetlilik ve hasar.|
|$skill_dodge|Kaçın|
|$skill_dodge_description|Kaçınırken enerjin tükenir.|
|$skill_elementalmagic|Element büyüsü|
|$skill_elementalmagic_description|Element büyüsü kullanırken hasar ve Eitr kullanımı.|
|$skill_farming|Tarım|
|$skill_farming_description|Enerji kullanımı, hasat menzili ve ek getiri ihtimali.|
|$skill_firemagic|Ateş büyüsü|
|$skill_firemagic_description|Bir şeyler yaz|
|$skill_fishing|Balıkçılık|
|$skill_fishing_description|Balık tutarken enerji tüketimi ve çekme hızı.|
|$skill_frostmagic|Ayaz büyüsü|
|$skill_frostmagic_description|Bir şeyler yaz|
|$skill_jump|Zıplama|
|$skill_jump_description|Zıplama yüksekliği.|
|$skill_knives|Bıçaklar|
|$skill_knives_description|Bıçaklarla verilen hasar.|
|$skill_pickaxes|Kazmalar|
|$skill_pickaxes_description|Kazmalarla verilen hasar.|
|$skill_polearms|Sırıklı silahlar|
|$skill_polearms_description|Sırıklı silahlarla verilen hasar.|
|$skill_ride|Yolculuk|
|$skill_ride_description|Hayvan hızı ve enerji tüketimi.|
|$skill_run|Koşma|
|$skill_run_description|Koşarken zamanla hızın yavaşlar ve enerjin tükenmeye devam eder.|
|$skill_shields|Kalkanlar|
|$skill_shields_description|not used.|
|$skill_sneak|Gizlilik|
|$skill_sneak_description|Gizlilikteyken enerjin tükenmeye devam eder.|
|$skill_spears|Mızraklar|
|$skill_spears_description|Mızraklarla verilen hasar.|
|$skill_swim|Yüzme|
|$skill_swim_description|Yüzerken enerjin tükenmeye devam eder.|
|$skill_swords|Kılıçlar|
|$skill_swords_description|Kılıçlarla verilen hasar.|
|$skill_unarmed|Silahsız|
|$skill_unarmed_description|Silahsızken verilen hasar.|
|$skill_woodcutting|Odun kesme|
|$skill_woodcutting_description|Ağaca baltayla verilen hasar.|
|$skill_woodworking|Ağaç işçiliği|
|$store_buy|Satın Al|
|$store_sell|Sat|
|$store_topic|Dükkân|
|$tamed_name_01|En yakın dostun|
|$text_hidden_notification_muted|METİN GİZLİ, YAZARIN SESİ KAPALI|
|$text_hidden_notification_ugc_settings|METİN GİZLİ, UGC AYARLARINI KONTROL ET|
|$text_mistlands_bosshint|Kapı mühürlendi. Bir daha açılmaması için anahtar fırlatılıp madenin derinliklerine saklandı. Kapı mühürlendi.|
|$text_player_arrived|Ben geldim!|
|$tool_batteringram|Koçbaşı|
|$tool_cart|El arabası|
|$tool_catapult|Mancınık|
|$tutorial_altar_label|Hugin: Yitik sunakları|
|$tutorial_altar_text|Yitiklerden birinin çağrı yerini buldun. Sunakta doğru adağı kurban edersen ortaya çıkacaklar.<br/><br/>Ama dikkatli ol çünkü Yitikler kolay hedef değiller. Çatışmaya girmeden önce iyisinden bir silah üret, en kaliteli zırhını giy ve karnını tıka basa doldur.|
|$tutorial_altar_topic|Yaratığın çağrısı|
|$tutorial_artisan_extension_label|Hugin: Zanaatkâr baskısı|
|$tutorial_artisan_extension_text|Artık mükemmel ısı yalıtımına sahip porselen fayanslar üretebilirsin.<br/>Güneye seyahat etmek istiyorsan onlara ihtiyacın olacak!|
|$tutorial_artisan_extension_topic|Yeni bir alet|
|$tutorial_ashlandocean_label|Hugin: Kaynar sular|
|$tutorial_ashlandocean_text|Görünüşe göre güneyin kaynar sularına ulaştın. Kesinlikle fark etmişsindir ki bu ölümcül bir arayış olabilir.<br/>Balıklara yem olmak istemiyorsan bu suları aşmak için daha dayanıklı bir şey inşa etmen gerekecek.|
|$tutorial_ashlandocean_topic|Aganta!|
|$tutorial_ashlands_label|Hugin: Kül Diyarı|
|$tutorial_ashlands_text|Onuncu dünyanın en sıcak tarafları olan Kül Diyarı'nı keşfettin.<br/>Burada su, bir kazan olmadan kaynar ve yanabilecek her şey yalnızca bir kıvılcımla tutuşabilir. Dikkatli ol!|
|$tutorial_ashlands_topic|Soğukkanlılığını koru!|
|$tutorial_bathtub_text|Atla! Su tam kıvamında!|
|$tutorial_batteringram_label|Hugin: Koçbaşı|
|$tutorial_batteringram_text|Düşman kaleleri sağlam malzemelerle inşa edilmiştir. Duvarlarını aşmak istiyorsan bunun gibi makinelere ihtiyacın olacak.|
|$tutorial_batteringram_topic|Kuvvetli bir kuşatma aracı|
|$tutorial_bed_label|Hugin: Yerleşme süreci|
|$tutorial_bed_text|Geceyi yatağında uyuyarak geçirirsen sabaha dinç ve enerjin dolu bir şekilde uyanırsın.<br/><br/>Evindeki olmazsa olmazlardan biri de eşyalarını depolayabileceğin sandıklardır. Keşif yaparken talihsiz bir durum yaşama ihtimaline karşı <color=yellow>yedek ekipman bulundurmak her zaman önemlidir</color>...|
|$tutorial_bed_topic|Savaş yorgunlarının dinlenme yeri!|
|$tutorial_bellfragment_label|Munin: Çan parçası|
|$tutorial_bellfragment_topic|Geçmişten bir parça|
|$tutorial_blackforest_label|Hugin: Kara Orman|
|$tutorial_blackforest_text|Kara Orman'a adım attın. Kara Orman, hazır olmayanlar için çok tehlikeli bir yer olabilir. Eikthyr'i öldürerek kendini kanıtla.|
|$tutorial_blackforest_topic|Geri dön! Burası tehlikeli bir yer|
|$tutorial_bosstrophy_label|Hugin: Yitik avı ganimeti|
|$tutorial_bosstrophy_text|Yitik avı ganimetini Kurban Taşları'na götür ve Tanrıları hoşnut etmek için bu ganimeti adak olarak sun.|
|$tutorial_bosstrophy_topic|Tebrikler savaşçı!|
|$tutorial_cold_label|Hugin: Soğuk|
|$tutorial_cold_text|Islandığında veya geceleri hava sıcaklığı düştüğünde, soğuğun etkilerini hissedeceksin, enerji yenilenme seviyen düşmeye başlayacak.<br/><br/>Böyle bir durumda en iyi yer, ateşin başı olacaktır.|
|$tutorial_cold_topic|Hava koşullarına dikkat et|
|$tutorial_crypt_label|Hugin: Zindanlar|
|$tutorial_crypt_text|10. Dünya'nın dört bir köşesinde mahzen ve zindanlar bulunabilir. Geçmişin hatırası olan bu yerler, çoğunlukla evvelde yitip gitmiş uygarlıkların hazineleriyle doludur.<br/><br/>Yanında bir ışık kaynağı taşımayı unutma!|
|$tutorial_crypt_topic|Bu toprağın altında hazineler yatıyor|
|$tutorial_death_label|Hugin: Ölüm|
|$tutorial_death_text|Her öldüğünde, <color=yellow>yeteneklerinin küçük bir kısmını unutacak ve kazanın yaşandığı yerde eşyalarını düşüreceksin</color>.<br/><br/>Keşke evde olsaydım diyorsan da evinin olduğu yere <color=yellow>bir yatak inşa etmeni</color> öneririm.|
|$tutorial_death_topic|Ölümcül bir darbe aldın!|
|$tutorial_dvergr_label|Hugin: Cüceler|
|$tutorial_dvergr_text|Gak gaaak! İnatçı ahmaklar! Cüceler yabancılara ve berduşlara tahammül eder ama çabuk sinirlenirler... ve eminim eninde sonunda bunu başaracaksın.|
|$tutorial_dvergrhalls_label|Hugin: Cüceyuvaları|
|$tutorial_dvergrhalls_text|Eminim hakir kardeşim de söylüyordur, aşağımızdaki salonlar çok etkileyici....<br/><br/>Sadece dolambaçlı tünellerde ve gizli geçitlerde kaybolmamaya çalış, olur mu?|
|$tutorial_eitr|Görünüşe göre Yggdrasil dalından bir yudum almışsın ve eminim bu çok hoşuna gitti. <br/>Etrafında dönüp durduğunu gördüğün o renkli, eğlenceli ışıklar artık yalnızca kafanda olmayabilir. <br/>Sadece bunu bir alışkanlık haline getirme. Pek büyülü diyemeyeceğin uzun vadeli etkileri olabilir.|
|$tutorial_eitr_label|Hugin: Eitr|
|$tutorial_eitr_text|Sisler bu topraklarda yaşayan ve yetişen her şeye sızar ve artık senin de bir parçan.<br/><br/>Eitr sayesinde çeşit çeşit güçlü büyüler kullanabileceksin. Daha fazla Eitr enerjili malzeme bulmaya çalışarak kabiliyetini geliştir.|
|$tutorial_food_label|Hugin: Yiyecek|
|$tutorial_food_text|Bir atıştırmalık buldun, bunu yiyerek sağlığını ve enerjini arttırabilirsin.<br/><br/>Çok geçmeden tekrar acıkacağını unutma ve yanında en az birkaç farklı yiyeceği hazır bulundur.|
|$tutorial_food_topic|Leziz bir lokma!|
|$tutorial_food2_text|Bir atıştırmalık buldun ve bu atıştırmalığı tüketerek açlığını giderebilirsin. Bazı yemek çeşitleri, Sağlık için daha iyidir ve sersemlemeden önce ne kadar hasar alacağını belirlerken, diğerleri enerji değerini güçlendirir. Bazıları da tamamıyla fayda sağlar!<br/><br/>Çok geçmeden tekrar acıkacağını unutma ve yanında en azından birkaç farklı yiyeceği hazır bulundur.|
|$tutorial_giants_label|Hugin: Dev kalıntıları|
|$tutorial_giants_text|Gaak! Kemiklerini kır ve kafataslarını yar!|
|$tutorial_guardstone_label|Hugin: Nazar|
|$tutorial_guardstone_text|Nazar, bulunduğu bölgede diğer vikinglerin inşa etmesini engelleyen güçlü bir aura yayar. Ayrıca etkisi altındaki kapıları da kilitler.|
|$tutorial_guardstone_topic|Bir Nazar inşa ettin|
|$tutorial_haldor|Söylentiye göre bazı tüccarlar kendilerini 10. dünyada bulmuş. Özel eşyalara ihtiyaç duyarsan onları bulmak isteyebilirsin.<br/><br/>Onlardan biri bu orman gibi bir yerlerde olmalı. Daha fazla yardımcı olamadığım için çok üzgünüm. Gözünü dört açman gerekecek.|
|$tutorial_haldor_label|Hugin: Tüccar Haldor|
|$tutorial_haldor_topic|Merhaba, maceracı…|
|$tutorial_hammer_label|Hugin: Çekiç|
|$tutorial_hammer_text|Bu aleti kullanarak kudretli salonlar ve dayanıklı duvarlar inşa edebilirsin.<br/><br/>Bir <color=yellow>Tezgâh</color> üretirsen, başka yapılar üretmene de olanak tanır.|
|$tutorial_hammer_topic|Bir Çekiç ürettin|
|$tutorial_hildir|Duyduğum kadarıyla buraya benzer bir bölgede bir tüccar görülmüş. Kendisi tuhaf biri ama çok özel mallar satıyor olabilir.<br/><br/>Ama duyduğum söylentiye göre stoku baskına uğramış. Birinin yardımına ihtiyacı olabilir, sen de epey becerikli gözükmeye başladın.|
|$tutorial_hildir_label|Hugin: Tüccar Hildir|
|$tutorial_hildir_topic|Merhabalar.|
|$tutorial_hildirdungeon|Burası senin tahmin edebileceğinden biraz daha zorlayıcı görünüyor.<br/><br/>Daha fazla ilerlemeden durup bir düşünmek isteyebilirsin. Hazırlıklı olduğundan emin ol.|
|$tutorial_hildirdungeon_label|Gizemli konum|
|$tutorial_hildirdungeon_topic|Dur orada kâşif.|
|$tutorial_hoe_label|Hugin: Çapa|
|$tutorial_hoe_text|Toprağı işlemek için kullanacağın bu alet, çekiçle harika bir ikili diyebiliriz.<br/><br/>Çapayı kullanarak toprağı temizle ve araziyi şekillendir. Düz zeminde bina inşa etmek daha kolaydır!|
|$tutorial_hoe_topic|Bir Çapa ürettin|
|$tutorial_hunger_label|Hugin: Açlık|
|$tutorial_hunger_text|Beslenmek, öteki dünyada bile bir viking savaşçısı için <color=yellow>çok önemlidir</color>. Karnını tok tutmak, sağlığını güçlendirir ve enerji seviyeni yükseltir.<br/><br/>Yiyecekler dünyanın dört bir köşesinde bulabilir ve büyük bir çoğunluğunu <color=yellow>Ocağında</color> pişirebilirsin.|
|$tutorial_hunger_topic|Beslenmen gerekiyor|
|$tutorial_hunger2_text|Beslenmek, öteki dünyada bile bir viking savaşçısı için <color=yellow>en önemli şeydir</color>.<br/><br/>Yiyecekleri dünyanın dört bir köşesinde bulabilir ve büyük bir çoğunluğunu <color=yellow>Pişirme İstasyonu</color>'nda hazırlayabilirsin.<br/><br/>Üç çeşit yemek vardır: Biri sağlığını arttırır, öteki enerji güçlendirir ve diğeri de ikisinin karışımıdır. Beslenme şeklini ihtiyaçlarına göre tamamla!|
|$tutorial_inventory_label|Hugin: Envanter|
|$tutorial_inventory_text|Çoğu eşya için üretim gereklidir. Fakat yakın zamanda Midgard'dan göçtüğün için, nesnelerin gerçek şeklini hatırlaman gerekiyor. Nesneleri yerden aldığında hafızan geri gelecektir, umarım.<br/><br/>Küçük kardeşim Munin'in dediğine göre <color=yellow>Ahşap ve Taş</color> kullanarak Taş Balta üretilebiliyormuş.|
|$tutorial_inventory_topic|Envanterini düzenle|
|$tutorial_magetable_label|Hugin: Galdr masası|
|$tutorial_magetable_text|Efsunları öngörmekte şansını mı deniyorsun? Etkilendim büyücü!<br/><br/>Tılsım ve sihirler durumu kötüleştirmekten başka bir şey yapmaz. Bu sanatta ustalaşmak istiyorsan uygun yiyecekleri bulman gerekiyor. Gel gör ki buralarda toplanmayı bekleyen çeşit çeşit lezzetli yeni malzemeler var ve taptaze görünüyorlar. Ne şanslısın!|
|$tutorial_maptable_label|Hugin: Haritacılık masası|
|$tutorial_maptable_text|Bu masa sayesinde, 10. Dünya'daki keşiflerini kaydedebilir ve paylaşabilirsin. Araç kutusunu kullanarak ilerleyişini kaydet ve diğerlerinin keşfettiklerini kopyalamak için bu büyük haritaya göz at.|
|$tutorial_maptable_topic|Keşfini kaydet|
|$tutorial_ore_label|Hugin: Maden|
|$tutorial_ore_text|Ham madeni <color=yellow>Ocak</color>'ta işlemeden önce <color=yellow>Eritme Fırını</color>'nda rafine etmen gerekiyor.<br/><br/>Eritme Fırını inşa etmek için <color=yellow>Alaz Çekirdekleri</color>'ne ihtiyacın olacak. Bulmak için toprağın altındaki karanlık yerleri arayabilirsin.|
|$tutorial_ore_topic|Biraz maden buldun|
|$tutorial_pickaxe_label|Hugin: Kazma|
|$tutorial_pickaxe_text|Maden açısından zengin olan Kara Orman'da bakır bulabilir ve okyanus kıyısı boyunca uzanmış kalaylara rastlayabilirsin.<br/><br/>Şimdi git ve maden çalışmalarına başla!|
|$tutorial_pickaxe_topic|Bir Kazma ürettin|
|$tutorial_portal_label|Hugin: Portallar|
|$tutorial_portal_text|Portallar, dünyanın farklı yerlerine hızlı seyahat etmek için oldukça elverişlilerdir.<br/><br/>Tabii, önce gideceğin yere de bir tane inşa etmen ve birbirlerine otomatik bağlanmalarını sağlamak için ikisine de aynı ismi vermen gerekiyor!|
|$tutorial_portal_topic|Bir Portal inşa ettin|
|$tutorial_random1|Gaak!|
|$tutorial_random2|Fare miydi o...|
|$tutorial_random3|Hava fena değil...|
|$tutorial_random4|Meşgul müsün savaşçı? Daha sonra tekrar gelebilirim...|
|$tutorial_random5|Lütfen gel ve dinle.|
|$tutorial_random6|Gak? Gaak!|
|$tutorial_randomevent_label|Hugin: Düşman İstilaları|
|$tutorial_randomevent_text|Yaratıklar zaman zaman kampını kuşatabilirler. Bu gibi durumlardan bilek gücüne güvenemezsin, hayatta kalmak için güçlü savunmalar inşa et!|
|$tutorial_randomevent_topic|İstilaya uğradın|
|$tutorial_randomimp1|Selam savaşçı.|
|$tutorial_randomimp2|Tanıştığımıza sevindim.|
|$tutorial_randomimp3|Havadislerle geldim.|
|$tutorial_randomimp4|Anlatacak bir iki hikâyem var.|
|$tutorial_randomimp5|Gak? Gaak!|
|$tutorial_randomimp6|Dinle!|
|$tutorial_shield_label|Hugin: Kalkan|
|$tutorial_shield_text|Kalkan, gelen hasarı engellemene olanak tanır. <br/>Zamanlamanı mükemmel ayarlarsan, düşmanları <color=yellow>savuşturabilirsin</color>. <br/>Tabii dikkatli ol: Çok fazla saldırıyı bloklarsan sen de <color=yellow>sersemlersin</color>. Sersemlemeden alabileceğin hasar miktarı azami Sağlık değerine bağlı olduğu için, fazla hasar almayı planlıyorsan Sağlık değerini artıracak yemekler yemelisin.<br/>Daha ağır kalkanlar daha çok hasarı bloklar ama yavaşlamana sebep olurlar.|
|$tutorial_shield_topic|Bir Kalkan ürettin|
|$tutorial_shieldgenerator_label|Hugin: Kalkan oluşturucu|
|$tutorial_shieldgenerator_text|Kraa! Bu, göklerin üstüne fırlatabileceği ne varsa uzak tutacaktır.|
|$tutorial_shieldgenerator_topic|Koruma yolları|
|$tutorial_smelter_label|Hugin: Eritme Fırını|
|$tutorial_smelter_text|Ham madenlerini bu fırına koyduğunda işleme sonrası Ocak'ta kullanabileceğin işlenmiş külçe metal elde edersin.<br/><br/>Dökümhane'ni çalıştırmak için <color=yellow>Kömüre</color> ihtiyacın olacak. Kömüre ihtiyacın olduğunda Kömür Ocağı inşa ederek içine Ahşap atabilirsin.|
|$tutorial_smelter_topic|Bir Döküm Atölyesi inşa ettin|
|$tutorial_start_label|Hugin: İpucu|
|$tutorial_start_text|Başlangıç ipuçları: Çevrendeki nesnelerle etkileşime geçmeye çalış. Taş, dal, çakmaktaşı parçaları gibi bazı eşyalar toplanılabilir ve üretimde kullanılabilir.<br/><br/>Envanterini ve üretim ekranını açmak için $KEY_Inventory tuşuna bas.|
|$tutorial_start_topic|Valheim'e Hoş Geldin|
|$tutorial_stemple1_label|Hugin: Karşılama|
|$tutorial_stemple1_text|Benim adım Hugin ve buraya yolculuğunda rehberlik etmek için gönderildim.<br/><br/>Etrafını çevreleyen anıttaşların ismi Kurban Taşları. Bu taşlar, Valhalla'ya yükselmek için öldürmen gereken Yitikler'i simgeliyorlar.|
|$tutorial_stemple1_topic|10. Dünya'ya hoş geldin savaşçı|
|$tutorial_stemple2_label|Hugin: Vegvisirler|
|$tutorial_stemple2_text|Bu büyülü taşlar, Odin tarafından diyarın dört bir köşesine dağıtıldı ve Yitikler'in ayin bölgelerini gösteren tabela görevi görürler.<br/><br/>Yakından incelendiğinde bu taş, <color=yellow>ilk avın</color> Eikthyr'in çağrı yerinin konumunu gösterir. Eikthyr güçlü bir yaratık olduğu için onu alt etmeye kalkışmadan önce iyice silahlanmalısın.|
|$tutorial_stemple2_topic|Bu taş bir Vegvisir|
|$tutorial_stemple4_label|Hugin: Eikthyr yenildi|
|$tutorial_stemple4_text|Sana Eikthyr'in gücü bahşedildi, ihtiyaç anlarında bu gücü kullan.<br/><br/>Sonraki hedefin, Kara Orman'da yaşıyor. Oraya git, toprakları keşfet, kayıp hazine ve kaynaklarını ortaya çıkar.<br/><br/>Kadim Olan bekliyor!|
|$tutorial_stemple4_topic|Odin hoşnut oldu|
|$tutorial_tissueref_label|Hugin: Eitr rafinerisi|
|$tutorial_tissueref_text|Savaşçı, bu nasıl bir mekanizma?! Tehlikeli görünüyor...<br/><br/>Burada öğütücü bir huni var, içine ne koyacaksın merak ediyorum.|
|$tutorial_trinket_label|Hugin: Süsler|
|$tutorial_trinket_text|Gak gaaak! Savaşçı, o ellerinde gördüğüm parlak bir süs mü yoksa? Gururla tak! Bırak gücü damarlarında dolaşsın, düşmanlarını onun adına devir. Bunu yap ki saf adrenalinle körüklenen bir öfkeyle ödüllendirilesin!|
|$tutorial_trinket_topic|Adrenalini damarlarında hissediyorsun!|
|$tutorial_weight_label|Hugin: Aşırı Yük|
|$tutorial_weight_text|Çok fazla eşya taşıdığında aşırı yükten etkilenirsin, hızın yavaşlar ve enerji yenilenmen düşer.|
|$tutorial_weight_topic|Yükünü hafifletmen gerekiyor!|
|$tutorial_wishbone_label|Hugin: Lades Kemiği|
|$tutorial_wishbone_text|Görünüşe göre, kemik dolu bu kütlenin midesinde bir de Lades Kemiği varmış.<br/><br/>Bu kemik, seni toprağın altında saklı olan ganimetlere yönlendirecek güçlü bir büyüye sahip. Lades Kemiği, Gümüş maden damarlarını bulabilir ve bazen seni gömülü bir hazineye de yönlendirebilir...|
|$tutorial_wishbone_topic|Kemik Kütlesi sana bir veda hediyesi bıraktı|
|$tutorial_wispattractor_label|Hugin: Peri çeşmesi|
|$tutorial_wispattractor_text|Tuhaf bir yapı kesinlikle. Acaba neleri cezbedecek? Burada biraz oturup neler olacağını görelim...<br/><br/>Savaşçı, ben sıkıldım. Ayrıca benim işim senin gibilere gece boyunca bakıcılık yapmak değil... Tetikte olursan eminim ödüllendirilirsin.|
|$tutorial_workbench_label|Hugin: Tezgâh|
|$tutorial_workbench_text|Tezgâh, <color=yellow>karmaşık eşyalar</color> yapmana olanak tanır ve Çekiç ile inşa edebileceğin çokça <color=yellow>Yapı Parçasına</color> erişmeni sağlar.|
|$tutorial_workbench_topic|Bir Tezgâh inşa ettin|
|$tutorial_yggroot_label|Hugin: Yggdrasil kökleri|
|$tutorial_yggroot_text|Gaak! Gövdesinden yarılmış olsa da eski dişbudak ağacının köklerinde hâlâ büyü olduğunu görebiliyorum.<br/><br/>Tabii ki tasarladığın hiçbir silah kabuğu kıramayacak. Daha kabiliyetli ellerin ürünü olan daha iyi bir alete ihtiyacın var.|
|$ui_hold|Hold|
|$wolf_statue|Fenris mabedi|
|$xbox_error_crossplayprivilege|Platform ayrıcalık ayarların, başka platformlarla çapraz oynanışa izin veren sunucularda oynamanı önlüyor.|
|$xbox_menu_resolve_privilege|Ayrıcalık sorunu çözülsün mü?|
|$xbox_settings_controller_layout|Oyun Kumandası Düzeni|
|$xbox_settings_gamepad|Oyun Kumandası|
|$xbox_settings_joysens|Oyun kumandası hassasiyeti|
