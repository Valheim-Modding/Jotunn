...
namespace JotunnLib.Bootstrap
{
    internal static class AssemblyPublicizer
    {
        ...
        {
            Debug.Log($"Attempting to publicize: {file} at valheimPath: {ValheimPath}");
            var outputPath = Path.Combine(ValheimPath, Bootstrapper.PublicizedAssemblies);
            ...
            try
            {
                ...
                if (Directory.Exists(Path.Combine(ValheimPath, Bootstrapper.ValheimData,
                    Bootstrapper.Managed)))
                {
                    ((BaseAssemblyResolver)assemblyDefinition.MainModule.AssemblyResolver).AddSearchDirectory(Path.Combine(ValheimPath, Bootstrapper.ValheimData,
                      Bootstrapper.Managed));
                }
                if (Directory.Exists(Path.Combine(ValheimPath, Bootstrapper.ValheimServerData,
                    Bootstrapper.Managed)))
                {
                    ((BaseAssemblyResolver)assemblyDefinition.MainModule.AssemblyResolver).AddSearchDirectory(Path.Combine(ValheimPath, Bootstrapper.ValheimServerData,
                        Bootstrapper.Managed));
                }

                ((BaseAssemblyResolver)assemblyDefinition.MainModule.AssemblyResolver).AddSearchDirectory(Path.Combine(ValheimPath, Bootstrapper.UnstrippedCorlib));
            }
            ...
            var outputFilename = Path.Combine(outputPath, $"{Path.GetFileNameWithoutExtension(file)}_{Bootstrapper.Publicized}{Path.GetExtension(file)}");
            ...
            {
                Debug.Log($"Could not write file {outputFilename}.");
                Debug.Log(exception.Message);
                return false;
            }
